 ...
 */
 var MMGLang = {}
    ...
    MMGLang['en'] = {
        loadingText:'loading...',
        noDataText:'no data',
        loadErrorText:'Data loading is abnormal',
        showCell:'show column',
        noName:'Unnamed',
        theNpage:'Page %s',
        nextPage:'next Page',
        limitLabel:'{0} per page',
        totalCountLabel:'A total of <span>{0}</span> records'
    }
    var MMGridLang = function(key,arr){
        var str = MMGLang[window.conf.sysLang][key]; 
        console.log(window.conf.sysLang+'->'+str);
        if(typeof(arr)=='undefined')return str;
...
!function($){
    ...
    MMGrid.prototype = {
        ...
        , _initOptions: function(){
            ...
            if(cols){
                var bbHtml = ['<h1>'+MMGridLang('showCell')+'</h1>'];
                for(var colIndex=0; colIndex<cols.length; colIndex++){
                    bbHtml.push('<label ');
        ...
        , _loadAjax: function(args){
            ...
            //modify by gzshangtao
            window['WST_CURR_PAGE'] = params.page;
            that._showLoading();
            $.ajax({
                type: opts.method,
                url: opts.url,
    ...
    $.fn.mmGrid.defaults = {
        ...
        , sortName: ''
        , sortStatus: 'asc'
        , loadingText: MMGridLang('loadingText')
        , noDataText: MMGridLang('noDataText')
        , loadErrorText: MMGridLang('loadErrorText')
        , multiSelect: false
        , checkCol: false
        , indexCol: false
        , indexColWidth: 40
...
!function($){
        ...
        , _plain: function(page, totalCount, limit){
            ...
            $.each(list, function(index, item){
                ...
                if(item === '...'){
                    ...
                    $li.find('a').text(item).prop('title',MMGridLang('theNpage',[item])).on('click', function(e){
                        $el.data('page', item);
                        that.$mmGrid.load();
                    });
                }
            ...
            //
            var $next = $('<li class="next"><a title="'+MMGridLang('nextPage')+'">»</a></li>');
            if(page>=totalPage){
    ...
    $.fn.mmPaginator.defaults = {
        ...
        , limitLabel: MMGridLang('limitLabel')
        , totalCountLabel: MMGridLang('totalCountLabel')
        , limit: undefined
