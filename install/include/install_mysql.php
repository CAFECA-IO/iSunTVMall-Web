...
class Mysql {
	var $tablepre;
	function connect($dbhost, $db_port,$dbuser, $dbpw, $dbname = '', $dbcharset, $tablepre='',$db_charset = 'utf8mb4') {
		$this->tablepre = $tablepre;
		if(!$this->link = mysqli_connect($dbhost, $dbuser, $dbpw,'',$db_port)) {
			die('{status:-1,msg:"Can not connect to MySQL server:"'. mysql_error().'}');
		}
	    if($dbname) {
			$db_selected = mysqli_select_db($this->link,$dbname);
			if (!$db_selected) {
				$sql="CREATE DATABASE $dbname DEFAULT CHARACTER SET ".$db_charset.";";
				self::query($sql);
	...
	function excute($sql,$db_charset = 'utf8mb4'){
		...
		foreach($ret as $query) {
			...
			if($query) {
				if(strtoupper(substr($query, 0, 12)) == 'CREATE TABLE') {
		            ...
		            $query = preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $query)." ENGINE=InnoDB DEFAULT CHARSET=".$db_charset;
				}
