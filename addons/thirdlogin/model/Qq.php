...
class Qq {
    ...
    public function __construct($appId,$appKey,$callbackUrl,$evnName = 'WSTMart'){
        //接收从qq登录页返回来的值
    ...
    public function getToken(){
    	if($this->getEvn('expires_in')<time()){
	        $url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=".$this->appId."&client_secret=".$this->appKey."&code=".$this->code."&redirect_uri=".urlencode($this->callbackUrl);
     ...
     * */
    public function getClientId($accessToken,$unionId=1){
        $url = "https://graph.qq.com/oauth2.0/me?access_token={$accessToken}";
        if($unionId==1)$url.='&unionid=1';
        $str = $this->reqestUrl($url);//访问url获得返回值
        return $this->changeCallbackUrl($str,$accessToken);//返回经过json转码后的数组
    }
    /**
     * 获取用户信息
     * @param $clientId
     ...
     * @return 数组
     * */
    protected function changeCallbackUrl($str,$accessToken=''){
        if (strpos($str, "callback") !== false){
            ...
            if(isset($result['error']) && $result['error']=='100048')return $this->getClientId($accessToken,0);
