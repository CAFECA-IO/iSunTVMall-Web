...
function editAddress(addressId){
    ...
    if(addressId>0){
        ...
        $('#header-title').html(WST.lang('seckill_add_address'));
    }
    switchPage(1);
}
...
function saveAddress(addressId){
    ...
    if(userName==''){
        WST.msg(WST.lang('seckill_require_consignee'),'info');
        return false;
    }
    if(userPhone==''){
        WST.msg(WST.lang('seckill_require_contact_number'),'info');
        return false;
    }
    if(areaId==''){
        WST.msg(WST.lang('seckill_please_select_area'),'info');
        return false;
    }
    if(userAddress==''){
        WST.msg(WST.lang('seckill_require_detail_address'),'info');
        return false;
    }
    var param = {};
...
function inDefault(obj,id){
    ...
    $.post(WST.U('mobile/useraddress/setDefault'), {id:id}, function(data){
        ...
        if( json.status == 1 ){
            WST.msg(json.msg,'success');
            setTimeout(function(){
                location.href = WST.AU('seckill://useraddress/moindex');
            },1500);
        }else{
            WST.msg(json.msg,'warn');
...
function chooseAddress(id){
    location.href = WST.AU('seckill://carts/moSettlement','addressId='+id);
}
