...
$(document).ready(function(){
	$(".wst-pays,.wst-intergral").bind('click', function(event) {
		$('.wst-pays').hide();
		$('.wst-intergral').hide();
	});
	$(".wst-pays-box,.wst-intergral-box").bind('click', function(event) {
...
//积分
function intergral(id){
	if(id==1){
		$('.wst-intergral').fadeIn(200);
	}else{
		$('.wst-intergral').fadeOut(200);
	}
}
function toIsUseScore(obj,id){
	var html =$(obj).html();
	$('#intergral').html(html);
	$('#isUseScore').val(id);
	intergral(0);
...
function getCartMoney(){
	...
	$.post(WST.AU('seckill://carts/getCartMoney'),params,function(data,textStatus){
		...
		if(json.status==1){
		    json = json.data;
		    $('#freight').html(WST.lang('currency_symbol')+json.shops.freight);
		 	$('#goodsMoney').html(WST.lang('currency_symbol')+json.realTotalMoney);
		 	$('#totalMoney').val(json.shops.goodsMoney);
		 	$('#realTotalMoney').html(WST.lang('currency_symbol')+json.realTotalMoney);
		 	// 设置可用积分及积分可抵金额
		 	$('#userScores').html(json.maxScore);
			$('#userOrderMoney').html(WST.lang('currency_symbol')+json.maxScoreMoney);
		}
	});
}
//提交订单
function toSubmit(){
    ...
    params.payType = $('#paymentsh').val();
    params.payCode = $('#paymentsw').val();
    params.isUseScore = $('#scoreh').val();
    params.useScore = $('#userOrderScore').html();
    params.remark = $('#remark').val();
    params.deliverType = $('#givesh').val();
    params.isInvoice = $('#isInvoice').val();
    params.invoiceId = $('#invoiceId').val();
    params.invoiceClient = $('#invoice_obj').val()==1?$('#invoice_head').val():'个人';
    $('.wst-se-confirm .button').attr('disabled', 'disabled');
   	if(params.deliverType==0 && !params.addressId){
   		WST.msg(WST.lang('seckill_please_select_address'),'info');
   		return false;
   	}
   	WST.load(WST.lang('seckill_submitting'));
   	$.post(WST.AU('seckill://carts/submit'),params,function(data){
   		var json = WST.toJson(data);
   		WST.noload();
   		if(json.status==1){
	    	setTimeout(function(){
    	    if(params.payType==1 && $('#totalMoney').val()>0){
    	    	if(params.payCode=='alipays' || params.payCode==''){
    		  	...
    		  	}else if(params.payCode=='weixinpays'){
...
function addAddress(type,id){
	...
	location.href = WST.AU('seckill://useraddress/moindex','type='+type+'&addressId='+id);
}
function goGoods(id){
    location.href=WST.AU('seckill://goods/modetail','id='+id);
}
...
function getStores(val,name){
	...
	var areaName = (name!=undefined)?name:(($('#areaName').val()!='')?$('#areaName').val():WST.lang('seckill_require_select_area'));
	var shopId =  $('#shopId').val();
	$.post(WST.AU('seckill://carts/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>"+WST.lang('seckill_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
