...
function goodsList(){
    $('.content').dropload({
      ...
      domDown : {//上拉
          ...
          domRefresh : '<div class="dropload-refresh f15 "><i class="icon icon-20"></i>'+WST.lang('seckill_list_load_tips_1')+'</div>',
          domLoad    : '<div class="dropload-load f15"><span class="weui-loading"></span>'+WST.lang('seckill_list_load_tips_2')+'</div>',
          domNoData  : '<div class="dropload-noData" style="text-align:center;">'+WST.lang('seckill_list_load_tips_3')+'</div>'
      },
  ...
  var opts = {
    ...
    callback: function(data){
        if(data.last>0){
          ...
          $(".seckill_items_timer").html('<strong class="status_tip">'+WST.lang('seckill_curr_status_3')+'</strong><label class="lab_timer"></label>');
...
$(document).ready(function(){
  ...
  $('.times').click(function(){
    ...
    if(vstatus=='0'){
      $(".seckill_items_tit").html(WST.lang('seckill_tip_words2'));
      $(".status_tip").html(WST.lang('seckill_to_start'));
    }else{
      $(".seckill_items_tit").html(WST.lang('seckill_tip_words'));
