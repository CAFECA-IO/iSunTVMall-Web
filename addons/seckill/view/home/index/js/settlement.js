...
function delAddr(id){
	WST.confirm({content:WST.lang('seckill_confirm_del'),yes:function(index){
...
function getAddressList(obj){
	...
	var load = WST.load({msg:WST.lang('seckill_loading_data')});
	     ...
	     if(json.status==1){
	    	 if(json.data && json.data && json.data.length){
	    		 ...
	    		 for(var i=0;i<json.data.length;i++){
	    			...
	    			if(tmp.isDefault==1){
	    				html.push('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="j-default">'+WST.lang('seckill_default_address')+'</span>')
	    			}
	    			html.push('<div class="operate-box">');
	    			if(tmp.isDefault!=1){
	    				html.push('<a href="javascript:;" onclick="setDeaultAddr('+tmp.addressId+')">'+WST.lang('seckill_set_default_address')+'</a>&nbsp;&nbsp;');
	    			}
	    			html.push('<a href="javascript:void(0)" onclick="javascript:toEditAddress('+tmp.addressId+',this,1,1)">'+WST.lang('seckill_edit')+'</a>&nbsp;&nbsp;');
	    			if(json.data.length>1){
	    				html.push('<a href="javascript:void(0)" onclick="javascript:delAddr('+tmp.addressId+',this)">'+WST.lang('seckill_del')+'</a></div>');
	    			}
	    			html.push('<div class="wst-clear"></div>','</li>');
	    		 }
	    		 html.push('<a style="color:#1c9eff" onclick="editAddress()" href="javascript:;">'+WST.lang('seckill_stow_address')+'</a>');
...
function editAddress(){
	...
	if(isNoSelected){
		WST.msg(WST.lang('seckill_select_complete_address'),{icon:2});
		return;
	}
	layer.close(layerbox);
	var load = WST.load({msg:WST.lang('seckill_submitting')});
		...
		var json = WST.toJson(data);
	     if(json.status==1){
	    	 ...
	    	 if(params.isDefault==1){
	    		 $('#isdefault').html(WST.lang('seckill_default_address')).addClass('j-default');
	    	 }else{
...
function emptyAddress(obj,n){
	...
	layerbox =	layer.open({
					title:WST.lang('seckill_user_address'),
					type: 1,
					area: ['800px', '300px'],
					content: $('.j-edit-box')
					});
...
function getCartMoney(){
	...
	var load = WST.load({msg:WST.lang('seckill_calculating_price_tips')});
	$.post(WST.AU('seckill://carts/getCartMoney'),params,function(data,textStatus){
		layer.close(load);
		var json = WST.toJson(data);
		if(json.status==1){
		    json = json.data;
...
function checkSupportStores(){
	$.post(WST.AU('seckill://carts/checkSupportStores'),{},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data){
    			$("#deliver_btn").show();
...
function getStores(areaId){
	var shopId = $("#shopId").val();
	$.post(WST.AU('seckill://carts/getStores'),{shopId:shopId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div>"+WST.lang('seckill_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
	    	WST.msg(json.msg,{icon:2});
	    }
...
function checkStores(shopId){
	...
	layerbox =	layer.open({
					title:WST.lang('seckill_select_pick_up'),
					type: 1,
					area: ['800px', '600px'],
					content: $('.j-store-box'),
					btn:[WST.lang('seckill_confirm'),WST.lang('seckill_cancel')],
				    cancel:function(){
				    	//$('#menuBox').hide();
				    },
				    yes:function(){
						...
						if(!(storeId>0)){
							WST.msg(WST.lang('seckill_select_pick_up'));
							return;
						}
...
function submitOrder(){
	...
	params.addressId = params.s_addressId;
	params.isUseScore = $('#isUseScore').prop('checked')?1:0
	var load = WST.load({msg:WST.lang('seckill_submitting')});
	$.post(WST.AU('seckill://carts/submit'),params,function(data,textStatus){
		layer.close(load);
		var json = WST.toJson(data);
	    if(json.status==1){
...
function changeInvoice(t,str,obj){
	...
	var loading = WST.load({msg:WST.lang('seckill_searching')});
	$.post(WST.U('home/invoices/index'),param,function(data){
		layer.close(loading);
		// layer弹出层
		invoicebox =	layer.open({
			title:WST.lang('seckill_invoice_information'),
			type: 1,
			area: ['628px', '420px'], //宽高
			content: data,
			success :function(){
...
function invAdd(){
	...
	$('.inv_ul').append('<li class="inv_li inv_li_curr inv_editing"><input type="text" id="invoiceHead" placeholder="'+WST.lang('seckill_add_invoice_head')+'" value="" style="width:65%;height:21px;padding:1px;"><i></i><div style="top:8px;" class="inv_opabox"><a href="javascript:void(0)" onCLick="addInvoice()">保存</a></div></li>');
...
function addInvoice(){
	...
	if(head.length==0){
		WST.msg(WST.lang('seckill_invoice_head_required'));
		return;
	}
	...
	$.post(WST.U('home/Invoices/add'),{invoiceHead:head},function(data){
		...
		if(json.status==1){
			...
			code.push('<a href=\'javascript:void(0)\' onClick="invEdit(\''+json.data.id+'\',this)" class="edit_btn">'+WST.lang('seckill_edit')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="editInvoice(\''+json.data.id+'\',this)" style="display:none;" class="save_btn">'+WST.lang('seckill_save')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="delInvoice(\''+json.data.id+'\',this)">'+WST.lang('seckill_del')+'</a></div></li>');
...
function setInvoiceText(invoiceHead){
	...
	var text = WST.lang('group_no_invoice');
	if(invoiceType==1){
		text = (invoiceObj==0)?(WST.lang('seckill_invoice_special_personal_detail')):(WST.lang('seckill_invoice_special_detail',['<br />'+invoiceHead+'<br />']));
	}else{
		text = (invoiceObj==0)?(WST.lang('seckill_invoice_normal_personal_detail')):(WST.lang('seckill_invoice_personal_detail',['<br />'+invoiceHead+'<br />']));
	}
