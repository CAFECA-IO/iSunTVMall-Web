...
function getCartMoney(){
	var params ={};
	params.isUseScore = $('#isUseScore').val();
    ...
    $('.wst-se-confirm .button').attr('disabled', 'disabled');
   	...
   	$.post(WST.AU('seckill://carts/submit'),params,function(data){
   		...
   		if(json.status==1){
	    	setTimeout(function(){
	    		if(params.payType==1){
	    		  	if(params.payCode=='weixinpays' || params.payCode==''){
...
function addAddress(type,id){
	...
	location.href = WST.AU('seckill://useraddress/wxindex','type='+type+'&addressId='+id);
}
function goGoods(id){
    location.href=WST.AU('seckill://goods/wxdetail','id='+id);
}
...
$(document).ready(function(){
    ...
    backPrevPage(WST.AU('seckill://goods/wxlists'));
	// 地址弹出层
	...
	$.post(WST.AU('seckill://carts/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
	    ...
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>该区域内没有自提点,请选择快递运输</div>");
	    	}
	    }else{
