...
function initSaleGrid(p,ids){
    ...
    var cols = [
        ...
        {title:WST.lang('shopcustompage_goods_img'), name:'goodsImg', width: 100, renderer: function(val,item,rowIndex){
        ...
        {title:WST.lang('shopcustompage_goods_name'), name:'goodsName', width: 300,sortable:true,renderer: function(val,item,rowIndex){
                return "<span  ><p class='wst-nowrap'>"+item['goodsName']+"</p></span>";
            }},
        {title:WST.lang('shopcustompage_price'), name:'shopPrice' ,width:100,sortable:true, renderer: function(val,item,rowIndex){
            return WST.lang('currency_symbol')+item['shopPrice'];
        }},
        {title:WST.lang('shopcustompage_goods_sale_num'), name:'saleNum', width: 50,sortable:true,align:'center'},
        {title:WST.lang('shopcustompage_goods_stock'), name:'goodsStock' ,width:50,sortable:true,align:'center'}
    ];

    mmg = $('.mmg').mmGrid({height: h,indexCol: true, indexColWidth:50, cols: cols,method:'POST',
        url: WST.AU('shopcustompage://shopcustompagedecoration/goodPageQuery'), fullWidthRows: true, autoLoad: false,remoteSort: true,sortName:'goodsSn',sortStatus:'desc',
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function initGridCoupon(p,ids){
    ...
    var cols = [
        ...
        {title:WST.lang('shopcustompage_coupon_value'), name:'couponValue', width: 50,renderer:function(val,item,rowIndex){
                return WST.lang('currency_symbol')+item['couponValue'];
            }},
        {title:WST.lang('shopcustompage_type'), name:'startPrice', width: 60,renderer:function(val,item,rowIndex){
                return (item['useCondition']==1)?(WST.lang('shopcustompage_coupon_condition5',[item['useMoney'],item['couponValue']])):WST.lang("shopcustompage_cash_coupon");
            }},
        {title:WST.lang('shopcustompage_use_objects'), name:'floorPrice', width: 60,renderer:function(val,item,rowIndex){
                return (item['useObjects']==0)?WST.lang("shopcustompage_use_objects_type1"):WST.lang("shopcustompage_use_objects_type2");
            }},
        {title:WST.lang('shopcustompage_start_date'), name:'startDate', width: 120},
        {title:WST.lang('shopcustompage_end_date'), name:'endDate', width: 120},
        {title:WST.lang('shopcustompage_coupon_num'), name:'couponNum', width: 30},
    ];

    mmg_coupon = $('.mmg-coupon').mmGrid({height: h,indexCol: true, cols: cols,method:'POST',nowrap:true,
        url: WST.AU('shopcustompage://shopcustompagedecoration/couponPageQuery'), fullWidthRows: true, autoLoad: false,
        plugins: [
...
function initGridNew(p,ids){
    ...
    var cols = [
        ...
        {title:WST.lang('shopcustompage_article_id'), name:'articleId' ,width:20,sortable:true},
        {title:WST.lang('shopcustompage_title'), name:'articleTitle' ,width:200,sortable:true},
        {title:WST.lang('shopcustompage_cat'), name:'catName' ,width:100,sortable:true},
        {title:WST.lang('shopcustompage_create_time'), name:'createTime' ,width:120,sortable:true},
    ];

    mmg_new = $('.mmg-new').mmGrid({height:h,indexCol: true, cols: cols,method:'POST',nowrap:true,
        url: WST.AU('shopcustompage://shopcustompagedecoration/newPageQuery'), fullWidthRows: true, autoLoad: false,
        plugins: [
            $('#pg-new').mmPaginator({})
        ]
    });
...
$(function(){
    ...
    // 商品推荐组件标题名称长度计算
    $('.attr-content-goods-recommend').each(function(idx,item){
        $(item).find('.attr-content-goods-recommend-title').each(function(index,obj){
            var goods_recommend_title = $(obj).val();
            if(goods_recommend_title != undefined){
                $(obj).parent().find('.goods-recommend-title-length').html(goods_recommend_title.length);
            }
        });
    });
    // 搜索组件提示文字与热搜关键字长度计算
    ...
    WST.upload({
        pick:'#page-picker',
        formData: {dir:'shopcustompagedecoration'},
        ...
        progress:function(rate){
            $('#uploadMsg').show().html(WST.lang('shopcustompage_has_upload')+rate+"%");
        }
    });
    ...
    // 楼层商品组件图片上传初始化
    $(".floor-goods-icon-picker").each(function(idx,item){
                ...
                if(json.status==1){
                    ...
                    $(".effect-floor-goods").each(function(idx,item){
                        if(parseInt($(item).attr("floor_goods_num")) == floor_goods_num){
                            $(item).find('.floor-icon').attr('src',WST.conf.RESOURCE_PATH+'/'+img_url);
                        }
                    });
                }else{
                    WST.msg(json.msg,{icon:2});
    ...
    $('.floor-goods-select-bg-color').colpick({
        layout:'hex',
        submit:1,
        colorScheme:'dark',
        onSubmit:function(hsb,hex,rgb,el,bySetColor){
            if(!bySetColor){
                $(el).css('background','#'+hex);
                var floor_goods_num = $(el).parent().parent().attr("floor_goods_num");
                $(".effect-floor-goods").each(function(idx, item) {
                    if($(item).hasClass("floor_goods"+floor_goods_num)){
                        $(item).find('.floors').css('background','#'+hex);
                        $(el).parent().parent().find(".floor-goods-bg-color").val('#'+hex);
                    }
                });
            }
            $(el).colpickHide();
    ...
    $('.goods-recommend-select-bg-color').colpick({
        layout:'hex',
        submit:1,
        colorScheme:'dark',
        onSubmit:function(hsb,hex,rgb,el,bySetColor){
            if(!bySetColor){
                $(el).css('background','#'+hex);
                var goods_recommend_num = $(el).parent().parent().attr("goods_recommend_num");
                $(".effect-goods-recommend").each(function(idx, item) {
                    if($(item).hasClass("goods_recommend"+goods_recommend_num)){
                        $(item).find('.goods-recommend-container').css('background','#'+hex);
                        $(el).parent().parent().find(".goods-recommend-bg-color").val('#'+hex);
                    }
                });
            }
            $(el).colpickHide();
    ...
    $(".effect-title").click(function(event){
        ...
        $(".attr-title").html(WST.lang("shopcustompage_page_set"));
    ...
    $(".component-media-swiper").click(function(event){
        ...
        $(".effect-content").append("<div class='max-el-relative'><div class='effect-media-swiper "+swiper_class_name+"' swiper_num="+swiper_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='swiper'/><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_banner_01.jpg' alt=''/><div class='dots round'><span class='roundness'></span><span class='roundness'></span></div><div class='btn-del' onclick='delSwiperBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addSwiperClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_swiper'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-swiper "+swiper_attr_class_name+"' swiper_num="+swiper_num+"><div class='attr-content-swiper-item' ><label class='d-label'>"+WST.lang('shopcustompage_indicator_type')+"</label><input type='radio' value='1' name='swiper-indicator"+swiper_num+"' id='i-rectangle"+swiper_num+"' onclick='changeSwiperIndicator(this)'  /><label for='i-rectangle"+swiper_num+"' class='d-a-label'>"+WST.lang('shopcustompage_rectangle')+"</label><input type='radio' value='2' name='swiper-indicator"+swiper_num+"' id='i-square"+swiper_num+"' onclick='changeSwiperIndicator(this)'  /><label for='i-square"+swiper_num+"' class='d-a-label'>"+WST.lang('shopcustompage_square')+"</label><input type='radio' value='3' name='swiper-indicator"+swiper_num+"' id='i-roundness"+swiper_num+"' checked onclick='changeSwiperIndicator(this)'  /><label for='i-roundness"+swiper_num+"' class='d-a-label'>"+WST.lang('shopcustompage_roundness')+"</label></div><div class='attr-content-swiper-item' ><label class='d-label'>"+WST.lang('shopcustompage_swiper_interval')+"</label><select name='swiper-inteval"+swiper_num+"' onchange='changeSwiperInterval(this)'><option value='3'>3s</option><option value='4'>4s</option><option value='5'>5s</option><option value='6'>6s</option></select></div><div class='attr-content-swiper-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_top')+"</label><div class='attr-content-swiper-item-content'><input type='hidden' class='swiper-range-slider'  value='0' onchange='changeSwiperPadding(this,1)'/><span class='swiper-padding-top-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-swiper-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_bottom')+"</label><div class='attr-content-swiper-item-content'><input type='hidden' class='swiper-range-slider'  value='0' onchange='changeSwiperPadding(this,2)'/><span class='swiper-padding-bottom-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-swiper-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_left')+"</label><div class='attr-content-swiper-item-content'><input type='hidden' class='swiper-range-slider'  value='0' onchange='changeSwiperPadding(this,3)'/><span class='swiper-padding-left-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-swiper-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_right')+"</label><div class='attr-content-swiper-item-content'><input type='hidden' class='swiper-range-slider'  value='0' onchange='changeSwiperPadding(this,4)'/><span class='swiper-padding-right-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-swiper-item' ><label class='d-label'>"+WST.lang('shopcustompage_indicator_color')+"</label><div class='select-component-item-color swiper-indicator-select-color' ></div></div><div class='attr-content-swiper-item-img' ><div class='attr-content-swiper-img' img_url='upload/shopcustompagedecoration/base/pc_banner_01.jpg'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='banner-picker' id='banner-picker-"+swiper_num+"-1'><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_banner_01.jpg'/></div><div class='image-tips'>"+WST.lang('shopcustompage_swiper_image_tips2')+"</div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-swiper-link' /><div class='attr-btn-del' onclick='delSwiperItemBtn(this)'>X</div></div><div class='attr-content-swiper-item-add' onclick='addSwiperItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='indicator-type' value=3 /><input type='hidden' class='indicator-color' value='' /><input type='hidden' class='interval' value='3' /><input type='hidden' class='swiper-padding-top' value='0'><input type='hidden' class='swiper-padding-bottom' value='0'><input type='hidden' class='swiper-padding-left' value='0'><input type='hidden' class='swiper-padding-right' value='0'></div>");
        WST.upload({
    ...
    $(".component-floor-goods").click(function(event){
        ...
        $(".effect-content").append("<div class='max-el-relative'><div class='interval'></div><div class='effect-floor-goods "+floor_goods_class_name+"' floor_goods_num="+floor_goods_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='floor_goods'/><div class='wst-flex-row wst-jsb wst-ac floors-header'><div class='floor-left wst-flex-row wst-ac'><img class='floor-icon' src='"+base_url+"/upload/shopcustompagedecoration/base/floor_icon.png'><p class='floor-goods-title-text'>"+WST.lang('shopcustompage_floor_goods_caption')+"</p></div><div class='wst-flex-row wst-jfe wst-ac columns-title floor-right' ><div columns_item='1'>"+WST.lang('shopcustompage_column_title')+"1</div><div columns_item='2'>"+WST.lang('shopcustompage_column_title')+"2</div><div columns_item='3'>"+WST.lang('shopcustompage_column_title')+"3</div></div></div><div class='wst-flex-row wst-center floors'><div class='wst-flex-row floors-main'><div class='floor-left'><img class='floor-img' src='"+base_url+"/upload/shopcustompagedecoration/base/floor_goods_poster.png'/></div><div class='wst-flex-row wst-jsb wst-fw floor-right'><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_01.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_02.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_03.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_04.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_01.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_02.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_03.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_04.jpg'/><div class='goods-info' ><div class='wst-flex-column wst-jsb' ><p class='goods-name '>"+WST.lang('shopcustompage_goods_name')+"</p><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p></div></div></div></div></div></div><div class='btn-del' onclick='delFloorGoodsBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addFloorGoodsClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_floor_goods'));
        hideComponentAttrContent(1);
            $(".attr-content").append("<div class='attr-content-floor-goods "+floor_goods_attr_class_name+"' floor_goods_num="+floor_goods_num+"><div class='attr-content-floor-goods-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color floor-goods-select-bg-color' ></div></div><div class='attr-content-floor-goods-item' ><label class='d-label'>"+WST.lang('shopcustompage_floor_goods_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_floor_goods_caption')+"' class='attr-content-floor-goods-title' onkeyup='setFloorGoodsTitle(this)'/><span class='floor-goods-title-tip'><span class='floor-goods-title-length'>4</span>/10</span></div><div class='attr-content-floor-goods-item' style='margin-bottom:5px'><div class='attr-content-floor-goods-icon' img_url='/upload/shopcustompagedecoration/base/floor_icon.png' ><label class='d-label'>"+WST.lang('shopcustompage_floor_goods_icon')+"</label><div class='floor-goods-icon-picker' id='floor-goods-icon-picker-"+floor_goods_num+"' onclick='addFloorGoodsIconImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/floor_icon.png' /></div><div class='image-tips'>"+WST.lang('shopcustompage_floor_goods_icon_tips')+"</div></div></div><div class='attr-content-floor-goods-item' ><div class='attr-content-floor-goods-img' img_url='/upload/shopcustompagedecoration/base/floor_goods_poster.png' ><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='floor-goods-picker' id='floor-goods-picker-"+floor_goods_num+"' onclick='addFloorGoodsItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/floor_goods_poster.png' /></div><div class='image-tips'>"+WST.lang('shopcustompage_floor_goods_image_tips')+"</div></div></div><div class='attr-content-floor-goods-item'><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-floor-goods-link' /></div><div class='attr-content-floor-goods-item' ><label class='d-label'>"+WST.lang('shopcustompage_goods_sort')+"</label><select name='floor-goods-order"+floor_goods_num+"' onchange='changeFloorGoodsOrder(this)'><option value='1' >"+WST.lang('shopcustompage_goods_group_order1')+"</option><option value='2' >"+WST.lang('shopcustompage_goods_group_order2')+"</option><option value='3' >"+WST.lang('shopcustompage_goods_group_order3')+"</option><option value='4' >"+WST.lang('shopcustompage_goods_group_order4')+"</option><option value='5' >"+WST.lang('shopcustompage_goods_group_order5')+"</option><option value='6' >"+WST.lang('shopcustompage_goods_group_order6')+"</option><option value='7' >"+WST.lang('shopcustompage_goods_group_order7')+"</option><option value='8' >"+WST.lang('shopcustompage_goods_group_order8')+"</option></select></div><div class='attr-content-floor-goods-item' ><label class='d-label'>"+WST.lang('shopcustompage_select_goods')+"</label><input type='radio' value='1' name='floor-goods-select"+floor_goods_num+"' id='floor-goods-select-condition"+floor_goods_num+"' onclick='changeFloorGoodsSelect(this)'  checked/><label for='floor-goods-select-condition"+floor_goods_num+"'>"+WST.lang('shopcustompage_condition_select')+"</label><input type='radio' value='2' name='floor-goods-select"+floor_goods_num+"' id='floor-goods-select-manual"+floor_goods_num+"' onclick='changeFloorGoodsSelect(this)'  /><label for='floor-goods-select-manual"+floor_goods_num+"'>"+WST.lang('shopcustompage_manual_add')+"</label><input type='hidden' class='floor-goods-select-ids-value' value='' ><input type='hidden' class='floor-goods-select-cats-id-value' value="+hidden_cats_id+" ><input type='hidden' class='floor-goods-tag-value' value='' /><input type='hidden' class='floor-goods-min-price-value' value='' /><input type='hidden' class='floor-goods-max-price-value' value='' /></div><div class='attr-content-floor-goods-item-column' ><div class='wst-flex-row wst-ac'><label class='d-label'>"+WST.lang('shopcustompage_column_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_column_title')+"1' class='floor-goods-columns-name' columns_item='1' onkeyup='setFloorGoodsColumnsTitle(this)' style='width:150px !important;'/><span class='floor-goods-columns-name-tip'><span class='floor-goods-columns-name-length'>5</span>/6</span></div><label class='d-label'>"+WST.lang('shopcustompage_jump_link')+"</label><input type='text' value='' class='floor-goods-columns-link' style='width:150px !important;'/><div class='attr-btn-del' onclick='delFloorGoodsColumnItemBtn(this)'>X</div></div><div class='attr-content-floor-goods-item-column' ><div class='wst-flex-row wst-ac'><label class='d-label'>"+WST.lang('shopcustompage_column_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_column_title')+"2' class='floor-goods-columns-name' columns_item='2' onkeyup='setFloorGoodsColumnsTitle(this)' style='width:150px !important;'/><span class='floor-goods-columns-name-tip'><span class='floor-goods-columns-name-length'>5</span>/6</span></div><label class='d-label'>"+WST.lang('shopcustompage_jump_link')+"</label><input type='text' value='' class='floor-goods-columns-link' style='width:150px !important;'/><div class='attr-btn-del' onclick='delFloorGoodsColumnItemBtn(this)'>X</div></div><div class='attr-content-floor-goods-item-column' ><div class='wst-flex-row wst-ac'><label class='d-label'>"+WST.lang('shopcustompage_column_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_column_title')+"3' class='floor-goods-columns-name' columns_item='3' onkeyup='setFloorGoodsColumnsTitle(this)' style='width:150px !important;'/><span class='floor-goods-columns-name-tip'><span class='floor-goods-columns-name-length'>5</span>/6</span></div><label class='d-label'>"+WST.lang('shopcustompage_jump_link')+"</label><input type='text' value='' class='floor-goods-columns-link' style='width:150px !important;'/><div class='attr-btn-del' onclick='delFloorGoodsColumnItemBtn(this)'>X</div></div><div class='attr-content-floor-goods-item-column-add' onclick='addFloorGoodsColumnItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='floor-goods-order' value='1' /><input type='hidden' class='floor-goods-bg-color' value='#ffffff' /></div>");
        WST.upload({
            pick:'#floor-goods-icon-picker-'+floor_goods_num,
            formData: {dir:'shopcustompagedecoration'},
            accept: {extensions: 'gif,jpg,jpeg,png',mimeTypes: 'image/jpg,image/jpeg,image/png,image/gif'},
            callback:function(f){
                var json = WST.toJson(f);
                if(json.status==1){
                    $('#uploadMsg').empty().hide();
                    var img_url = json.savePath+json.thumb; //保存到数据库的路径
                    $("#floor-goods-icon-picker-"+floor_goods_num).parent().attr('img_url',img_url);
                    $("#floor-goods-icon-picker-"+floor_goods_num).find('img').attr('src',WST.conf.RESOURCE_PATH+'/'+json.savePath+json.thumb);
                    $(".effect-floor-goods").each(function(idx,item){
                        if($(item).hasClass(floor_goods_class_name)){
                            $(item).find('.floor-icon').attr('src',WST.conf.RESOURCE_PATH+'/'+json.savePath+json.thumb);
                        }
                    });
                }else{
                    WST.msg(json.msg,{icon:2});
    ...
    // 商品推荐组件
    $(".component-goods-recommend").click(function(event){
        var hidden_cats_id = $('.hidden-cats-id').val();
        componentRemoveClasses(1);
        var goods_recommend_num = 0;
        var goods_recommend_num_arr = [];
        $(".effect-goods-recommend").each(function(idx, item) {
            goods_recommend_num = parseInt($(item).attr('goods_recommend_num'));
            goods_recommend_num_arr.push(goods_recommend_num);
        });
        if(goods_recommend_num_arr.length>0){
            goods_recommend_num = Math.max.apply(null,goods_recommend_num_arr)+1;
        }else{
            goods_recommend_num = 1;
        }
        var goods_recommend_class_name = "goods_recommend"+goods_recommend_num;
        var goods_recommend_attr_class_name = "goods_recommend_attr"+goods_recommend_num;
        $(".effect-content").append("<div class='max-el-relative'><div class='interval'></div><div class='effect-goods-recommend "+goods_recommend_class_name+"' goods_recommend_num="+goods_recommend_num+"><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='goods_recommend'/><div class='wst-flex-row wst-jsb wst-ac goods-recommend-header'><p class='goods-recommend-title-text'>"+WST.lang('shopcustompage_manager_recommend')+"</p></div><div class='wst-flex-row wst-center goods-recommend-container' ><div class='wst-flex-row goods-recommend-main' ><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_01.jpg'/></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_02.jpg'/></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_03.jpg'/></div><div class='goods' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_04.jpg'/></div></div></div><div class='btn-del' onclick='delGoodsRecommendBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addGoodsRecommendClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_goods_recommend'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-goods-recommend "+goods_recommend_attr_class_name+"' goods_recommend_num="+goods_recommend_num+"><div class='attr-content-goods-recommend-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color goods-recommend-select-bg-color' ></div></div><div class='attr-content-goods-recommend-item' ><label class='d-label'>"+WST.lang('shopcustompage_goods_group_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_manager_recommend')+"' class='attr-content-goods-recommend-title' onkeyup='setGoodsRecommendTitle(this)'/><span class='goods-recommend-title-tip'><span class='goods-recommend-title-length'>4</span>/10</span></div><div class='attr-content-goods-recommend-item' ><label class='d-label'>"+WST.lang('shopcustompage_goods_group_title')+"</label><input type='radio' value='1' name='goods_recommend_title_is_show"+goods_recommend_num+"' id='goods-recommend-title-show"+goods_recommend_num+"' onclick='changeGoodsRecommendTitleDisplay(this)' checked /><label for='goods-recommend-title-show"+goods_recommend_num+"'>"+WST.lang('shopcustompage_show')+"</label><input type='radio' value='0' name='goods_recommend_title_is_show"+goods_recommend_num+"' id='goods-recommend-title-hide"+goods_recommend_num+"' onclick='changeGoodsRecommendTitleDisplay(this)'  /><label for='goods-recommend-title-hide"+goods_recommend_num+"'>"+WST.lang('shopcustompage_hide')+"</label></div><div class='attr-content-goods-recommend-item' ><label class='d-label'>"+WST.lang('shopcustompage_goods_sort')+"</label><select name='goods-recommend-order"+goods_recommend_num+"' onchange='changeGoodsRecommendOrder(this)'><option value='1' >"+WST.lang('shopcustompage_goods_group_order1')+"</option><option value='2' >"+WST.lang('shopcustompage_goods_group_order2')+"</option><option value='3' >"+WST.lang('shopcustompage_goods_group_order3')+"</option><option value='4' >"+WST.lang('shopcustompage_goods_group_order4')+"</option><option value='5' >"+WST.lang('shopcustompage_goods_group_order5')+"</option><option value='6' >"+WST.lang('shopcustompage_goods_group_order6')+"</option><option value='7' >"+WST.lang('shopcustompage_goods_group_order7')+"</option><option value='8' >"+WST.lang('shopcustompage_goods_group_order8')+"</option></select></div><div class='attr-content-goods-recommend-item' ><label class='d-label'>"+WST.lang('shopcustompage_select_goods')+"</label><input type='radio' value='1' name='goods-recommend-select"+goods_recommend_num+"' id='goods-recommend-select-condition"+goods_recommend_num+"' onclick='changeRecommendGoodsSelect(this)' checked/><label for='goods-recommend-select-condition"+goods_recommend_num+"'>"+WST.lang('shopcustompage_condition_select')+"</label><input type='radio' value='2' name='goods-recommend-select"+goods_recommend_num+"' id='goods-recommend-select-manual"+goods_recommend_num+"' onclick='changeRecommendGoodsSelect(this)' /><label for='goods-recommend-select-manual"+goods_recommend_num+"'>"+WST.lang('shopcustompage_manual_add')+"</label><input type='hidden' class='goods-select-ids-value' value='' /><input type='hidden' class='goods-select-cats-id-value' value="+hidden_cats_id+" /><input type='hidden' class='goods-tag-value' value='' /><input type='hidden' class='goods-min-price-value' value='' /><input type='hidden' class='goods-max-price-value' value='' /></div><input type='hidden' class='show-goods-recommend-title' value='1' /><input type='hidden' class='goods-recommend-order' value='1' /><input type='hidden' class='goods-recommend-bg-color' value='#ffffff' /></div>");
        $('.goods-recommend-select-bg-color').colpick({
            layout:'hex',
            submit:1,
            colorScheme:'dark',
        ...
        $(".attr-content-goods-recommend").each(function(idx, item) {
            if($(item).hasClass("goods_recommend_attr"+goods_recommend_num)){
                $(item).show();
            }else{
                $(item).hide();
            }
    ...
    $(".component-nav").click(function(event){
        ...
        nav_num += 1;
        if(nav_num > 1){
            WST.msg(WST.lang('shopcustompage_nav_only_add_one'),{time:1000,anim: 6});
            return;
        }
        var nav_class_name = "nav"+nav_num;
        var nav_attr_class_name = "nav_attr"+nav_num;
        $(".effect-content").append("<div class='max-el-relative'><div class='interval'></div><div class='effect-nav wst-flex-row wst-jc "+nav_class_name+"' nav_num="+nav_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='nav'/><div class='navs-main wst-flex-row wst-ac'><div class='nav-left'>"+WST.lang('shopcustompage_shop_goods_cat')+"</div><div class='nav-right wst-flex-row wst-jsa wst-ac'><div class='nav-item' nav_item='1'><div class='nav-text'>"+WST.lang('shopcustompage_bottom_text')+"</div></div><div class='nav-item' nav_item='2'><div class='nav-text'>"+WST.lang('shopcustompage_bottom_text')+"</div></div><div class='nav-item' nav_item='3'><div class='nav-text'>"+WST.lang('shopcustompage_bottom_text')+"</div></div><div class='nav-item' nav_item='4'><div class='nav-text'>"+WST.lang('shopcustompage_bottom_text')+"</div></div></div><div class='nav-btn'>"+WST.lang('shopcustompage_return_mall_index')+"</div></div><div class='btn-del' onclick='delNavBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addNavClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_nav'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-nav "+nav_attr_class_name+"' nav_num="+nav_num+"><div class='attr-content-nav-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color nav-select-bg-color' style='background:#ff6a53;'></div></div><div class='attr-content-nav-item' ><label class='d-label'>"+WST.lang('shopcustompage_show_cat')+"</label><input type='radio' value='1' name='nav-category"+nav_num+"'  id='n-category-show"+nav_num+"' onclick='changeNavCategoryDisplay(this)' checked /><label for='n-category-show"+nav_num+"'>"+WST.lang('shopcustompage_show')+"</label><input type='radio' value='0' name='nav-category"+nav_num+"' id='n-category-hide"+nav_num+"' onclick='changeNavCategoryDisplay(this)' /><label for='n-category-hide"+nav_num+"'>"+WST.lang('shopcustompage_hide')+"</label></div><div class='attr-content-nav-item' ><label class='d-label'>"+WST.lang('shopcustompage_show_button')+"</label><input type='radio' value='1' name='nav-button"+nav_num+"'  id='n-button-show"+nav_num+"' onclick='changeNavButtonDisplay(this)' checked /><label for='n-button-show"+nav_num+"'>"+WST.lang('shopcustompage_show')+"</label><input type='radio' value='0' name='nav-button"+nav_num+"' id='n-button-hide"+nav_num+"' onclick='changeNavButtonDisplay(this)' /><label for='n-button-hide"+nav_num+"'>"+WST.lang('shopcustompage_hide')+"</label></div><div class='attr-content-nav-item-group' nav_item='1'><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_content')+"</label><input type='text' value='"+WST.lang('shopcustompage_bottom_text')+"' class='attr-content-nav-text' onkeyup='setNavItemTitle(this)' /><span class='nav-text-tip'><span class='nav-text-length'>4</span>/4</span></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color nav-select-text-color' style='background:#ffffff;' ></div><input type='hidden' class='attr-content-nav-text-color' value='#ffffff' /></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-nav-link' /></div><div class='attr-btn-del' onclick='delNavItemBtn(this)'>X</div></div><div class='attr-content-nav-item-group' nav_item='2'><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_content')+"</label><input type='text' value='"+WST.lang('shopcustompage_bottom_text')+"' class='attr-content-nav-text' onkeyup='setNavItemTitle(this)' /><span class='nav-text-tip'><span class='nav-text-length'>4</span>/4</span></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color nav-select-text-color' style='background:#ffffff;' ></div><input type='hidden' class='attr-content-nav-text-color' value='#ffffff' /></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-nav-link' /></div><div class='attr-btn-del' onclick='delNavItemBtn(this)'>X</div></div><div class='attr-content-nav-item-group' nav_item='3'><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_content')+"</label><input type='text' value='"+WST.lang('shopcustompage_bottom_text')+"' class='attr-content-nav-text' onkeyup='setNavItemTitle(this)' /><span class='nav-text-tip'><span class='nav-text-length'>4</span>/4</span></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color nav-select-text-color' style='background:#ffffff;' ></div><input type='hidden' class='attr-content-nav-text-color' value='#ffffff' /></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-nav-link' /></div><div class='attr-btn-del' onclick='delNavItemBtn(this)'>X</div></div><div class='attr-content-nav-item-group' nav_item='4'><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_content')+"</label><input type='text' value='"+WST.lang('shopcustompage_bottom_text')+"' class='attr-content-nav-text' onkeyup='setNavItemTitle(this)' /><span class='nav-text-tip'><span class='nav-text-length'>4</span>/4</span></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color nav-select-text-color' style='background:#ffffff;' ></div><input type='hidden' class='attr-content-nav-text-color' value='#ffffff' /></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-nav-link' /></div><div class='attr-btn-del' onclick='delNavItemBtn(this)'>X</div></div><div class='attr-content-nav-item-add' onclick='addNavItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='nav-bg-color' value='#ff6a53' /><input type='hidden' class='show-nav-category' value='1' /><input type='hidden' class='show-nav-button' value='1' /></div>");
        $('.nav-select-bg-color').colpick({
            layout:'hex',
            submit:1,
            colorScheme:'dark',
    ...
    $(".component-notice").click(function(event){
        ...
        notice_num += 1;
        if(notice_num > 1){
            WST.msg(WST.lang('shopcustompage_notice_add_tips'),{time:1000,anim: 6});
            return;
        }
        var notice_class_name = "notice"+notice_num;
        var notice_attr_class_name = "notice_attr"+notice_num;
        $(".effect-content").append("<div class='el-relative'><div class='interval'></div><div class='effect-notice "+notice_class_name+"' notice_num="+notice_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='notice'/><div class='notice-info'><div class='notice-left'><img src='"+base_url+"/upload/shopcustompagedecoration/base/notice.png' alt=''/></div><div class='notice-right'><p>"+WST.lang('shopcustompage_here_input_notice_content')+"</p></div></div><div class='btn-del' onclick='delNoticeBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addNoticeClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_notice'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-notice "+notice_attr_class_name+"' notice_num="+notice_num+"><div class='attr-content-notice-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-notice-item-content'><input type='hidden' class='notice-range-slider' value='0' onchange='changeNoticePadding(this)'/><span class='notice-vertical-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-notice-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color notice-select-bg-color'></div></div><div class='attr-content-notice-item' ><label class='d-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color notice-select-text-color' style='background:#000000;'></div></div><div class='attr-content-notice-item'><label class='d-label'>"+WST.lang('shopcustompage_notice_icon')+"</label><div class='notice-picker' id='notice-picker-"+notice_num+"' onclick='addNoticeImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/notice.png'/></div></div><div class='attr-content-notice-item' ><label class='d-label'>"+WST.lang('shopcustompage_scroll_direction')+"</label><input type='radio' value='1' name='notice-direction' id='notice-direction-vertical' onclick='changeNoticeDirection(this)' checked /><label for='notice-direction-vertical' class='d-a-label'>"+WST.lang('shopcustompage_vertical')+"</label><input type='radio' value='2' name='notice-direction' id='notice-direction-horizontal' onclick='changeNoticeDirection(this)'   /><label for='notice-direction-horizontal' class='d-a-label'>"+WST.lang('shopcustompage_horizontal')+"</label></div><div class='attr-content-notice-item-group' ><div><label class='d-label'>"+WST.lang('shopcustompage_notice')+"</label><input type='text' value='"+WST.lang('shopcustompage_here_input_notice_content')+"' class='attr-content-notice-text'  onkeyup='setNoticeText(this)'/></div><div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-notice-link' /></div><div class='attr-btn-del' onclick='delNoticeItemBtn(this)'>X</div></div><div class='attr-content-notice-item-group' ><div><label class='d-label'>"+WST.lang('shopcustompage_notice')+"</label><input type='text' value='"+WST.lang('shopcustompage_here_input_notice_content')+"' class='attr-content-notice-text' onkeyup='setNoticeText(this)' /></div><div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-notice-link' /></div><div class='attr-btn-del' onclick='delNoticeItemBtn(this)'>X</div></div><div class='attr-content-notice-item-group-add' onclick='addNoticeItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='notice-bg-color' value='#ffffff' /><input type='hidden' class='notice-text-color' value='#000000' /><input type='hidden' class='notice-img' value='/upload/shopcustompagedecoration/base/notice.png' /><input type='hidden' class='notice-vertical-padding' value='0'><input type='hidden' class='notice-direction' value='1' /></div>");
        WST.upload({
    ...
    $(".component-search").click(function(){
        ...
        search_num += 1;
        if(search_num > 1){
            WST.msg(WST.lang('shopcustompage_search_add_tips'),{time:1000,anim: 6});
            return;
        }
        var search_class_name = "search"+search_num;
        var search_attr_class_name = "search_attr"+search_num;
        $(".effect-content").append("<div class='el-relative'><div class='interval'></div><div class='effect-search "+search_class_name+"' search_num="+search_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='search'/><div class='search-info '><div class='search-left'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg'/></div><div class='search-mid '><div class='search-mid-content wst-flex-row wst-jsb'><div class='wst-flex-row'><p class='search-type'>"+WST.lang('shopcustompage_search_goods')+"</p><p class='search-text'>"+WST.lang('shopcustompage_search_goods_demo')+"</p></div><div class='search-btn wst-flex-row wst-center'>"+WST.lang('shopcustompage_search')+"</div></div><div class='wst-flex-row search-hot-container'><div class='search-hot-item wst-flex-row wst-ac' hots_item='1'><p>"+WST.lang('shopcustompage_fruit')+"</p></div><div class='search-hot-item wst-flex-row wst-ac' hots_item='2'><div class='search-hot-line'></div><p>"+WST.lang('shopcustompage_phone')+"</p></div><div class='search-hot-item wst-flex-row wst-ac' hots_item='3'><div class='search-hot-line'></div><p>"+WST.lang('shopcustompage_clothes')+"</p></div></div></div><div class='search-right wst-flex-row wst-center'><div class='search-icon'></div><p>"+WST.lang('shopcustompage_my_cart')+"</p></div></div><div class='btn-del' onclick='delSearchBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addSearchClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_search'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-search "+search_attr_class_name+"' search_num="+search_num+"><div class='attr-content-search-item' ><div class='attr-content-search-img' img_url='/upload/shopcustompagedecoration/base/banner_01.jpg' ><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='search-picker' id='search-picker-"+search_num+"' onclick='addSearchItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' /></div><div class='image-tips'>"+WST.lang('shopcustompage_search_image_tips')+"</div></div></div><div class='attr-content-search-item'><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-search-link' /></div><div class='attr-content-search-item'><div class='attr-content-search-item-title'><label class='d-label'>"+WST.lang('shopcustompage_search_goods_prompt_text')+"</label></div><div class='attr-content-search-group'><div class='attr-content-search-item-content'><input type='text' value='"+WST.lang('shopcustompage_search_goods')+"' class='attr-content-search-type' item='1' onkeyup='setSearchTextType(this)' style='width:150px !important;'/><span class='search-type-tip'><span class='search-type-length'item='1'>3</span>/3</span></div><div class='attr-content-search-item-content'><input type='text' value='"+WST.lang('shopcustompage_search_goods_demo')+"' class='attr-content-search-text' item='1' onkeyup='setSearchText(this)' style='width:150px !important;'/><span class='search-text-tip'><span class='search-text-length' item='1'>9</span>/10</span></div></div></div><div class='search-tips'>"+WST.lang('shopcustompage_search_tips')+"</div><div class='attr-content-search-item'><div class='attr-content-search-item-title'><label class='d-label'>"+WST.lang('shopcustompage_search_shop_prompt_text')+"</label></div><div class='attr-content-search-group'><div class='attr-content-search-item-content'><input type='text' value='"+WST.lang('shopcustompage_search_shop')+"' class='attr-content-search-type' item='2' onkeyup='setSearchTextType(this)' style='width:150px !important;'/><span class='search-type-tip'><span class='search-type-length' item='2' >3</span>/3</span></div><div class='attr-content-search-item-content'><input type='text' value='"+WST.lang('shopcustompage_search_shop_demo')+"' class='attr-content-search-text' item='2' onkeyup='setSearchText(this)' style='width:150px !important;'/><span class='search-text-tip'><span class='search-text-length' item='2'>9</span>/10</span></div></div></div><div class='attr-content-search-hot-item' ><div class='wst-flex-row wst-ac'><label class='d-label search-label'>"+WST.lang('shopcustompage_hot_keyword')+"</label><input type='text' value='"+WST.lang('shopcustompage_fruit')+"' class='search-hots-text' hots_item='1' onkeyup='setSearchHotsText(this)' style='width:120px !important;'/><span class='search-hots-text-tip'><span class='search-hots-text-length'>2</span>/5</span></div><div class='attr-btn-del' onclick='delSearchHotItemBtn(this)'>X</div></div><div class='attr-content-search-hot-item' ><div class='wst-flex-row wst-ac'><label class='d-label search-label'>"+WST.lang('shopcustompage_hot_keyword')+"</label><input type='text' value='"+WST.lang('shopcustompage_phone')+"' class='search-hots-text' hots_item='2' onkeyup='setSearchHotsText(this)' style='width:120px !important;'/><span class='search-hots-text-tip'><span class='search-hots-text-length'>2</span>/5</span></div><div class='attr-btn-del' onclick='delSearchHotItemBtn(this)'>X</div></div><div class='attr-content-search-hot-item' ><div class='wst-flex-row wst-ac'><label class='d-label search-label'>"+WST.lang('shopcustompage_hot_keyword')+"</label><input type='text' value='"+WST.lang('shopcustompage_clothes')+"' class='search-hots-text' hots_item='3' onkeyup='setSearchHotsText(this)' style='width:120px !important;'/><span class='search-hots-text-tip'><span class='search-hots-text-length'>2</span>/5</span></div><div class='attr-btn-del' onclick='delSearchHotItemBtn(this)'>X</div></div><div class='attr-content-search-hot-item-add' onclick='addSearchHotItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div></div>");
        WST.upload({
    ...
    $(".component-coupon").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='interval'></div><div class='effect-coupon "+coupon_class_name+"' coupon_num="+coupon_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='coupon'/><div class='coupons'><div class='coupon-left wst-flex-column wst-jc wst-afs'><div class='coupon-title'>"+WST.lang('shopcustompage_coupon_center')+"</div><div class='coupon-desc'>"+WST.lang('shopcustompage_coupon_sub_title')+"</div></div><div class='coupon-right wst-flex-row'><div class='coupon-item wst-flex-column wst-center'><div class='wst-flex-row wst-afe'><div class='coupon-value-content'>"+WST.lang('currency_symbol')+"<span class='coupon-value'>5</span></div><div><div class='coupon-text'>"+WST.lang('shopcustompage_coupon')+"</div><div class='coupon-condition'>"+WST.lang('shopcustompage_coupon_condition_demo2')+"</div></div></div><div class='coupon-get'>"+WST.lang('shopcustompage_get_now')+"</div></div><div class='coupon-item wst-flex-column wst-center'><div class='wst-flex-row wst-afe'><div class='coupon-value-content'>"+WST.lang('currency_symbol')+"<span class='coupon-value'>5</span></div><div><div class='coupon-text'>"+WST.lang('shopcustompage_coupon')+"</div><div class='coupon-condition'>"+WST.lang('shopcustompage_coupon_condition_demo2')+"</div></div></div><div class='coupon-get'>"+WST.lang('shopcustompage_get_now')+"</div></div><div class='coupon-item wst-flex-column wst-center'><div class='wst-flex-row wst-afe'><div class='coupon-value-content'>"+WST.lang('currency_symbol')+"<span class='coupon-value'>5</span></div><div><div class='coupon-text'>"+WST.lang('shopcustompage_coupon')+"</div><div class='coupon-condition'>"+WST.lang('shopcustompage_coupon_condition_demo2')+"</div></div></div><div class='coupon-get'>"+WST.lang('shopcustompage_get_now')+"</div></div><div class='coupon-item wst-flex-column wst-center'><div class='wst-flex-row wst-afe'><div class='coupon-value-content'>"+WST.lang('currency_symbol')+"<span class='coupon-value'>5</span></div><div><div class='coupon-text'>"+WST.lang('shopcustompage_coupon')+"</div><div class='coupon-condition'>"+WST.lang('shopcustompage_coupon_condition_demo2')+"</div></div></div><div class='coupon-get'>"+WST.lang('shopcustompage_get_now')+"</div></div></div></div><div class='btn-del' onclick='delCouponBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addCouponClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_coupon'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-coupon "+coupon_attr_class_name+"' coupon_num="+coupon_num+"><div class='attr-content-coupon-item' ><div class='attr-content-coupon-img' img_url='/upload/shopcustompagedecoration/base/coupon_bg.png' ><label class='d-label'>"+WST.lang('shopcustompage_background_image')+"</label><div class='coupon-picker' id='coupon-picker-"+coupon_num+"' onclick='addCouponBgImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/coupon_bg.png' /></div><div class='image-tips coupon-image-tips'>"+WST.lang('shopcustompage_coupon_image_tips')+"</div></div></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_coupon_center')+"' class='attr-content-coupon-title' onkeyup='setCouponTitle(this)'/><span class='coupon-title-tip'><span class='coupon-title-length'>4</span>/4</span></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_desc')+"</label><input type='text' value='"+WST.lang('shopcustompage_coupon_sub_title')+"' class='attr-content-coupon-desc' onkeyup='setCouponDesc(this)'/><span class='coupon-desc-tip'><span class='coupon-desc-length'>9</span>/9</span></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_title_select_color')+"</label><div class='select-component-item-color coupon-title-select-color' style='background:#ffffff;' ></div></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_desc_select_color')+"</label><div class='select-component-item-color coupon-desc-select-color'  style='background:#ffffff;'></div></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_text_select_color')+"</label><div class='select-component-item-color coupon-text-select-color' style='background:#F9EE9E;' ></div></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_btn_select_color')+"</label><div class='select-component-item-color coupon-btn-select-color'  style='background:#F9EE9E;'></div></div><div class='attr-content-coupon-item' ><label class='d-label'>"+WST.lang('shopcustompage_coupon_btn_text_select_color')+"</label><div class='select-component-item-color coupon-btn-text-select-color' style='background:#000000;' ></div></div><div class='attr-content-coupon-item'><label class='d-label'>"+WST.lang('shopcustompage_add_coupon')+"</label><div class='add-coupon' onclick='addCoupons(this)'>+ "+WST.lang('shopcustompage_add_coupon')+"</div></div><div class='coupon-tips'>"+WST.lang('shopcustompage_coupon_tips5')+"</div><div class='coupon-tips'>"+WST.lang('shopcustompage_coupon_tips6')+"</div><div class='coupon-tips red'>"+WST.lang('shopcustompage_coupon_tips3')+"</div><div class='coupon-tips red'>"+WST.lang('shopcustompage_coupon_tips4')+"</div><input type='hidden' class='coupon-select-ids-value' value='' ><input type='hidden' class='coupon-title-color' value='#ffffff' /><input type='hidden' class='coupon-desc-color' value='#ffffff' /><input type='hidden' class='coupon-text-color' value='#F9EE9E' /><input type='hidden' class='coupon-btn-color' value='#F9EE9E' /><input type='hidden' class='coupon-btn-text-color' value='#000000' /></div>");
        WST.upload({
    ...
    $(".component-image").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-image "+image_class_name+"' image_num="+image_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='image'/><div class='images'><div class='images-item'><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_image_01.jpg' alt='' image_item='1'/></div></div><div class='btn-del' onclick='delImageBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addImageClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_image'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-image "+image_attr_class_name+"' image_num="+image_num+"><div class='attr-content-image-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_top')+"</label><div class='attr-content-image-item-content'><input type='hidden' class='image-range-slider'  value='0' onchange='changeImagePadding(this,1)'/><span class='image-padding-top-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-image-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_bottom')+"</label><div class='attr-content-image-item-content'><input type='hidden' class='image-range-slider'  value='0' onchange='changeImagePadding(this,2)'/><span class='image-padding-bottom-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-image-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_left')+"</label><div class='attr-content-image-item-content'><input type='hidden' class='image-range-slider'  value='0' onchange='changeImagePadding(this,3)'/><span class='image-padding-left-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-image-item'><label class='d-label'>"+WST.lang('shopcustompage_padding_right')+"</label><div class='attr-content-image-item-content'><input type='hidden' class='image-range-slider'  value='0' onchange='changeImagePadding(this,4)'/><span class='image-padding-right-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-image-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color image-select-bg-color'></div></div><div class='attr-content-image-item-img' ><div class='attr-content-image-img' img_url='/upload/shopcustompagedecoration/base/pc_image_01.jpg' image_item='1'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='image-picker' id='image-picker-"+image_num+"-1' onclick='addImageItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_image_01.jpg'/></div><div class='image-tips'>"+WST.lang('shopcustompage_image_img_tips2')+"</div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-image-link' /><div class='attr-btn-del' onclick='delImageItemBtn(this)'>X</div></div><div class='attr-content-image-item-add' onclick='addImageItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='image-padding-top' value='0'><input type='hidden' class='image-padding-bottom' value='0'><input type='hidden' class='image-padding-left' value='0'><input type='hidden' class='image-padding-right' value='0'><input type='hidden' class='image-bg-color' value='#ffffff' /></div>");
        WST.upload({
    ...
    $(".component-shopwindow").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-shopwindow "+shopwindow_class_name+"' shopwindow_num="+shopwindow_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='shopwindow'/><div class='shopwindows'><div class='shopwindows-item' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_01.jpg' alt='' shopwindow_item='1'/></div><div class='shopwindows-item' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_02.jpg' alt='' shopwindow_item='2'/></div><div class='shopwindows-item' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_03.jpg' alt='' shopwindow_item='3'/></div><div class='shopwindows-item' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_04.jpg' alt='' shopwindow_item='4'/></div></div><div class='btn-del' onclick='delShopwindowBtn(this)'>"+WST.lang('shopcustompage_del')+"</div><div class='shopwindows-layout none'><div class='s-layout'><div class='s-layout-left'><div class='s-layout-left-item' ></div></div><div class='s-layout-right'><div class='s-layout-top'><div class='s-layout-top-item' ></div></div><div class='s-layout-bottom'><div class='s-layout-bottom-item' ></div><div class='s-layout-bottom-item' ></div></div></div></div></div><div class='shopwindows-s-hide'></div></div><div class='border-absolute effect-active select' onclick='addShopwindowClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_shopwindow'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-shopwindow "+shopwindow_attr_class_name+"' shopwindow_num="+shopwindow_num+"><div class='attr-content-shopwindow-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color shopwindow-select-bg-color'></div></div><div class='attr-content-shopwindow-item'><label class='d-label'>"+WST.lang('shopcustompage_layout_type')+"</label><div class='shopwindow-layout' style='width:150px;'><input type='radio' value='1' name='shopwindow-layout"+shopwindow_num+"' id='s-two"+shopwindow_num+"' onclick='changeShopwindowLayout(this)' checked /><label for='s-two"+shopwindow_num+"'>"+WST.lang('shopcustompage_layout_type1')+"</label><input type='radio' value='2' name='shopwindow-layout"+shopwindow_num+"' id='s-three"+shopwindow_num+"' onclick='changeShopwindowLayout(this)' /><label for='s-three"+shopwindow_num+"'>"+WST.lang('shopcustompage_layout_type2')+"</label><input type='radio' value='3' name='shopwindow-layout"+shopwindow_num+"' id='s-four"+shopwindow_num+"' onclick='changeShopwindowLayout(this)' /><label for='s-four"+shopwindow_num+"'>"+WST.lang('shopcustompage_layout_type3')+"</label><input type='radio' value='4' name='shopwindow-layout"+shopwindow_num+"' id='s-s"+shopwindow_num+"' onclick='changeShopwindowLayout(this)' /><label for='s-s"+shopwindow_num+"'>"+WST.lang('shopcustompage_layout_type4')+"</label></div></div><div class='attr-content-shopwindow-item'><label class='d-label'>"+WST.lang('shopcustompage_layout_desc')+"</label><div class='shopwindow-layout-info'>"+WST.lang('shopcustompage_shopwindow_image_tips1')+"</div></div><div class='attr-content-shopwindow-item-img' ><div class='attr-content-shopwindow-img' img_url='/upload/shopcustompagedecoration/base/shopwindow_01.jpg' shopwindow_item='1'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='shopwindow-picker' id='shopwindow-picker-"+shopwindow_num+"-1' onclick='addShopwindowItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_01.jpg'/></div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-shopwindow-link' /><div class='attr-btn-del' onclick='delShopwindowItemBtn(this)'>X</div></div><div class='attr-content-shopwindow-item-img' ><div class='attr-content-shopwindow-img' img_url='/upload/shopcustompagedecoration/base/shopwindow_02.jpg' shopwindow_item='2'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='shopwindow-picker' id='shopwindow-picker-"+shopwindow_num+"-2' onclick='addShopwindowItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_02.jpg'/></div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-shopwindow-link' /><div class='attr-btn-del' onclick='delShopwindowItemBtn(this)'>X</div></div><div class='attr-content-shopwindow-item-img' ><div class='attr-content-shopwindow-img' img_url='/upload/shopcustompagedecoration/base/shopwindow_03.jpg' shopwindow_item='3'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='shopwindow-picker' id='shopwindow-picker-"+shopwindow_num+"-3' onclick='addShopwindowItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_03.jpg'/></div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-shopwindow-link' /><div class='attr-btn-del' onclick='delShopwindowItemBtn(this)'>X</div></div><div class='attr-content-shopwindow-item-img' ><div class='attr-content-shopwindow-img' img_url='/upload/shopcustompagedecoration/base/shopwindow_04.jpg' shopwindow_item='4'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='shopwindow-picker' id='shopwindow-picker-"+shopwindow_num+"-4' onclick='addShopwindowItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_04.jpg'/></div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-shopwindow-link' /><div class='attr-btn-del' onclick='delShopwindowItemBtn(this)'>X</div></div><div class='attr-content-shopwindow-item-add' onclick='addShopwindowItem(this)'>"+WST.lang('shopcustompage_add_one')+"</div><input type='hidden' class='shopwindow-bg-color' value='#ffffff' /><input type='hidden' class='shopwindow-layout-value' value='1' /></div>");
        WST.upload({
    ...
    $(".component-video").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-video "+video_class_name+"' video_num="+video_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='video'/><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_video.png' alt=''/><div class='btn-del' onclick='delVideoBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addVideoClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_video'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-video "+video_attr_class_name+"' video_num="+video_num+" ><div class='attr-content-video-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-video-item-content'><input type='hidden' class='video-range-slider'  value='0' onchange='changeVideoMargin(this)'/><span class='video-vertical-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-video-item'><div class='attr-content-video-img' img_url='/upload/shopcustompagedecoration/base/pc_video.png' ></div></div><div class='attr-content-video-item'><label class='d-label'>"+WST.lang('shopcustompage_video_link')+"</label><input type='text' value='' class='attr-content-video-link' /></div><input type='hidden' class='video-vertical-padding' value='0' /></div>");
        WST.upload({
    ...
    $(".component-line").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-line "+line_class_name+"' line_num="+line_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='line'/><div class='line'></div><div class='btn-del' onclick='delLineBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addLineClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_line'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-line "+line_attr_class_name+"' line_num="+line_num+" ><div class='attr-content-line-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='attr-content-line-item-content'><div class='select-component-item-color line-select-bg-color'></div></div></div><div class='attr-content-line-item'><label class='d-label'>"+WST.lang('shopcustompage_line_style')+"</label><div class='attr-content-line-item-content'><input type='radio' value='1' name='line-class"+line_num+"' id='l-solid' onclick='changeLineClass(this)' checked /><label for='l-solid'>"+WST.lang('shopcustompage_line_class1')+"</label><input type='radio' value='2' name='line-class"+line_num+"' id='l-dashed' onclick='changeLineClass(this)' /><label for='l-dashed'>"+WST.lang('shopcustompage_line_class2')+"</label><input type='radio' value='3' name='line-class"+line_num+"' id='l-dotted' onclick='changeLineClass(this)' /><label for='l-dotted'>"+WST.lang('shopcustompage_line_class3')+"</label></div></div><div class='attr-content-line-item' ><label class='d-label'>"+WST.lang('shopcustompage_line_color')+"</label><div class='attr-content-line-item-content'><div class='select-component-item-color line-select-color'></div></div></div><div class='attr-content-line-item'><label class='d-label'>"+WST.lang('shopcustompage_line_height')+"</label><div class='attr-content-line-item-content'><input type='hidden' class='line-height-range-slider'  value='1' onchange='changeLineHeight(this)'/><span class='line-height-value'>1</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-line-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-line-item-content'><input type='hidden' class='line-margin-range-slider'  value='10' onchange='changeLineMargin(this)'/><span class='line-vertical-margin-value'>10</span><span>px ("+WST.lang('shopcustompage_px')+")</span></div></div><input type='hidden' class='line-bg-color' value='#ffffff' /><input type='hidden' class='line-class' value='1' /><input type='hidden' class='line-border-color' value='#000000' /><input type='hidden' class='line-height' value='1' /><input type='hidden' class='line-vertical-margin' value='10' /></div>");
        $('.line-select-bg-color').colpick({
            layout:'hex',
            submit:1,
            colorScheme:'dark',
    ...
    $(".component-text").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-text "+text_class_name+"' text_num="+text_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='text'/><div class='text'>"+WST.lang('shopcustompage_here_is_text_content')+"</div><div class='btn-del' onclick='delTextBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addTextClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_text'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-text "+text_attr_class_name+"' text_num="+text_num+" ><div class='attr-content-text-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-text-item-content'><input type='hidden' class='text-range-slider'  value='0' onchange='changeTextPadding(this,1)'/><span class='text-vertical-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-text-item'><label class='d-label'>"+WST.lang('shopcustompage_horizontal_padding')+"</label><div class='attr-content-text-item-content'><input type='hidden' class='text-range-slider'  value='0' onchange='changeTextPadding(this,2)'/><span class='text-horizontal-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-text-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color text-select-bg-color'></div></div><div class='attr-content-text-item'><textarea rows='2' cols='60' class='text-desc"+text_num+"'>"+WST.lang('shopcustompage_here_is_text_content')+"</textarea></div><input type='hidden' class='text-vertical-padding' value='0' /><input type='hidden' class='text-horizontal-padding' value='0' /><input type='hidden' class='text-bg-color' value='#ffffff' /></div>");
        $('.text-select-bg-color').colpick({
            layout:'hex',
            submit:1,
            colorScheme:'dark',
    ...
    $(".component-txt").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='interval'></div><div class='effect-txt "+txt_class_name+"' txt_num="+txt_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='txt'/><div class='txt-info'><p>"+WST.lang('shopcustompage_add_txt_content')+"</p></div><div class='btn-del' onclick='delTxtBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addTxtClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_txt'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-txt "+txt_attr_class_name+"' txt_num="+txt_num+" ><div class='attr-content-txt-item'><label class='d-label'>"+WST.lang('shopcustompage_prompt_text')+"</label><input type='text' value='"+WST.lang('shopcustompage_add_txt_content')+"' class='attr-content-txt-text' onkeyup='setTxtText(this)' /><span class='txt-text-tip'><span class='txt-text-length'>8</span>/50</span></div><div class='attr-content-txt-item'><label class='d-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-txt-link' /></div><div class='attr-content-txt-item'><label class='d-label'>"+WST.lang('shopcustompage_font_position')+"</label><input type='radio' value='1' checked name='txt-text-alignment"+txt_num+"'  id='t-left"+txt_num+"' onclick='changeTxtTextAlignment(this)' /><label for='t-left"+txt_num+"'>"+WST.lang('shopcustompage_alignment_left')+"</label><input type='radio' value='2' name='txt-text-alignment"+txt_num+"' id='s-center"+txt_num+"' onclick='changeTxtTextAlignment(this)' /><label for='s-center"+txt_num+"'>"+WST.lang('shopcustompage_alignment_center')+"</label><input type='radio' value='3' name='txt-text-alignment"+txt_num+"' id='s-right"+txt_num+"' onclick='changeTxtTextAlignment(this)' /><label for='s-right"+txt_num+"'>"+WST.lang('shopcustompage_alignment_right')+"</label></div><div class='attr-content-txt-item' ><label class='d-label'>"+WST.lang('shopcustompage_background_color')+"</label><div class='select-component-item-color txt-select-bg-color'></div></div><div class='attr-content-txt-item' ><label class='d-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color txt-select-text-color' style='background:#000000;'></div></div><div class='attr-content-txt-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-txt-item-content'><input type='hidden' class='txt-range-slider'  value='0' onchange='changeTxtPadding(this,1)'/><span class='txt-vertical-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='attr-content-txt-item'><label class='d-label'>"+WST.lang('shopcustompage_horizontal_padding')+"</label><div class='attr-content-txt-item-content'><input type='hidden' class='txt-range-slider'  value='0' onchange='changeTxtPadding(this,2)'/><span class='txt-horizontal-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><input type='hidden' class='txt-bg-color' value='#ffffff' /><input type='hidden' class='txt-text-color' value='#000000' /><input type='hidden' class='attr-content-txt-text-alignment' value='1' /><input type='hidden' class='txt-vertical-padding' value='0' /><input type='hidden' class='txt-horizontal-padding' value='0' /></div>");
        $('.txt-select-bg-color').colpick({
            layout:'hex',
            submit:1,
            colorScheme:'dark',
    ...
    $(".component-image-text").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-image-text image_text"+image_text_num+"' image_text_num="+image_text_num+"><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='image_text'/><div class='image-text image-text-style-1' image_text_item='1'><div class='image-text-imgs image-text-style-1-left'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/></div><div class='image-text-style-1-right'><p class='image-text-style-title'>"+WST.lang('shopcustompage_image_text_title_demo')+"</p><p class='image-text-style-desc'>"+WST.lang('shopcustompage_image_text_desc_demo')+"</p></div></div><div class='image-text image-text-style-2 none2' image_text_item='2'><div class='image-text-style-2-left'><p class='image-text-style-title'>"+WST.lang('shopcustompage_image_text_title_demo')+"</p><p class='image-text-style-desc'>"+WST.lang('shopcustompage_image_text_desc_demo')+"</p></div><div class='image-text-imgs image-text-style-2-right'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/></div></div><div class='image-text image-text-style-3 none' image_text_item='3'><div class='image-text-style-3-top'><p class='image-text-style-title'>"+WST.lang('shopcustompage_image_text_title_demo')+"</p><p class='image-text-style-desc'>"+WST.lang('shopcustompage_image_text_desc_demo')+"</p></div><div class='image-text-imgs image-text-style-3-bottom'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/></div></div><div class='image-text image-text-style-4 none' image_text_item='4'><div class='image-text-style-4-top'><p class='image-text-style-title'>"+WST.lang('shopcustompage_image_text_title_demo')+"</p><p class='image-text-style-desc'>"+WST.lang('shopcustompage_image_text_desc_demo')+"</p></div><div class='image-text-imgs wst-flex-row wst-jsa image-text-style-4-bottom'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/></div></div><div class='image-text image-text-style-5 none' image_text_item='5'><div class='image-text-style-5-top'><p class='image-text-style-title'>"+WST.lang('shopcustompage_image_text_title_demo')+"</p><p class='image-text-style-desc'>"+WST.lang('shopcustompage_image_text_desc_demo')+"</p></div><div class='image-text-imgs wst-flex-row wst-jsa image-text-style-5-bottom'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg' alt=''/></div></div><div class='btn-del' onclick='delImageTextBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute effect-active select' onclick='addImageTextClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_image_text'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-image-text image_text_attr"+image_text_num+"' image_text_num="+image_text_num+" ><div class='attr-content-image-text-item'><label class='d-label'>"+WST.lang('shopcustompage_image_text_style')+"</label></div><div class='attr-content-image-text-item'><div class='image-text-style-item active' onclick='changeImageTextStyle(this)' image_text_item='1'>"+WST.lang('shopcustompage_image_text_style1')+"</div><div class='image-text-style-item' onclick='changeImageTextStyle(this)' image_text_item='2'>"+WST.lang('shopcustompage_image_text_style2')+"</div><div class='image-text-style-item' onclick='changeImageTextStyle(this)' image_text_item='3'>"+WST.lang('shopcustompage_image_text_style3')+"</div><div class='image-text-style-item' onclick='changeImageTextStyle(this)' image_text_item='4'>"+WST.lang('shopcustompage_image_text_style4')+"</div><div class='image-text-style-item' onclick='changeImageTextStyle(this)' image_text_item='5'>"+WST.lang('shopcustompage_image_text_style5')+"</div></div><div class='attr-content-image-text-item-img' ><div class='image-text-img'><div class='attr-content-image-text-img ' img_url='upload/shopcustompagedecoration/base/banner_01.jpg'><div class='wst-flex-row'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='image-text-picker' id='image-text-picker-"+image_text_num+"-1'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg'/></div></div><div class='image-tips'>"+WST.lang('shopcustompage_image_text_img_tips1')+"</div></div></div><div class='image-text-img none'><div class='attr-content-image-text-img ' img_url='upload/shopcustompagedecoration/base/banner_01.jpg'><div class='wst-flex-row'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='image-text-picker' id='image-text-picker-"+image_text_num+"-2'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg'/></div></div><div class='image-tips'>"+WST.lang('shopcustompage_image_text_img_tips1')+"</div></div></div><div class='image-text-img none'><div class='attr-content-image-text-img ' img_url='upload/shopcustompagedecoration/base/banner_01.jpg'><div class='wst-flex-row'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='image-text-picker' id='image-text-picker-"+image_text_num+"-3'><img src='"+base_url+"/upload/shopcustompagedecoration/base/banner_01.jpg'/></div></div><div class='image-tips'>"+WST.lang('shopcustompage_image_text_img_tips1')+"</div></div></div><div><label class='d-label'>"+WST.lang('shopcustompage_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_here_input_title')+"' class='attr-content-image-text-title' onkeyup='setImageTextTitle(this)'style='width:145px !important;' /><span class='image-text-title-tip'><span class='image-text-title-length'>6</span>/20</span></div><div><label class='d-label'>"+WST.lang('shopcustompage_desc')+"</label><input type='text' value='"+WST.lang('shopcustompage_here_input_desc')+"' class='attr-content-image-text-desc' onkeyup='setImageTextDesc(this)' style='width:145px !important;' /><span class='image-text-desc-tip'><span class='image-text-desc-length'>6</span>/200</span></div><div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-image-text-link' /></div></div><div class='attr-content-image-text-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-image-text-item-content'><input type='hidden' class='image-text-range-slider'  value='0' onchange='changeImageTextPadding(this)'/><span class='image-text-vertical-padding-value'>10</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><input type='hidden' class='image-text-style' value='1' /><input type='hidden' class='image-text-vertical-padding' value='0'></div>");
        WST.upload({
    ...
    $(".component-new").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-new "+new_class_name+"' new_num="+new_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='new'/><div class='new-title'><div class='new-title-text'>"+WST.lang('shopcustompage_new_info')+"</div><div>"+WST.lang('shopcustompage_more')+"</div></div><div class='btn-del' onclick='delNewBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addNewClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_new'));
        hideComponentAttrContent(1);
        $(".attr-content").append("<div class='attr-content-new "+new_attr_class_name+"' new_num="+new_num+"><div class='attr-content-new-item' ><label class='d-label'>"+WST.lang('shopcustompage_new_num')+"</label><input type='radio' value='2' name='new-count"+new_num+"'  id='n-count-2-"+new_num+"' checked onclick='changeNewCount(this)' /><label for='n-count-2-"+new_num+"' class='d-a-label'>2"+WST.lang('shopcustompage_goods_unit')+"</label><input type='radio' value='4' name='new-count"+new_num+"' id='n-count-4-"+new_num+"' onclick='changeNewCount(this)' /><label for='n-count-4-"+new_num+"' class='d-a-label'>4"+WST.lang('shopcustompage_goods_unit')+"</label><input type='radio' value='6' name='new-count"+new_num+"' id='n-count-6-"+new_num+"' onclick='changeNewCount(this)' /><label for='n-count-6-"+new_num+"' class='d-a-label'>6"+WST.lang('shopcustompage_goods_unit')+"</label><input type='radio' value='8' name='new-count"+new_num+"' id='n-count-8-"+new_num+"' onclick='changeNewCount(this)' /><label for='n-count-8-"+new_num+"' class='d-a-label'>8"+WST.lang('shopcustompage_goods_unit')+"</label></div><div class='attr-content-new-item'><label class='d-label'>"+WST.lang('shopcustompage_new_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_new_info')+"' class='attr-content-new-title' onkeyup='setNewTitle(this)'/><span class='new-title-tip'><span class='new-title-length'>4</span>/8</span></div><div class='attr-content-new-item'><label class='d-label'>"+WST.lang('shopcustompage_new_column_add')+"</label><input type='radio' value='1' name='news-select"+new_num+"' onclick='addNews(this)' /></div><input type='hidden' class='new-select-ids-value' value='' ><input type='hidden' class='new-count' value='2' /></div>");
        $(".attr-content-new").each(function(idx, item) {
    ...
    $(".component-marketing").click(function(event){
        ...
        $(".effect-content").append("<div class='el-relative'><div class='effect-marketing "+marketing_class_name+"' marketing_num="+marketing_num+" ><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort' value='0'/><input type='hidden' class='sort-name' value='marketing'/><div class='marketing-title'><div class='wst-flex-row wst-ac'><div class='marketing-title-text'>"+WST.lang('shopcustompage_marketing_title')+"</div><div class='seckill-title none2'><div class='seckill-title-text'>12:00"+WST.lang('shopcustompage_session')+"</div><div class='seckill-time'>01:15:16</div></div></div><div class='seckill-more'>"+WST.lang('shopcustompage_more')+"</div></div><div class='wst-flex-row marketing-main'><div class='goods wst-flex-row' ><div class='goods-image'><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_01.jpg' alt=''  /></div><div class='goods-info wst-flex-column wst-jsb' ><p class='goods-name'>"+WST.lang('shopcustompage_here_show_goods_name')+"</p><div class='pintuan-detail none'><div class='wst-flex-row wst-jsb'><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p><p class='pintuan-num' >2"+WST.lang('shopcustompage_pintuan_num')+"</p></div></div><div class='seckill-detail none'><div class='wst-flex-row wst-jsb'><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p><p class='goods-price goods-price-decoration' >"+WST.lang('currency_symbol')+"150.00</p></div></div><div class='auction-detail none'><p class='goods-price auction-num'>1"+WST.lang('shopcustompage_auction_num')+"</p><p class='goods-price'>"+WST.lang('shopcustompage_current_price')+"：<span class='red'>"+WST.lang('currency_symbol')+"10.00</span></p></div><div class='bargain-detail none'><div class='wst-flex-row wst-jsb'><p class='market-price goods-price-decoration '>"+WST.lang('shopcustompage_old_price')+":"+WST.lang('currency_symbol')+"20.00</p><p class='bargain-num'>2"+WST.lang('shopcustompage_join_num')+"</p></div><p class='goods-price red '>"+WST.lang('shopcustompage_bottom_price')+"："+WST.lang('currency_symbol')+"5.00</p></div></div></div><div class='goods wst-flex-row' ><div class='goods-image'><img src='"+base_url+"/upload/shopcustompagedecoration/base/good_02.jpg' alt=''  /></div><div class='goods-info wst-flex-column wst-jsb' ><p class='goods-name'>"+WST.lang('shopcustompage_here_show_goods_name')+"</p><div class='pintuan-detail none'><div class='wst-flex-row wst-jsb'><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p><p class='pintuan-num' >2"+WST.lang('shopcustompage_pintuan_num')+"</p></div></div><div class='seckill-detail none'><div class='wst-flex-row wst-jsb'><p class='goods-price red '>"+WST.lang('currency_symbol')+"99.00</p><p class='goods-price goods-price-decoration' >"+WST.lang('currency_symbol')+"150.00</p></div></div><div class='auction-detail none'><p class='goods-price auction-num'>1"+WST.lang('shopcustompage_auction_num')+"</p><p class='goods-price'>"+WST.lang('shopcustompage_current_price')+"：<span class='red'>"+WST.lang('currency_symbol')+"10.00</span></p></div><div class='bargain-detail none'><div class='wst-flex-row wst-jsb'><p class='market-price goods-price-decoration '>"+WST.lang('shopcustompage_old_price')+":"+WST.lang('currency_symbol')+"20.00</p><p class='bargain-num'>2"+WST.lang('shopcustompage_join_num')+"</p></div><p class='goods-price red '>"+WST.lang('shopcustompage_bottom_price')+"："+WST.lang('currency_symbol')+"5.00</p></div></div></div></div><div class='btn-del' onclick='delMarketingBtn(this)'>"+WST.lang('shopcustompage_del')+"</div></div><div class='border-absolute' onclick='addMarketingClassActive(this)' onmouseover='addClassActive(this)' onmouseout='removeClassActive(this)'></div></div>");
        $(".attr-title").html(WST.lang('shopcustompage_component_marketing'));
        hideComponentAttrContent(1);
        var marketingTypeHtml = $('.marketingType').html();
        $(".attr-content").append("<div class='attr-content-marketing "+marketing_attr_class_name+"' marketing_num="+marketing_num+" ><div class='attr-content-marketing-item'><label class='d-label'>"+WST.lang('shopcustompage_component_marketing')+"</label>"+marketingTypeHtml+"</div><div class='attr-content-marketing-item'><label class='d-label'>"+WST.lang('shopcustompage_column_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_marketing_title')+"' class='attr-content-marketing-title' onkeyup='setMarketingTitle(this)'/><span class='marketing-title-tip'><span class='marketing-title-length'>0</span>/10</span></div><div class='attr-content-marketing-item'><label class='d-label'>"+WST.lang('shopcustompage_vertical_padding')+"</label><div class='attr-content-marketing-item-content'><input type='hidden' class='marketing-range-slider'  value='0' onchange='changeMarketingPadding(this)'/><span class='marketing-vertical-padding-value'>0</span><span class='pixel'>px ("+WST.lang('shopcustompage_px')+")</span></div></div><div class='marketing-tips red'>"+WST.lang('shopcustompage_marking_tips3')+"</div><input type='hidden' class='marketing-type' value=''><input type='hidden' class='marketing-vertical-padding' value='0'></div>");
        $('.marketing-range-slider').jRange({
            from: 0,
            to: 50,
            step: 1,
...
function delBtn(obj){
    var that = $(obj);
    layer.confirm(WST.lang('shopcustompage_confirm_del'), {icon: 3, title:WST.lang('shopcustompage_info')}, function(index){
        that.parent().remove();
        layer.close(index);
    });
}
...
function hideComponentAttrContent(type){
    ...
    $(".attr-content-goods-recommend").hide();
...
function delSwiperItemBtn(obj){
    ...
    layer.confirm(WST.lang('shopcustompage_confirm_del'), {icon: 3, title:WST.lang('shopcustompage_info')}, function(index){
        ...
        if(swiper_item_counts == 1){
            WST.msg(WST.lang('shopcustompage_at_least_keep_one'),{time:1000,anim: 6});
        }else{
            that.parent().remove();
            $(".attr-content-swiper").each(function(idx, item) {
...
function addSwiperItem(obj){
    ...
    $(obj).before("<div class='attr-content-swiper-item-img'><div class='attr-content-swiper-img' img_url='upload/shopcustompagedecoration/base/pc_banner_01.jpg'><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='banner-picker' id='banner-picker-"+swiper_num+"-"+swiper_img_num+"'><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_banner_01.jpg'/></div><div class='image-tips'>"+WST.lang('shopcustompage_swiper_image_tips2')+"</div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-swiper-link' /><div class='attr-btn-del' onclick='delSwiperItemBtn(this)'>X</div></div>");
    WST.upload({
...
function addFloorGoodsIconImg(obj){
}
...
function addFloorGoodsColumnItem(obj){
    ...
    if(columns_item_counts == 5){
        layer.msg(WST.lang('shopcustompage_max_five_column'),{time:1000,anim: 6});
    }else{
        columns_item += 1;
        $('.attr-content-floor-goods-item-column-add').before("<div class='attr-content-floor-goods-item-column' ><div class='wst-flex-row wst-ac'><label class='d-label'>"+WST.lang('shopcustompage_column_title')+"</label><input type='text' value='"+WST.lang('shopcustompage_column_title')+"1' class='floor-goods-columns-name' columns_item="+columns_item+" onkeyup='setFloorGoodsColumnsTitle(this)' style='width:150px !important;'/><span class='floor-goods-columns-name-tip'><span class='floor-goods-columns-name-length'>5</span>/6</span></div><label class='d-label'>"+WST.lang('shopcustompage_jump_link')+"</label><input type='text' value='' class='floor-goods-columns-link' style='width:150px !important;'/><div class='attr-btn-del' onclick='delFloorGoodsColumnItemBtn(this)'>X</div></div>");
        $(".effect-floor-goods").each(function(idx, item) {
            if ($(item).hasClass(floor_goods_class_name)) {
                $(item).find('.columns-title').append("<div columns_item="+columns_item+">"+WST.lang('shopcustompage_column_title')+"1</div>");
            }
        });
    }
}
...
function changeFloorGoodsSelect(obj){
    ...
    if(goods_select == 1){
        ...
        if(floor_goods_min_price_value!=''){
            $(".goods-min-price").val(floor_goods_min_price_value);
        }else{
            $(".goods-min-price").val('');
        }
        if(floor_goods_max_price_value!=''){
            $(".goods-max-price").val(floor_goods_max_price_value);
        }else{
            $(".goods-max-price").val('');
        }

        var box = WST.open({title:WST.lang('shopcustompage_select_goods_by_condition'),type:1,content:$('#goodBox2'),area: ['80%', '80%'],offset:['10%','10%'],btn: [WST.lang('shopcustompage_confirm'),WST.lang('shopcustompage_cancel')],
            yes:function(){
                var floor_goods_select_cats_ids = [];
                var floor_goods_tag = $("input[name='goods-tag']:checked").val();
                var min_price = '';
                var max_price = '';
                ...
                if(floor_goods_min_price>0 && floor_goods_max_price==''){
                    WST.msg(WST.lang('shopcustompage_require_goods_max_price'),{time:1000,anim: 6});
                    return;
                }
                if(floor_goods_max_price>0 && floor_goods_min_price==''){
                    WST.msg(WST.lang('shopcustompage_require_goods_min_price'),{time:1000,anim: 6});
                    return;
                }
                if(floor_goods_min_price > floor_goods_max_price){
                    WST.msg(WST.lang('shopcustompage_goods_price_set_error'),{time:1000,anim: 6});
                    return;
                }
                $('.goods-select-cat-name').find('input[type=radio]').each(function(idx, item) {
                    if($(item).prop('checked')){
                        floor_goods_select_cats_ids.push($(item).attr('cats-id'));
                    }
                });
                $(".attr-content-floor-goods").each(function(idx, item) {
                    if($(item).attr('floor_goods_num')==floor_goods_num){
                        $(item).find('.floor-goods-select-ids-value').val('');
                        $(item).find('.floor-goods-select-cats-id-value').val(floor_goods_select_cats_ids);
                        $(item).find('.floor-goods-tag-value').val(floor_goods_tag);
                        $(item).find('.floor-goods-min-price-value').val(floor_goods_min_price);
                        $(item).find('.floor-goods-max-price-value').val(floor_goods_max_price);
                    }
                });
                $('#goodBox2').hide();
                layer.close(box);
        ...
        var box = WST.open({title:WST.lang('shopcustompage_select_goods'),type:1,content:$('#goodBox'),area: ['80%', '80%'],offset:['10%','10%'],btn: [WST.lang('shopcustompage_confirm'),WST.lang('shopcustompage_cancel')],
            yes:function(){
                $(".attr-content-floor-goods").each(function(idx, item) {
                    if($(item).attr('floor_goods_num')==floor_goods_num){
                        $(item).find('.floor-goods-select-ids-value').val(global_good_select_ids);
                        $(item).find('.floor-goods-select-cats-id-value').val('');
                    }
                });
                layer.close(box);
            },cancel:function(){
...
function changeGoodsRecommendOrder(obj){
    var order = $(obj).val();
    $(obj).parent().parent().find(".goods-recommend-order").val(order);
}
// 楼层商品组件方法结束

// 商品推荐组件方法开始
function delGoodsRecommendBtn(obj){
    var goods_recommend_num = $(obj).parent().attr("goods_recommend_num");
    layer.confirm(WST.lang('shopcustompage_confirm_del'), {icon: 3, title:WST.lang('shopcustompage_info')}, function(index){
        $(".effect-goods-recommend").each(function(idx, item) {
            if($(item).hasClass("goods_recommend"+goods_recommend_num)){
                $(item).parent().remove();
...
function addGoodsRecommendClassActive(obj){
    var goods_recommend_num = $(obj).parent().find(".effect-goods-recommend").attr("goods_recommend_num");
    componentRemoveClasses(1);
    $(obj).addClass("effect-active");
    $(obj).addClass("select");
...
function setGoodsRecommendTitle(obj){
    var goods_recommend_num = $(obj).parent().parent().attr("goods_recommend_num");
    var goods_recommend_title = $(obj).val();
    var goods_recommend_title_length = goods_recommend_title.length;
    $(".effect-goods-recommend").each(function(idx, item) {
        if($(item).hasClass("goods_recommend"+goods_recommend_num)){
            if(goods_recommend_title.length>10){
                goods_recommend_title = goods_recommend_title.substr(0,10);
            }
            $(item).find(".goods-recommend-title-text").html(goods_recommend_title);
        }
    });
    $(".attr-content-goods-recommend").each(function(idx, item) {
        if($(item).hasClass("goods_recommend_attr"+goods_recommend_num)){
            if(goods_recommend_title_length > 10){
                $(item).find('.goods-recommend-title-length').html(10);
            }else{
                $(item).find('.goods-recommend-title-length').html(goods_recommend_title_length);
            }
        }
    });
}

function changeGoodsRecommendTitleDisplay(obj){
    var show_goods_recommend_title = $(obj).val();
    var goods_recommend_num = $(obj).parent().parent().attr("goods_recommend_num");
    $(obj).parent().parent().find('.show-goods-recommend-title').val(show_goods_recommend_title);
    $(".effect-goods-recommend").each(function (idx, item) {
        if ($(item).hasClass("goods_recommend" + goods_recommend_num)) {
            if (show_goods_recommend_title == 1) {
                $(item).find('.goods-recommend-header').show();
            } else {
                $(item).find('.goods-recommend-header').hide();
            }
        }
    });
}

function changeRecommendGoodsSelect(obj){
    var goods_select = $(obj).val();
    var goods_recommend_num = $(obj).parent().parent().attr("goods_recommend_num");
    if(goods_select == 1){
        // 条件选取
        ...
        var box = WST.open({title:WST.lang('shopcustompage_select_goods_by_condition'),type:1,content:$('#goodBox2'),area: ['80%', '80%'],offset:['10%','10%'],btn: [WST.lang('shopcustompage_confirm'),WST.lang('shopcustompage_cancel')],
            yes:function(){
                ...
                $(".attr-content-goods-recommend").each(function(idx, item) {
                    if($(item).attr('goods_recommend_num')==goods_recommend_num){
...
function delNavItemBtn(obj){
    ...
    layer.confirm(WST.lang('shopcustompage_confirm_del'), {icon: 3, title:WST.lang('shopcustompage_info')}, function(index){
        ...
        if(nav_item_counts == 3){
            WST.msg(WST.lang('shopcustompage_at_least_keep_three'),{time:1000,anim: 6});
        }else{
            $(obj).parent().remove();
            $(".effect-nav").each(function(idx, item) {
                if($(item).hasClass("nav"+nav_num)){
...
function addNavItem(obj){
    ...
    if(nav_item_counts == 6){
        layer.msg(WST.lang("shopcustompage_max_six_text_nav"),{time:1000,anim: 6});
    }else {
        nav_item += 1;
        $(".effect-nav").each(function(idx, item) {
            if($(item).hasClass("nav"+nav_num)){
                $(item).find('.nav-right').append("<div class='nav-item' nav_item=" + nav_item + "><div class='nav-text'>"+WST.lang('shopcustompage_bottom_text')+"</div></div>")
            }
        });
        $(obj).before("<div class='attr-content-nav-item-group' nav_item=" + nav_item + "><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_content')+"</label><input type='text' value='"+WST.lang('shopcustompage_bottom_text')+"' class='attr-content-nav-text' onkeyup='setNavItemTitle(this)' /><span class='nav-text-tip'><span class='nav-text-length'>4</span>/4</span></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_text_color')+"</label><div class='select-component-item-color nav-select-text-color' style='background:#ffffff;'></div><input type='hidden' class='attr-content-nav-text-color' value='#ffffff' /></div><div class='attr-content-nav-info'><label class='nav-label'>"+WST.lang('shopcustompage_link_address')+"</label><input type='text' value='' class='attr-content-nav-link' /></div><div class='attr-btn-del' onclick='delNavItemBtn(this)'>X</div></div>");
        $('.nav-select-text-color').colpick({
            layout: 'hex',
            submit: 1,
            colorScheme: 'dark',
    ...
    $(".effect-nav").each(function(idx, item) {
        if($(item).hasClass("nav"+nav_num)){
            if(show_nav_category==1){
                ...
                $(item).find(".nav-right").css('width','70%');
            }
        }
    });
...
function changeNavButtonDisplay(obj){
    var nav_num = $(obj).parent().parent().attr("nav_num");
    var show_nav_button = $(obj).val();
    $(".effect-nav").each(function(idx, item) {
        if($(item).hasClass("nav"+nav_num)){
            if(show_nav_button==1){
                $(item).find(".nav-btn").show();
            }else{
                $(item).find(".nav-btn").hide();
            }
        }
    });
    $(obj).parent().parent().find('.show-nav-button').val(show_nav_button);
}
...
function addNoticeItem(obj){
    ...
    if(notice_item_counts == 10){
        layer.msg(WST.lang("shopcustompage_max_notice_tips"),{time:1000,anim: 6});
    }else{
        notice_item += 1;
        $('.attr-content-notice-item-group-add').before("<div class='attr-content-notice-item-group' ><div><label class='d-label'>"+WST.lang('shopcustompage_notice')+"</label><input type='text' value='"+WST.lang('shopcustompage_here_input_notice_content')+"' class='attr-content-notice-text' onkeyup='setNoticeText(this)' /></div><div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-notice-link' /></div><div class='attr-btn-del' onclick='delNoticeItemBtn(this)'>X</div></div>");
    }
}
        ...
        $(".attr-content-notice").each(function(idx, item) {
            if($(item).attr("notice_num") == notice_num){
                ...
                if(notice_item_counts == 2){
                    WST.msg(WST.lang('shopcustompage_at_least_keep_two_notice'),{time:1000,anim: 6});
                }else {
                    that.parent().remove();
                    layer.close(index);
                }
...
function addCoupons(obj){
    ...
    var box = WST.open({title:WST.lang('shopcustompage_coupon_list')+"&nbsp;<span style='color:#ff0000;'>("+WST.lang('custompage_index_coupon_tips')+")</span>",type:1,content:$('#couponBox'),area: ['80%', '55%'],btn: [WST.lang('shopcustompage_confirm'),WST.lang('shopcustompage_cancel')],
        yes:function(){
            $(".attr-content-coupon").each(function(idx, item) {
...
function addImageItem(obj){
    ...
    $(".effect-image").each(function(idx, item) {
        if($(item).hasClass("image"+image_num)){
            ...
            $(item).find(".images").append("<div class='images-item' style='"+style+"' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_image_01.jpg' alt='' image_item="+image_item+" /></div>");
        }
    });
    $(".attr-content-image").each(function(idx, item) {
    ...
    $(obj).before("<div class='attr-content-image-item-img' ><div class='attr-content-image-img' img_url='/upload/shopcustompagedecoration/base/pc_image_01.jpg' image_item="+image_item+"><label class='d-label'>"+WST.lang('shopcustompage_image')+"</label><div class='image-picker' id='image-picker-"+image_num+"-"+image_img_num+"' onclick='addImageItemImg(this)'><img src='"+base_url+"/upload/shopcustompagedecoration/base/pc_image_01.jpg'/></div><div class='image-tips'>"+WST.lang('shopcustompage_image_img_tips2')+"</div></div><label class='d-label'>"+WST.lang('shopcustompage_link')+"</label><input type='text' value='' class='attr-content-image-link' /><div class='attr-btn-del' onclick='delImageItemBtn(this)'>X</div></div>");
    WST.upload({
...
function addShopwindowItem(obj){
    ...
    $(".effect-shopwindow").each(function(idx, item) {
        if($(item).hasClass("shopwindow"+shopwindow_num)){
            ...
            $(item).find(".shopwindows").append("<div class='shopwindows-item' style='"+style+"' ><img src='"+base_url+"/upload/shopcustompagedecoration/base/shopwindow_01.jpg' alt='' shopwindow_item="+shopwindow_item+" /></div>");
            if(shopwindow_layout == 4){
                $(item).find(".shopwindows-layout img").each(function(key, value) {
                    $(value).remove();
                });
...
function changeShopwindowLayout(obj){
    ...
    var shopwindow_layout_info1 = WST.lang("shopcustompage_shopwindow_image_tips1");
    var shopwindow_layout_info2 = WST.lang("shopcustompage_shopwindow_image_tips2");
    var shopwindow_layout_count = 0;
    var shopwindow_layout_array = [];
    var flag = false;
        ...
        if($(item).hasClass(image_text_attr_class_name)){
            ...
            $(item).find(".attr-content-image-text-img").each(function(index, obj) {
                ...
                switch(image_text_item){
                    case '1':
                        $(obj).find('.image-tips').html(WST.lang('shopcustompage_image_text_img_tips1'));
                        break;
                    case '2':
                        $(obj).find('.image-tips').html(WST.lang('shopcustompage_image_text_img_tips1'));
                        break;
                    case '3':
                        $(obj).find('.image-tips').html(WST.lang('shopcustompage_image_text_img_tips2'));
                        break;
                    case '4':
                        $(obj).find('.image-tips').html(WST.lang('shopcustompage_image_text_img_tips3'));
                        break;
                    case '5':
                        $(obj).find('.image-tips').html(WST.lang('shopcustompage_image_text_img_tips1'));
                        break;
...
function addNews(obj){
    ...
    var box = WST.open({title:WST.lang('shopcustompage_select_new'),type:1,content:$('#newBox'),area: ['80%', '55%'],btn: [WST.lang('shopcustompage_confirm'),WST.lang('shopcustompage_cancel')],
        yes:function(){
            $(".attr-content-new").each(function(idx, item) {
...
function submit(){
    ...
    if(params.page_name==''){
        WST.msg(WST.lang('shopcustompage_require_page_name'),{time:1000,anim: 6});
        return;
    }
    // 页面设置组件开始
    var page_component = [];
    ...
    var floor_goods_icon = [];
    var floor_goods_img = [];
    var floor_goods_link = [];
    var floor_goods_title = [];
    var floor_goods_columns_title = [];
    var floor_goods_columns_link = [];
    var floor_goods_columns_title_temp = [];
    var floor_goods_columns_link_temp = [];
    var floor_goods_goods_select = [];
    var floor_goods_goods_select_ids = [];
    var floor_goods_goods_select_cats_id = [];
    var floor_goods_order = [];
    var floor_goods_goods_tag = [];
    var floor_goods_goods_min_price = [];
    var floor_goods_goods_max_price = [];
    ...
    for(var i=0;i<floor_goods_nums.length;i++){
        $(".attr-content-floor-goods").each(function(idx, item) {
            if($(item).attr("floor_goods_num") == floor_goods_nums[i]){
                ...
                floor_goods_icon.push($(item).find(".attr-content-floor-goods-icon").attr("img_url"));
                ...
                floor_goods_background_color.push($(item).find(".floor-goods-bg-color").val());
                var selectIds = $(item).find('.floor-goods-select-ids-value').val();
                $(item).find("input[name^='floor-goods-select']").each(function (key, value) {
                    if ($(value).prop('checked')) {
                        floor_goods_goods_select.push($(value).val());
                    }
                });
                floor_goods_goods_select_ids.push(selectIds);
                var selectCatsId = $(item).find('.floor-goods-select-cats-id-value').val();
                floor_goods_goods_select_cats_id.push(selectCatsId);
                var tag = $(item).find('.floor-goods-tag-value').val();
                floor_goods_goods_tag.push(tag);
                var minPrice = $(item).find('.floor-goods-min-price-value').val();
                floor_goods_goods_min_price.push(minPrice);
                var maxPrice = $(item).find('.floor-goods-max-price-value').val();
                floor_goods_goods_max_price.push(maxPrice);
                var floor_goods_num = $(item).attr("floor_goods_num");
                $(".attr-content-floor-goods-item-column").each(function(index, obj) {
                    if($(obj).parent().attr("floor_goods_num") == floor_goods_num){
                        ...
                        floor_goods_columns_link_temp.push($(obj).find('.floor-goods-columns-link').val());
                    }
                });
                floor_goods_columns_title[i] = floor_goods_columns_title_temp;
                floor_goods_columns_link[i] = floor_goods_columns_link_temp;
                floor_goods_columns_title_temp = [];
                floor_goods_columns_link_temp = [];
            }
        });
        $(".attr-content-floor-goods").each(function(idx, item) {
            if($(item).attr("floor_goods_num") == floor_goods_nums[i]){
    ...
    params.floor_goods_background_color = floor_goods_background_color;
    params.floor_goods_icon = floor_goods_icon;
    ...
    params.floor_goods_columns_link = floor_goods_columns_link;
    params.floor_goods_goods_select = floor_goods_goods_select;
    params.floor_goods_goods_select_ids = floor_goods_goods_select_ids;
    params.floor_goods_goods_select_cats_id = floor_goods_goods_select_cats_id;
    params.floor_goods_goods_tag = floor_goods_goods_tag;
    params.floor_goods_goods_max_price = floor_goods_goods_max_price;
    params.floor_goods_goods_min_price = floor_goods_goods_min_price;
    ...
    // 商品推荐组件开始
    var goods_recommend_component = [];
    var goods_recommend_item_id = [];
    var goods_recommend_title = [];
    var goods_recommend_background_color = [];
    var show_goods_recommend_title = [];
    var goods_recommend_goods_select = [];
    var goods_recommend_goods_select_ids = [];
    var goods_recommend_goods_select_cats_id = [];
    var goods_recommend_order = [];
    var goods_recommend_goods_tag = [];
    var goods_recommend_goods_min_price = [];
    var goods_recommend_goods_max_price = [];
    var goods_recommend_nums = [];
    $(".effect-goods-recommend").each(function(idx, item) {
        var goods_recommend_num = $(item).attr("goods_recommend_num");
        goods_recommend_nums.push(goods_recommend_num);
    });
    for(var i=0;i<goods_recommend_nums.length;i++) {
        $(".attr-content-goods-recommend").each(function (idx, item) {
            if ($(item).attr("goods_recommend_num") == goods_recommend_nums[i]) {
                var goodsGroupName = "goods_recommend"+$(item).attr("goods_recommend_num");
                goods_recommend_component.push(goodsGroupName);
                goods_recommend_item_id.push($(item).find(".goods-recommend-item-id").val());
                var goodsGroupTitle = $(item).find(".attr-content-goods-recommend-title").val();
                if (goodsGroupTitle.length > 10) {
                    goodsGroupTitle = goodsGroupTitle.substr(0, 10);
                }
                goods_recommend_title.push(goodsGroupTitle);
                show_goods_recommend_title.push($(item).find(".show-goods-recommend-title").val());
                goods_recommend_order.push($(item).find(".goods-recommend-order").val());
                goods_recommend_background_color.push($(item).find(".goods-recommend-bg-color").val());
                $(item).find("input[name^='goods-recommend-select']").each(function (key, value) {
                    if ($(value).prop('checked')) {
                        goods_recommend_goods_select.push($(value).val());
                    }
                });
                var selectIds = $(item).find('.goods-select-ids-value').val();
                goods_recommend_goods_select_ids.push(selectIds);
                var selectCatsId = $(item).find('.goods-select-cats-id-value').val();
                goods_recommend_goods_select_cats_id.push(selectCatsId);
                var tag = $(item).find('.goods-tag-value').val();
                goods_recommend_goods_tag.push(tag);
                var minPrice = $(item).find('.goods-min-price-value').val();
                goods_recommend_goods_max_price.push(minPrice);
                var maxPrice = $(item).find('.goods-max-price-value').val();
                goods_recommend_goods_min_price.push(maxPrice);
            }
        });
    }
    params.goods_recommend_component = goods_recommend_component;
    params.goods_recommend_background_color = goods_recommend_background_color;
    params.goods_recommend_title = goods_recommend_title;
    params.show_goods_recommend_title = show_goods_recommend_title;
    params.goods_recommend_goods_select = goods_recommend_goods_select;
    params.goods_recommend_goods_select_ids = goods_recommend_goods_select_ids;
    params.goods_recommend_goods_select_cats_id = goods_recommend_goods_select_cats_id;
    params.goods_recommend_goods_tag = goods_recommend_goods_tag;
    params.goods_recommend_goods_max_price = goods_recommend_goods_max_price;
    params.goods_recommend_goods_min_price = goods_recommend_goods_min_price;
    params.goods_recommend_order = goods_recommend_order;
    params.goods_recommend_item_id = goods_recommend_item_id;
    //console.log("params:",params);return;
    // 商品推荐组件开始
    ...
    for(var i=0;i<nav_nums.length;i++){
        ...
        $(".attr-content-nav").each(function(idx, item) {
            if($(item).attr("nav_num") == nav_nums[i]){
                ...
                show_nav_button .push($(item).find(".show-nav-button").val());
            }
        });
    }
    ...
    params.show_nav_button = show_nav_button;
        ...
        $(".attr-content-marketing").each(function(idx, item) {
            if($(item).attr("marketing_num") == marketing_nums[i]){
                ...
                if(marketing_type_value == 0){
                    WST.msg(WST.lang('shopcustompage_select_marketing'),{time:1000,anim: 6});
                    marketing_flag = true;
                }else{
    ...
    if(!submitStatus){
        submitStatus = true;
        $.post(WST.AU('shopcustompage://shopcustompagedecoration/edit'),params,function(res){
            var json = WST.toJson(res);
            if(json.status==1){
                WST.msg(WST.lang("shopcustompage_operation_success"),{icon:1});
                setTimeout(function(){
                    location.href=WST.AU('shopcustompage://shop/index','type='+params.page_type);
                },1000);
            }else{
                WST.msg(json.msg,{icon:2});
            }
