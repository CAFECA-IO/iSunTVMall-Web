 ...
 * 店铺首页自定义页面详情业务处理
 */
class ShopCustomPageDecoration extends Base{
    /**
     * 获取自定义页面装修内容
     */
    ...
    public function pageDetail($pageId=0){
        return Db::name('shop_custom_pages')->where(['id'=>$pageId])->find();
    }
    ...
    public function edit(){
        ...
        try{
            if($pageId){
                Db::name('shop_custom_pages')->where("id","=",$pageId)->update($pageData);
            }else{
                $pageId = Db::name('shop_custom_pages')->insertGetId($pageData);
            }
            $swiper_item_id = (array)input("swiper_item_id");
            $floor_goods_item_id = (array)input("floor_goods_item_id");
            $goods_recommend_item_id = (array)input("goods_recommend_item_id");
            $goods_group_item_id = (array)input("goods_group_item_id");
            $goods_swiper_item_id = (array)input("goods_swiper_item_id");
            ...
            if(!$goods_recommend_item_id){
                $this->deleteComponent("goods_recommend");
            }
            if(!$goods_group_item_id){
                $this->deleteComponent("goods_group");
            }
            if(!$goods_swiper_item_id){
                $this->deleteComponent("goods_swiper");
            }
            if(!$nav_item_id){
            ...
            $floor_goods_background_color = input("floor_goods_background_color");
            $floor_goods_title = input("floor_goods_title");
            $floor_goods_img = input("floor_goods_img");
            $floor_goods_icon = input("floor_goods_icon");
            $floor_goods_link = input("floor_goods_link");
            $floor_goods_columns_title = input("floor_goods_columns_title");
            $floor_goods_columns_link = input("floor_goods_columns_link");
            $floor_goods_goods_select = input("floor_goods_goods_select"); // 选取商品类型
            $floor_goods_goods_select_ids = input("floor_goods_goods_select_ids"); // 手动添加的商品id
            $floor_goods_goods_select_cats_id = input("floor_goods_goods_select_cats_id"); // 按条件选取的分类id
            $floor_goods_goods_tag = input("floor_goods_goods_tag"); // 按条件选取的商品标签
            $floor_goods_goods_min_price = input("floor_goods_goods_min_price"); // 按条件设置的商品最低价
            $floor_goods_goods_max_price = input("floor_goods_goods_max_price"); // 按条件设置的商品最高价
            $floor_goods_order = input("floor_goods_order");
            for($i=0;$i<count($floor_goods_component);$i++){
                $floor_goods_data_attr[$floor_goods_component[$i]]["background_color"] = $floor_goods_background_color[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["title"] = $floor_goods_title[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["icon"] = $floor_goods_icon[$i];
                ...
                $floor_goods_data_attr[$floor_goods_component[$i]]["columns_link"] = $floor_goods_columns_link[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_select"] = $floor_goods_goods_select[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_select_ids"] = $floor_goods_goods_select_ids[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_select_cats_id"] = $floor_goods_goods_select_cats_id[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_tag"] = $floor_goods_goods_tag[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_min_price"] = $floor_goods_goods_min_price[$i];
                $floor_goods_data_attr[$floor_goods_component[$i]]["goods_max_price"] = $floor_goods_goods_max_price[$i];
            ...
            // 商品推荐组件开始
            $goods_recommend_data = [];
            $goods_recommend_data_attr = [];
            $goods_recommend_component = (array)input("goods_recommend_component");
            $goods_recommend_background_color = input("goods_recommend_background_color");
            $goods_recommend_goods_select = input("goods_recommend_goods_select"); // 选取商品类型
            $goods_recommend_goods_select_ids = input("goods_recommend_goods_select_ids"); // 手动添加的商品id
            $goods_recommend_goods_select_cats_id = input("goods_recommend_goods_select_cats_id"); // 按条件选取的分类id
            $goods_recommend_goods_tag = input("goods_recommend_goods_tag"); // 按条件选取的商品标签
            $goods_recommend_goods_min_price = input("goods_recommend_goods_min_price"); // 按条件设置的商品最低价
            $goods_recommend_goods_max_price = input("goods_recommend_goods_max_price"); // 按条件设置的商品最高价
            $goods_recommend_order = input("goods_recommend_order");
            $goods_recommend_title = input("goods_recommend_title");
            $show_goods_recommend_title = input("show_goods_recommend_title");
            for($i=0;$i<count($goods_recommend_component);$i++){
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["background_color"] = $goods_recommend_background_color[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_recommend_title"] = $goods_recommend_title[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["show_goods_recommend_title"] = $show_goods_recommend_title[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_select"] = $goods_recommend_goods_select[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_select_ids"] = $goods_recommend_goods_select_ids[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_select_cats_id"] = $goods_recommend_goods_select_cats_id[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_tag"] = $goods_recommend_goods_tag[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_min_price"] = $goods_recommend_goods_min_price[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_max_price"] = $goods_recommend_goods_max_price[$i];
                $goods_recommend_data_attr[$goods_recommend_component[$i]]["goods_order"] = $goods_recommend_order[$i];
                $goods_recommend_data = [
                    "pageId"=>$pageId,
                    "name"=>"goods_recommend",
                    "attr"=>serialize($goods_recommend_data_attr[$goods_recommend_component[$i]]),
                    "createTime"=>date("Y-m-d H:i:s",time())
                ];
                if(($i+1)<=count($goods_recommend_item_id)){
                    $this->where("id","=",$goods_recommend_item_id[$i])->update($goods_recommend_data);
                }else{
                    $id = $this->insertGetId($goods_recommend_data);
                    $goods_recommend_item_id[] = $id;
                }
            }
            if($goods_recommend_item_id){
                $this->updateComponent($goods_recommend_item_id,"goods_recommend");
            }
            // 商品推荐组件结束
            ...
            // 商品轮播组件开始
            $goods_swiper_data = [];
            $goods_swiper_data_attr = [];
            $goods_swiper_component = (array)input("goods_swiper_component");
            $goods_swiper_title = input("goods_swiper_title");
            $goods_swiper_title_bg = input("goods_swiper_title_bg");
            $goods_swiper_goods_nums = input("goods_swiper_goods_nums");
            $goods_swiper_goods_select = input("goods_swiper_goods_select"); // 选取商品类型
            $goods_swiper_goods_select_ids = input("goods_swiper_goods_select_ids"); // 手动添加的商品id
            $goods_swiper_goods_select_cats_id = input("goods_swiper_goods_select_cats_id"); // 按条件选取的分类id
            $goods_swiper_goods_tag = input("goods_swiper_goods_tag"); // 按条件选取的商品标签
            $goods_swiper_goods_min_price = input("goods_swiper_goods_min_price"); // 按条件设置的商品最低价
            $goods_swiper_goods_max_price = input("goods_swiper_goods_max_price"); // 按条件设置的商品最高价
            $goods_swiper_order = input("goods_swiper_order");
            for($i=0;$i<count($goods_swiper_component);$i++){
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_nums"] = $goods_swiper_goods_nums[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["title"] = $goods_swiper_title[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["title_bg"] = $goods_swiper_title_bg[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_select"] = $goods_swiper_goods_select[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_select_ids"] = $goods_swiper_goods_select_ids[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_select_cats_id"] = $goods_swiper_goods_select_cats_id[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_tag"] = $goods_swiper_goods_tag[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_min_price"] = $goods_swiper_goods_min_price[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_max_price"] = $goods_swiper_goods_max_price[$i];
                $goods_swiper_data_attr[$goods_swiper_component[$i]]["goods_order"] = $goods_swiper_order[$i];
                $goods_swiper_data = [
                    "pageId"=>$pageId,
                    "name"=>"goods_swiper",
                    "attr"=>serialize($goods_swiper_data_attr[$goods_swiper_component[$i]]),
                    "createTime"=>date("Y-m-d H:i:s",time())
                ];
                if(($i+1)<=count($goods_swiper_item_id)){
                    $this->where("id","=",$goods_swiper_item_id[$i])->update($goods_swiper_data);
                }else{
                    $id = $this->insertGetId($goods_swiper_data);
                    $goods_swiper_item_id[] = $id;
                }
            }
            if($goods_swiper_item_id){
                $this->updateComponent($goods_swiper_item_id,"goods_swiper");
            }
            // 商品轮播组件结束
    ...
    public function goodPageQuery(){
        ...
        $rs = Db::name('goods')
            ...
            ->where($where)
            ->field('g.goodsId,gl.goodsName,goodsSn,saleNum,shopPrice,g.shopId,goodsImg,s.shopName,goodsCatIdPath,goodsStock,saleNum')
            ->order($order)
            ->paginate(input('limit/d'))->toArray();
        return $rs;
    }
    ...
    public function getCustomPageDecorationData($pageId){
        $data = Db::name('shop_custom_page_decoration')->field('id,name,attr,sort')->where(['dataFlag'=>'1','pageId'=>$pageId])->order('sort asc')->select();
        foreach($data as $k => $v){
            $data[$k]["attr"] = unserialize($data[$k]["attr"]);
        }
        foreach($data as $k => $v){
            if($v["name"] == "swiper"){
                ...
                if(isset($v['attr']['show_nav_category'])){
                    ...
                    $data[$k]['showNavButton'] = $v["attr"]["show_nav_button"];
                }else {
                    // 手机端和微信端的导航组件参数
                    // 样式类型
                    $style = $v["attr"]["style"];
                ...
                $data[$k]['columnsLink'] = $v["attr"]["columns_link"];
                $where = [
                    'columns_title'=>[1],
                ...
                unset($data[$k]['attr']);
            }elseif($v["name"] == "goods_recommend") {
                $data[$k]['backgroundColor'] = $v["attr"]["background_color"];
                $data[$k]['goodsRecommendTitle'] = $v["attr"]["goods_recommend_title"];
                $data[$k]['showGoodsRecommendTitle'] = $v["attr"]["show_goods_recommend_title"];
                $where = [
                    'columns_title'=>[1],
                    'goods_select'=>(array)$v['attr']['goods_select'],
                    'goods_nums'=>4,
                ...
                $data[$k]['goods'] = $this->getGoods($where);
                unset($data[$k]['attr']);
            }elseif($v["name"] == "goods_swiper") {
                $data[$k]['title'] = $v["attr"]["title"];
                $data[$k]['title_bg'] = $v["attr"]["title_bg"];
                $where = [
                ...
                $data[$k]['goods'] = $this->getSwiperGoods($where);
                unset($data[$k]['attr']);
            }elseif($v['name'] == 'search'){
     ...
     * 获取店铺首页自定义商品组件的商品
     */
    public function getGoods($array,$sId=0){
        ...
        for($i=0;$i<count($columnsTitle);$i++){
            if($goodsSelect[$i]==1){
                ...
                $data[$i] = Db::name('goods')->alias('g')
                    ...
                    ->where([['g.shopId','=',$shopId],['g.shopCatId1','=',$goodsSelectCatsId[$i]],['g.isSale','=',1],['g.dataFlag','=',1],['g.goodsStatus','=',1]])
                    ->where($where)
                    ->where($where2)
                    ->where($where3)
     ...
     * 获取店铺首页自定义商品轮播组件的商品
     */
    public function getSwiperGoods($array,$sId=0){
        $shopId = ($sId>0)?$sId:(int)session('WST_USER.shopId');
        $goodsNums = $array['goods_nums'];
        $goodsSelect = $array['goods_select'];
        $goodsSelectIds = $array['goods_select_ids'];
        ...
        if($goodsSelect==1){
            ...
            $minPrice = $goodsMinPrice; // 最低价格
            $maxPrice = $goodsMaxPrice; // 最高价格
            if($minPrice!="")$where2 = "g.shopPrice >= ".(float)$minPrice;
            if($maxPrice!="")$where3 = "g.shopPrice <= ".(float)$maxPrice;
            $data = Db::name('goods')->alias('g')
                ->join('__GOODS_LANGS__ gl','gl.goodsId=g.goodsId and gl.langId='.WSTCurrLang())
                ->where([['g.shopId','=',$shopId],['g.shopCatId1','=',$goodsSelectCatsId],['g.isSale','=',1],['g.dataFlag','=',1],['g.goodsStatus','=',1]])
                ->where($where)
                ->where($where2)
                ->where($where3)
                ->field('g.goodsId,gl.goodsName,g.goodsImg,g.shopPrice')
                ->order($order)->limit($goodsNums)->select();
        }else{
            // 手动添加
            $data = Db::name('goods')->alias('g')
                ->join('__GOODS_LANGS__ gl','gl.goodsId=g.goodsId and gl.langId='.WSTCurrLang())
                ->where([['g.goodsId','in',$goodsSelectIds],['g.isSale','=',1],['g.dataFlag','=',1],['g.goodsStatus','=',1]])
                ->field('g.goodsId,gl.goodsName,g.goodsImg,g.shopPrice')
                ->order($order)->select();
        }
        return $data;
     ...
     * 获取店铺首页自定义营销活动组件的商品
     */
    function getMarketingGoods($type,$sId=0){
        $shopId = ($sId>0)?$sId:(int)session('WST_USER.shopId');
        $rs = [];
        switch($type){
            case 'Pintuan':
                if(WSTConf('WST_ADDONS.pintuan')!='') {
                    $rs = custom_page_pintuan_list($shopId);
                }
                break;
            case 'Seckill':
                if(WSTConf('WST_ADDONS.seckill')!='') {
                    $rs = custom_page_seckill_list($shopId);
                }
                break;
            case 'Auction':
                if(WSTConf('WST_ADDONS.auction')!='') {
                    $rs = custom_page_auction_list($shopId);
                }
                break;
            case 'Bargain':
                if(WSTConf('WST_ADDONS.bargain')!='') {
                    $rs = custom_page_bargain_list($shopId);
                }
                break;
        }
        return $rs;
    ...
    /*
     * 获取小程序端店铺首页自定义页面数据【weapp】
     */
    public function getWeappCustomPageDecorationData(){
        $pageId = input('pageId');
        $shopId = input('shopId');
        $pageData = Db::name('shop_custom_pages')->where(['dataFlag'=>'1','id'=>$pageId])->value('attr');
        $pageAttr = unserialize($pageData);
        $data = Db::name('shop_custom_page_decoration')->field('name,attr,sort')->where(['dataFlag'=>'1','pageId'=>$pageId])->order('sort asc')->select();
        foreach($data as $k => $v){
            $data[$k]["attr"] = unserialize($data[$k]["attr"]);
        }
        $data[] = [
        ...
        foreach($data as $k => $v){
            if($v["name"] == "swiper"){
                ...
                $data[$k]['goods'] = $this->getGoods($where,$shopId);
                unset($data[$k]['attr']);
            }elseif($v["name"] == "goods_swiper") {
                $data[$k]['title'] = $v["attr"]["title"];
                $data[$k]['titleBg'] = $v["attr"]["title_bg"];
                $where = [
                    'goods_select'=>$v['attr']['goods_select'],
                    'goods_nums'=>$v['attr']['goods_nums'],
                    'goods_select_ids'=>$v['attr']['goods_select_ids'],
                ...
                $data[$k]['goods'] = $this->getSwiperGoods($where,$shopId);
                unset($data[$k]['attr']);
            }else if($v["name"] == "image"){
                $data[$k]['backgroundColor'] = $v["attr"]["background_color"];
                $data[$k]['paddingTop'] = $v["attr"]["padding_top"];
                ...
                $data[$k]['verticalPadding'] = $v["attr"]["vertical_padding"];
                $data[$k]['type'] = $v["attr"]["type"];
                $goods = $this->getMarketingGoods($v['attr']['type'],$shopId);
                $data[$k]['goods'] = $goods;
