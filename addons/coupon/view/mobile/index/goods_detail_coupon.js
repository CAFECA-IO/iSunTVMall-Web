...
function getcouponList(){
    ...
    $.post(WST.AU('coupon://coupons/getCouponsByGoods'), param, function(data){
        ...
        if(json && json.status==1 && json.data!=undefined){
          ...
          var txt1 = _obj.useCondition==0?WST.lang('coupon_money_no_threshold',[_obj.couponValue]):WST.lang('coupon_full_reduction_tips',[_obj.useMoney,_obj.couponValue]);
          $('#coupon_txt1').html(txt1);

          if(json.data.length>1){
          	  _obj = json.data[1];
          	  $('#gc_item2').show();
          	  var txt2 = _obj.useCondition==0?WST.lang('coupon_money_no_threshold',[_obj.couponValue]):WST.lang('coupon_full_reduction_tips',[_obj.useMoney,_obj.couponValue]);
          ...
          $('#totalPage').val(json.last_page);
           ...
           mhtml += '<p>'+WST.lang('coupon_no_data')+'</p>';
           mhtml += '</li>';
           mhtml += '</ul>';
