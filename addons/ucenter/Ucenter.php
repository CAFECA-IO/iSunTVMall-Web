...
class Ucenter extends Addons{
    ...
    public $info = [
        ...
        'description' => '實現用戶壹個賬號，在壹處登錄，全站通行',    // 插件简介
        'status' => 0,  // 状态
        'author' => 'WSTMart',
        'version' => '1.0.1'
    ];
    ...
    public function install(){
    	$m = new DM();
        $lang = WSTSwitchLang();
        $langFile = Env::get('root_path').'addons'.DS.'ucenter'.DS.'lang'.DS.$lang.'.php';
        if(is_file($langFile)){
            Lang::load($langFile);
        }
        WSTLangAddonJs('ucenter');
    	$flag = $m->install();
    	WSTClearHookCache();
        return $flag;
    }
    ...
    public function afterUserLogout($params){
	    ...
	    $rd = ['status'=>'1','msg'=>$uid."成功退出"];//$uids返回在页面上，才同步成功登录
	    exit(json_encode($rd));
    }
