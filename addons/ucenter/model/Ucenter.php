...
class Ucenter extends Base{
	...
	public function afterUserRegist($params){
    	...
    	if($decrypt_data['status']==1){
			...
			exit(json_encode(WSTReturn(lang('ucenter_login_has_failed'))));
    	}
	...
	public function beforeUserLogin($params){
    	...
    	if($decrypt_data['status']==1){
			...
			exit(json_encode(WSTReturn(lang('ucenter_login_failed'))));
    	}
		//UC同步
		$ucenter = new \addons\ucenter\api\UcenterApi();
	...
	public function afterUserLogin($params){
		...
		if(isset($params['user']['datau']['uid']) && $params['user']['datau']['uid']>0){
			...
			$rd = ['status'=>'1','msg'=>$uids.lang('ucenter_login_successful')];//$uids返回在页面上，才同步成功登录
			exit(json_encode($rd));
		}
	}
	...
	public function afterEditPass($params){
    	...
    	if($decrypt_data['status']==1){
			...
			exit(json_encode(WSTReturn(lang('ucenter_modification_failed'))));
    	}
		...
		if($data['type']==1){
			...
			if(!empty($rs)){
				...
				if(input("post.typ")==2){
					...
					if(empty($shoprs)){
						return WSTReturn(lang('ucenter_you_haven_applied_for_a_store_yet'));
					}
				}
				$ip = request()->ip();
				$update = [];
