...
class Weixinpaysmo extends Controller{
	...
	public function notify() {
		...
		if($signStatus){
			file_put_contents("runtime/log_wxpay_nodify.txt",date('Y-m-d H:i:s')."--auction--signStatus----ok----\n", FILE_APPEND); 
			...
			file_put_contents("runtime/log_wxpay_nodify.txt",date('Y-m-d H:i:s')."----auction----".$cipherText."----\n", FILE_APPEND);
			$order = json_decode($cipherText,true);
			file_put_contents("runtime/log_wxpay_nodify.txt",date('Y-m-d H:i:s')."----auction----process----\n", FILE_APPEND);   
