	...
	*/
	public function getAuctionDetail(){
		...
		if(isset($rs['article'])){
			...
			$rs['article'] = str_replace('${DOMAIN}',WSTConf('CONF.resourceDomain'),$rs['article']);
		}
		// 热门拍卖
		...
		if(empty($data)){
			...
			if($data['isPay']==1){
				...
				if($auction->orderId>0){
					...
					// 获取店铺id及店铺是否允许开发票
					$addData["shopId"] = $auction["shopId"];
					$invRes = model("common/shops")->getFieldsById($auction["shopId"],"isInvoice");
					$addData["isInvoice"] = isset($invRes['isInvoice'])?$invRes['isInvoice']:0;
