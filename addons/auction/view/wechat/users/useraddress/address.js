...
function editAddress(addressId){
    ...
    if(addressId>0){
    	...
    	$('#header-title').html(WST.lang('auction_add_address'));
    }
    switchPage(1);
}
...
function saveAddress(addressId){
    ...
    if(userName==''){
        WST.msg(WST.lang('auction_require_consignee'),'info');
        return false;
    }
    if(userPhone==''){
        WST.msg(WST.lang('auction_require_contact_number'),'info');
        return false;
    }
    if(areaId==''){
        WST.msg(WST.lang('auction_please_select_area'),'info');
        return false;
    }
    if(userAddress==''){
        WST.msg(WST.lang('auction_require_detail_address'),'info');
        return false;
    }
    var param = {};
...
WST.ITAreas = function(opts){
	...
	$.post(WST.U('wechat/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
	     ...
	     if(json.data && json.data.length>0){
		     ...
		     $(".wst-fr-box .option").append('<p class="ui-nowrap-flex term active_'+level+' active" onclick="javascript:inOption(this,'+level+')">'+WST.lang('auction_please_select')+'</p>');
	     }else{
	    	 opts.isLast = true;
...
function addWechatAddress(){
	wx.openAddress({
		success: function (res) {
			var params = {};
			params.userName = res.userName;
			params.userPhone = res.telNumber;
			params.userAddress = res.detailInfo;
			params.province = res.provinceName;
			params.city = res.cityName;
			params.county = res.countryName;
			$.post(WST.U('wechat/useraddress/addWechatAddress'), params, function(data){
				var json = WST.toJson(data);
				if( json.status == 1 ){
					WST.msg(json.msg,'success');
					setTimeout(function(){
						location.href = WST.AU('auction://useraddress/index');
					},1500);
				}else{
					WST.msg(json.msg,'warn');
				}
