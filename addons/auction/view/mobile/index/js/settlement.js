...
function setInvoiceText(){
	...
	if(invoiceType==1){
		text = (invoiceObj==0)?(WST.lang('auction_invoice_special_personal_detail')):(WST.lang('auction_invoice_special_detail',['<br />'+invoiceHead+'<br />']));
	}else{
		text = (invoiceObj==0)?(WST.lang('auction_invoice_normal_personal_detail')):(WST.lang('auction_invoice_personal_detail',['<br />'+invoiceHead+'<br />']));
	}
	$('#invoicest').html(text);
	invoiceHide();
}
...
function submitOrder(){
	...
	if($('#givesh').val()==0 && addressId==''){
		WST.msg(WST.lang('auction_please_select_address'),'info');
		return false;
	}
	WST.load(WST.lang('auction_submitting'));
...
function getStores(val,name){
	...
	var areaName = (name!=undefined)?name:(($('#areaName').val()!='')?$('#areaName').val():WST.lang('auction_require_select_area'));
	var shopId =  $('#shopId').val();
	$.post(WST.AU('auction://auction/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>"+WST.lang('auction_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
...
WST.ITAreas = function(opts){
	...
	$.post(WST.U('mobile/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
		...
		if(json.data && json.data.length>0){
			...
			var listh = h/2-106;
			$(".wst-fr-box3 .list").css('overflow-y','scroll').css('height',listh+'px');
			$(".wst-fr-box3 .option").append('<p class="ui-nowrap-flex term active_'+level+' active" onclick="javascript:inOption(this,'+level+')">'+WST.lang('auction_please_select')+'</p>');
		}else{
			opts.isLast = true;
