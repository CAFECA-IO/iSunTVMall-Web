function queryByPage(p){
	$('#list').html('<tr><td colspan="11"><img src="'+WST.conf.RESOURCE_PATH+'/wstmart/home/view/default/img/loading.gif">'+WST.lang('auction_loading_data')+'</td></tr>');
	var params = {};
	params = {};
...
function delAddr(id){
	WST.confirm({content:WST.lang('auction_confirm_del'),yes:function(index){
...
function getAddressList(obj){
	...
	var load = WST.load({msg:WST.lang('auction_loading_data')});
...
function editAddress(){
	...
	var load = WST.load({msg:WST.lang('auction_submitting')});
...
function emptyAddress(obj,n){
	...
	layerbox =	layer.open({
					title:WST.lang('auction_user_address'),
					type: 1,
					area: ['800px', '300px'],
					content: $('.j-edit-box')
					});
...
function checkSupportStores(){
	...
	$.post(WST.AU('auction://auction/checkSupportStores'),{auctionId:auctionId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data){
    			$("#deliver_btn").show();
...
function getStores(areaId){
	var shopId = $("#shopId").val();
	$.post(WST.AU('auction://auction/getStores'),{shopId:shopId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div>"+WST.lang('auction_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
	    	WST.msg(json.msg,{icon:2});
	    }
...
function checkStores(shopId){
	...
	layerbox =	layer.open({
					title:WST.lang('auction_select_pick_up'),
					type: 1,
					area: ['800px', '600px'],
					content: $('.j-store-box'),
					btn:[WST.lang('auction_confirm'),WST.lang('auction_cancel')],
				    cancel:function(){
				    	//$('#menuBox').hide();
				    },
				    yes:function(){
						...
						if(!(storeId>0)){
							WST.msg(WST.lang('auction_select_pick_up'));
							return;
						}
...
function invAdd(){
	...
	$('.inv_ul').append('<li class="inv_li inv_li_curr inv_editing"><input type="text" id="invoiceHead" placeholder="'+WST.lang('auction_add_invoice_head')+'" value="" style="width:65%;height:21px;padding:1px;"><i></i><div style="top:8px;" class="inv_opabox"><a href="javascript:void(0)" onCLick="addInvoice()">'+WST.lang('auction_save')+'</a></div></li>');
...
function addInvoice(){
	...
	if(head.length==0){
		WST.msg(WST.lang('auction_invoice_head_required'));
		return;
	}
	...
	$.post(WST.U('home/Invoices/add'),{invoiceHead:head},function(data){
		...
		if(json.status==1){
			...
			code.push('<a href=\'javascript:void(0)\' onClick="invEdit(\''+json.data.id+'\',this)" class="edit_btn">'+WST.lang('auction_edit')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="editInvoice(\''+json.data.id+'\',this)" style="display:none;" class="save_btn">'+WST.lang('auction_save')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="delInvoice(\''+json.data.id+'\',this)">'+WST.lang('auction_del')+'</a></div></li>');
...
function setInvoiceText(invoiceHead){
	...
	var text = WST.lang('auction_no_invoice');
	if(isInvoice==1){
		text = (invoiceObj==0)?WST.lang('auction_invoice_personal_detail'):WST.lang('auction_invoice_head_detail',[invoiceHead]);
	}
