...
function toCombineGoods(combineId){
    location.href = WST.AU('combination://combination/mobile','combineId='+combineId);
}
// 查看大图
...
function selectSpec(id){
    ...
    $('.j-spec'+id).html(WST.lang('combination_has_select')+'：'+specNames.join(' '));
...
function toSettlement(combineId){
    ...
    if(!(combineChk['1'] && combineChk['0'])){
        WST.msg(WST.lang('combination_select_least_one'),'info');
        return;
    }
    var json = goodsInfo['list'];
    for(var i=0;i<json.length;i++){
        if($.inArray(json[i]['id'].toString(),combineGoodsIds)!=-1){
            if(json[i]['goodsStock']<=0){
                WST.msg(WST.lang('combination_stock_less_tips',[(i+1)]),'info');
                return;
            }
        }
    }
    location.href = WST.AU('combination://carts/moSettlement','id='+window.base64.encode('combineId='+combineId+'&combineGoodsIds='+combineGoodsIds.join(',')+'&'+g.join('&')+'&time='+new Date().getTime()));
}
