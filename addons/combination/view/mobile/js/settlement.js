...
function setInvoiceText(){
	...
	if(isInvoice==1){
		if(invoiceType==1){
			text = (invoiceObj==0)?(WST.lang('combination_invoice_special_personal_detail')):(WST.lang('combination_invoice_special_detail',['<br />'+invoiceHead+'<br />']));
		}else{
			text = (invoiceObj==0)?(WST.lang('combination_invoice_personal_detail')):(WST.lang('combination_invoice_head_detail',['<br />'+invoiceHead+'<br />']));
		}
	}
	$('#invoicest').html(text);
	invoiceHide();
...
function getCartMoney(){
	...
	WST.load(WST.lang('combination_calculating_price_tips'));
	$.post(WST.AU('combination://carts/getCartMoney'),params,function(data,textStatus){
		...
		if(json.status==1){
		    ...
		    $('#shopC').html(WST.lang('currency_symbol')+json.realTotalMoney.toFixed(2));
...
function submitOrder(){
	...
	if($('#givesh').val()==0 && addressId==''){
		WST.msg(WST.lang('combination_please_select_address'),'info');
		return false;
	}
	WST.load(WST.lang('combination_submitting'));
    var param = WST.getParams('.j-ipt');
    param.combineKey = $('#combineKey').val();
    param.combineId = $('#combineId').val();
	...
	$.post(WST.AU('combination://carts/submit'),param,function(data,textStatus){
		var json = WST.toJson(data);
	    WST.noload();
	    if(json.status==1){
...
function addAddress(type,id){
	...
	location.href = WST.AU('combination://useraddress/moindex','type='+type+'&addressId='+id+'&combineKey='+$('#combineKey').val());
}
...
function getStores(val,name){
	...
	var areaName = (name!=undefined)?name:(($('#areaName').val()!='')?$('#areaName').val():WST.lang('combination_require_select_area'));
	var shopId =  $('#shopId').val();
	$.post(WST.AU('combination://carts/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>"+WST.lang('combination_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
