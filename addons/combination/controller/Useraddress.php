...
class UserAddress extends Controller{
	...
	public function weindex(){
        ...
        $this->assign('combineKey', input('combineKey'));
		...
		return $this->fetch('/wechat/useraddress/list');
	}
	/**
	 * 手机地址管理
	 */
	public function moindex(){
		...
		$this->assign('list', $addressList);
		$this->assign('type', (int)input('type'));
		$this->assign('addressId', (int)input('addressId'));
		$this->assign('combineKey', input('combineKey'));
		return $this->fetch('/mobile/useraddress/list');
	}
}
