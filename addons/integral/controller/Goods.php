...
class Goods extends Controller{
    ...
    public function toEdit(){
        ...
        if($id>0){
            ...
            $object['marketPrice'] = '';
            $object['goodsName'] = lang('integral_please_select_goods');
            ...
            foreach (WSTSysLangs() as $key => $lv) {
                ...
                $object['langParams'][$lv['id']]['integralDesc'] = '';
	...
	public function lists(){
        ...
        $cats = WSTGoodsCats(0);
        $catName = lang('integral_all_goods_cat');
        foreach($cats as $k => $v){
    ...
    */
    ...
    public function wxdetail(){
    	...
    	if(!empty($goods)){
    		...
    		session('wxdetail',lang('integral_goods_lost'));
    ...
    public function modetail(){
    	...
    	if(!empty($goods)){
    		...
    		session('modetail',lang('integral_goods_lost'));
    ...
    public function weDetail(){
        ...
        return jsonReturn(lang('integral_not_find_goods'),-1);
    }
    ...
    public function wxCreatePoster(){
        ...
        $today = date("Ymd");
        $fname = 'integral_qr_wx_'.$today.'_'.$id.'.png';
        $outImg = $subDir.'/'.$fname;
    ...
    public function moCreatePoster(){
        ...
        WSTCreateDir(WSTRootPath().'/'.$subDir);
        $today = date("Ymd");
        $fname = 'integral_qr_mo_'.$today.'_'.$id.'.png';
        $outImg = $subDir.'/'.$fname;
        $shareImg = WSTRootPath().'/'.$outImg;
        if($isNew==0){
            if (file_exists($shareImg)) {
        ...
        $qr_url = addon_url('integral://goods//modetail',array('id'=>$id,'shareUserId'=>base64_encode($userId)),true,true);//二维码内容
        //生成二维码图片
        $qr_code = WSTCreateQrcode($qr_url,'','integral',3600,2);
        $qr_code = WSTRootPath().'/'.$qr_code;
        $rs = $m->createPoster($userId,$qr_code,$outImg);
    ...
    public function weAppShareQrCode(){
        ...
        WSTCreateDir(WSTRootPath().'/'.$subDir);
        $today = date("Ymd");
        $fname = 'integral_qr_we_'.$today.'_'.$goodsId.'.png';
        $outImg = $subDir.$fname;
        $shareImg = WSTRootPath().'/'.$outImg;
        if (file_exists($shareImg)) {
