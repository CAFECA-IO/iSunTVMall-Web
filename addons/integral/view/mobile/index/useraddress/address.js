...
function editAddress(addressId){
    ...
    $('#areaId').val('');
    $('#addresst').html(WST.lang('integral_please_select_address'));
    $('.wst-ad-submit .button').attr('onclick','javascript:saveAddress('+addressId+');');
    $('#defaults').removeClass('default').addClass('nodefault');
    if(addressId>0){
        $('#header-title').html(WST.lang('integral_update_receiving_address'));
        ...
        $('#header-title').html(WST.lang('integral_add_address'));
    }
    switchPage(1);
}
...
function saveAddress(addressId){
    ...
    if(userName==''){
        WST.msg(WST.lang('integral_tips12'),'info');
        return false;
    }
    if(userPhone==''){
        WST.msg(WST.lang('integral_tips13'),'info');
        return false;
    }
    if(areaId==''){
        WST.msg(WST.lang('integral_tips14'),'info');
        return false;
    }
    if(userAddress==''){
        WST.msg(WST.lang('integral_tips15'),'info');
        return false;
    }
    var param = {};
...
function delAddress(addressId){
    WST.dialog(WST.lang('integral_confirm_del'),'toDelAddress('+addressId+')');
}
//删除收货地址
...
WST.ITAreas = function(opts){
    ...
    $.post(WST.U('mobile/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
         ...
         if(json.data && json.data.length>0){
             ...
             $(".wst-fr-box .option").append('<p class="ui-nowrap-flex term active_'+level+' active" onclick="javascript:inOption(this,'+level+')">'+WST.lang('integral_please_select')+'</p>');
         }else{
             opts.isLast = true;
