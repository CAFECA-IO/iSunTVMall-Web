...
function setInvoiceText(){
	...
	if(invoiceType==1){
		text = (invoiceObj==0)?(WST.lang('integral_invoice_special_personal_detail')):(WST.lang('integral_invoice_special_detail',['<br />'+invoiceHead+'<br />']));
	}else{
		text = (invoiceObj==0)?(WST.lang('integral_invoice_normal_personal_detail')):(WST.lang('integral_invoice_personal_detail',['<br />'+invoiceHead+'<br />']));
	}
	$('#invoicest').html(text);
	invoiceHide();
}
...
function getCartMoney(){
	...
	WST.load(WST.lang('integral_calculating_price_tips'));
	if(goodsType==0){
		$.post(WST.AU('integral://carts/getCartMoney'),params,function(data,textStatus){
			...
			if(json.status==1){
		    	...
		    	$('#shopC').html(WST.lang('currency_symbol')+json.shops['goodsMoney'].toFixed(2));
...
function submitOrder(){
	...
	if(goodsType==0 && $('#givesh').val()==0 && addressId==''){
		WST.msg(WST.lang('integral_please_select_address'),'info');
		return false;
	}
	WST.load(WST.lang('integral_submitting'));
...
function getStores(val,name){
	...
	var areaName = (name!=undefined)?name:(($('#areaName').val()!='')?$('#areaName').val():WST.lang('integral_require_select_area'));
	var shopId =  $('#shopId').val();
	$.post(WST.AU('integral://carts/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>"+WST.lang('integral_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
