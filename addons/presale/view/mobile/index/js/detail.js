...
function addCart(goodsType){
    ...
    if(goodsInfo.isSpec==1){
        ...
        if(specIds.length==0){
            WST.msg(WST.lang('presale_select_buy_goods_info'),'info');
        }
        specIds.sort(function(a,b){return a-b;});
        if(goodsInfo.sku[specIds.join(':')]){
	...
	$.post(WST.AU('presale://carts/addCart'),{id:goodsInfo.presaleId,goodsSpecId:goodsSpecId,buyNum:buyNum,rnd:Math.random()},function(data,textStatus){
	     ...
	     if(json.status==1){
	    	 ...
	    	 cartHide();
    		 setTimeout(function(){
    			 location.href=WST.AU('presale://carts/moSettlement');
    		 },1000);
	     }else{
...
function reloadPoster(isNew){
    ...
    $('#Load').show();
    $.post(WST.AU('presale://goods/moCreatePoster'), {id:id,isNew:isNew},function(data){
        $('#Load').hide();
        var json = WST.toJson(data);
        if(json.status==1){
...
function timer(){
    ...
    if(presaleStatus==-1){
        ...
        $('#presaletime').html(WST.lang('presale_activity_is_end'));
    }else{
            ...
            var opts = {
                ...
                callback: function(data){
                    if(data.last>0){
                        ...
                        $('#presaletime').html(WST.lang("presale_to_start")+" "+html.join(''));
                        $('#buyBtn').removeClass('active').attr('disabled', 'disabled');
                    }else{
                        var opts2 = {
                            ...
                            callback: function(data2){
                                if(data2.last>=0){
                                    ...
                                    $('#presaletime').html(WST.lang("presale_to_end")+" "+html.join(''));
...
function goConsult(){
    ...
    location.href=WST.U('mobile/goodsconsult/index',{goodsId:goodsId})
}
