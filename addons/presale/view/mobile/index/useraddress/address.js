    ...
    if(addressId>0){
        ...
        $.post(WST.U('mobile/useraddress/getById'), {addressId:addressId}, function(data){
            ...
            if(info){
                ...
                $('#area_code').html(info.areaCode);
                $('#areaCode').val(info.areaCode);
                jQuery(':radio[name="areacode"][value='+info.areaCode+']').attr("checked",true);
        ...
        $('#header-title').html(WST.lang('presale_add_address'));
    }
    switchPage(1);
}
...
function saveAddress(addressId){
    ...
    if(userName==''){
        WST.msg(WST.lang('presale_require_consignee'),'info');
        return false;
    }
    if(userPhone==''){
        WST.msg(WST.lang('presale_require_contact_number'),'info');
        return false;
    }
    if(areaId==''){
    ...
    if(userAddress==''){
        WST.msg(WST.lang('presale_require_detail_address'),'info');
        return false;
    }
    var param = {};
...
function inDefault(obj,id){
    ...
    $.post(WST.U('mobile/useraddress/setDefault'), {id:id}, function(data){
        ...
        if( json.status == 1 ){
            WST.msg(json.msg,'success');
            setTimeout(function(){
                location.href = WST.AU('presale://useraddress/moindex');
            },1500);
        }else{
            WST.msg(json.msg,'warn');
...
function delAddress(addressId){
    WST.dialog(WST.lang('presale_confirm_del'),'toDelAddress('+addressId+')');
}
//删除收货地址
...
WST.ITAreas = function(opts){
    ...
    $.post(WST.U('mobile/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
         ...
         if(json.data && json.data.length>0){
             ...
             $(".wst-fr-box .option").append('<p class="ui-nowrap-flex term active_'+level+' active" onclick="javascript:inOption(this,'+level+')">'+WST.lang('presale_please_select')+'</p>');
         }else{
             opts.isLast = true;
...
function chooseAddress(id){
    location.href = WST.AU('presale://carts/moSettlement','addressId='+id);
}
