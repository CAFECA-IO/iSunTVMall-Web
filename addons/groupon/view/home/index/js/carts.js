...
function delAddr(id){
	WST.confirm({content:WST.lang('group_confirm_del'),yes:function(index){
...
function getAddressList(obj){
	...
	var load = WST.load({msg:WST.lang('group_loading_data')});
	     ...
	     if(json.status==1){
	    	 if(json.data && json.data && json.data.length){
	    		 ...
	    		 for(var i=0;i<json.data.length;i++){
	    			...
	    			if(tmp.isDefault==1){
	    				html.push('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="j-default">'+WST.lang('group_default_address')+'</span>')
	    			}
	    			html.push('<div class="operate-box">');
	    			if(tmp.isDefault!=1){
	    				html.push('<a href="javascript:;" onclick="setDeaultAddr('+tmp.addressId+')">'+WST.lang('group_set_default_address')+'</a>&nbsp;&nbsp;');
	    			}
	    			html.push('<a href="javascript:void(0)" onclick="javascript:toEditAddress('+tmp.addressId+',this,1,1)">'+WST.lang('group_edit')+'</a>&nbsp;&nbsp;');
	    			if(json.data.length>1){
	    				html.push('<a href="javascript:void(0)" onclick="javascript:delAddr('+tmp.addressId+',this)">'+WST.lang('group_del')+'</a></div>');
	    			}
	    			html.push('<div class="wst-clear"></div>','</li>');
	    		 }
	    		 html.push('<a style="color:#1c9eff" onclick="editAddress()" href="javascript:;">'+WST.lang('group_stow_address')+'</a>');
...
function editAddress(){
	...
	if(isNoSelected){
		WST.msg(WST.lang('group_select_complete_address'),{icon:2});
		return;
	}
	layer.close(layerbox);
	var load = WST.load({msg:WST.lang('group_submitting')});
		...
		var json = WST.toJson(data);
	     if(json.status==1){
	    	 ...
	    	 if(params.isDefault==1){
	    		 $('#isdefault').html(WST.lang('group_default_address')).addClass('j-default');
	    	 }else{
...
function emptyAddress(obj,n){
	...
	layerbox =	layer.open({
					title:WST.lang('group_user_address'),
					type: 1,
					area: ['800px', '300px'],
					content: $('.j-edit-box')
					});
...
function getCartMoney(){
	...
	var load = WST.load({msg:WST.lang('group_calculating_price_tips')});
...
function checkSupportStores(){
	$.post(WST.AU('groupon://carts/checkSupportStores'),{},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data){
    			$("#deliver_btn").show();
...
function getStores(areaId){
	var shopId = $("#shopId").val();
	$.post(WST.AU('groupon://carts/getStores'),{shopId:shopId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div>"+WST.lang('group_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
	    	WST.msg(json.msg,{icon:2});
	    }
...
function checkStores(shopId){
	...
	layerbox =	layer.open({
					title:WST.lang('group_select_pick_up'),
					type: 1,
					area: ['800px', '600px'],
					content: $('.j-store-box'),
					btn:[WST.lang('group_confirm'),WST.lang('group_cancel')],
				    cancel:function(){
				    	//$('#menuBox').hide();
				    },
				    yes:function(){
						...
						if(!(storeId>0)){
							WST.msg(WST.lang('group_select_pick_up'));
							return;
						}
...
function changeInvoice(t,str,obj){
	...
	var loading = WST.load({msg:WST.lang('group_loading_data')});
	$.post(WST.U('home/invoices/index'),param,function(data){
		layer.close(loading);
		// layer弹出层
		invoicebox =	layer.open({
			title:WST.lang('group_invoice_information'),
			type: 1,
			area: ['628px', '420px'], //宽高
			content: data,
			success :function(){
...
function invAdd(){
	...
	$('.inv_ul').append('<li class="inv_li inv_li_curr inv_editing"><input type="text" id="invoiceHead" placeholder="'+WST.lang('group_add_invoice_head')+'" value="" style="width:65%;height:21px;padding:1px;"><i></i><div style="top:8px;" class="inv_opabox"><a href="javascript:void(0)" onCLick="addInvoice()">'+WST.lang('group_save')+'</a></div></li>');
...
function addInvoice(){
	...
	if(head.length==0){
		WST.msg(WST.lang('group_invoice_head_required'));
		return;
	}
	...
	$.post(WST.U('home/Invoices/add'),{invoiceHead:head},function(data){
		...
		if(json.status==1){
			...
			code.push('<a href=\'javascript:void(0)\' onClick="invEdit(\''+json.data.id+'\',this)" class="edit_btn">'+WST.lang('group_edit')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="editInvoice(\''+json.data.id+'\',this)" style="display:none;" class="save_btn">'+WST.lang('group_save')+'</a>');
			code.push('<a href=\'javascript:void(0)\' onClick="delInvoice(\''+json.data.id+'\',this)">'+WST.lang('group_del')+'</a></div></li>');
...
function setInvoiceText(invoiceHead){
	...
	if(invoiceType==1){
		text = (invoiceObj==0)?(WST.lang('group_invoice_special_personal_detail')):(WST.lang('group_invoice_special_detail',['<br />'+invoiceHead+'<br />']));
	}else{
		text = (invoiceObj==0)?(WST.lang('group_invoice_normal_personal_detail')):(WST.lang('group_invoice_personal_detail',['<br />'+invoiceHead+'<br />']));
	}
