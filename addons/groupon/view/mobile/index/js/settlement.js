...
function getCartMoney(){
	...
	WST.load(WST.lang('group_calculating_price_tips'));
	if(goodsType==0){
...
function submitOrder(){
	...
	WST.load(WST.lang('group_submitting'));
...
function getStores(val,name){
	...
	var areaName = (name!=undefined)?name:(($('#areaName').val()!='')?$('#areaName').val():WST.lang('group_require_select_area'));
	var shopId =  $('#shopId').val();
	$.post(WST.AU('groupon://carts/getShopStores'),{shopId:shopId,addressId:addressId,areaId:areaId},function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
	    	if(json.data && json.data.length>0){
	    		...
	    		$("#storelist").html("<div class='no-store'>"+WST.lang('group_curr_area_no_pick_up_tips')+"</div>");
	    	}
	    }else{
