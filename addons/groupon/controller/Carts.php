...
class Carts extends Controller{
    ...
    public function weSettlement(){
        ...
        if(empty($CARTS)){
            return jsonReturn(lang('group_please_select_goods'),-1);
            exit;
        }
        //获取一个用户地址
        $addressId = (int)input('addressId');
        ...
        if(empty($carts['carts'])) return jsonReturn(lang('group_goods_not_exist'),-1);
        if($carts['goodsTotalNum']>0){
            if(empty($carts['carts']))return jsonReturn(lang('group_please_select_goods'),-1);
        }
        ...
        if($payments){
            ...
            $carts['payNames'] = [lang('group_no_payment_method')];
        }
