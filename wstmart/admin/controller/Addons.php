...
class Addons extends Base{
    ...
    */
    ...
    public function toEdit(){
    	$m = new M();
    	$addon = $m->getById();
    	if(!$addon) $this->error(lang('require_addon_install'));
    	$addon_class = get_addon_class($addon['name']);
    	if(!class_exists($addon_class)){
    		trace(lang('require_addon_init_err',[$addon['name']]),'ADDONS','ERR');
    	}
    	$data = new $addon_class;
    	$addon['addons_path'] = $data->addons_path;
    	$this->meta_title   =   lang('addon_set_title').$data->info['title'];
    ...
    public function saveConfig(){
    	...
    	if(!class_exists($class)){
    		return WSTReturn(lang('addon_no_exist'),-1);
    	}
    	$addons = new $class;
    	$addons->saveConfig();
        $this->assign("p",(int)input("p"));
