...
function initGrid(){
    ...
    var cols = [
            {title:WST.lang('supp_settlement_name'), name:'supplierName' ,width:60,renderer:function(val,item,rowIndex){
    ...
    mmg = $('.mmg').mmGrid({height: (h-87),indexCol: true,indexColWidth:50,  cols: cols,method:'POST',
        url: WST.U('admin/suppliers/renewMoneyByPage',WST.getParams('.ipt')), fullWidthRows: true, autoLoad: true,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });  
...
function toExport(){
    ...
    var box = WST.confirm({content:WST.lang('are_you_sure_you_want_to_export_this_data'),yes:function(){
        layer.close(box);
        location.href=WST.U('admin/suppliers/toExportRenewMoney',params);
    }});
}