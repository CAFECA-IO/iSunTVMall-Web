...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:WST.lang('label_attr_cat'), name:'goodsCatNames', width: 300,renderer: function(val,item,rowIndex){
                return "<span  ><p class='wst-nowrap'>"+item['goodsCatNames']+"</p></span>";
            }},
            {title:WST.lang('is_it_allowed_to_upload_pictures'), name:'isAllowImg', width: 10,renderer: function(val,item,rowIndex){
            	return (val==1)?"<span class='statu-yes'><i class='fa fa-check-circle'></i> "+WST.lang('allow')+"</span>":'';
            }},
            {title:WST.lang('is_show'), name:'attrVal', width: 10,renderer: function(val,item,rowIndex){
            	return '<input type="checkbox" '+((item['isShow']==1)?"checked":"")+' id="isShow1" name="isShow1" value="1" class="ipt" lay-skin="switch" lay-filter="isShow1" data="'+item['catId']+'" lay-text="'+WST.lang('is_show_val')+'">'
            }},
            {title:WST.lang('op'), name:'op' ,width:50, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
		        if(item.specId>0){
		        	if(WST.GRANT.SPGG_02)h += "<a class='btn btn-blue' href='javascript:toEdit("+ item['catId']+"," + item['id'] + ")'><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";
		        	if(WST.GRANT.SPGG_03)h += "<a class='btn btn-red' href='javascript:toDel(" + item['id'] + ")'><i class='fa fa-trash-o'></i>"+WST.lang('del')+"</a> ";
		            return h;
		        }else{
		        	if(WST.GRANT.SPGG_02)h += "<a class='btn btn-blue' href='javascript:toEditCat(" + item['id'] + ")' ><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";
		        	if(WST.GRANT.SPGG_03)h += "<a class='btn btn-red' href='javascript:toDelCat(" + item['id'] + ")'><i class='fa fa-trash-o'></i>"+WST.lang('del')+"</a> ";
		        }
		        return h;
            }}
            ];
...
function toEditCat(catId){
	...
	$.post(WST.U('admin/speccats/get'),{catId:catId},function(data,textStatus){
		...
		var box = WST.open({title:title,type:1,content:$('#specCatsBox'),area: ['100%', '100%'],offset: 't',btn:[WST.lang('confirm'),WST.lang('cancel')],btnAlign: 'c',end:function(){$('#specCatsBox').hide();},yes:function(){
			$('#specCatsForm').submit();
		}});
		var fields = {};
		var n = 0;
		for(var i in WST.conf.sysLangs){
			...
			fields['langParams'+n+'catName'] = {
				tip: WST.lang('require_cat_name'),
				rule: WST.lang('form_name')+':required;'
			}
		}
...
function toEdit(catId,specId){
	    $.post(WST.U('admin/specs/get'),{specId:specId},function(data,textStatus){
			...
			var box = WST.open({title:title,type:1,content:$('#specBox'),area: ['450px', '160px'],btn:[WST.lang('confirm'),WST.lang('cancel')],yes:function(){
				$('#specForm').submit();
			}});
			$('#specForm').validator({
				rules: {
		        ...
		        fields: {
		        	'specName': {rule:"required; remote;",msg:{required:WST.lang('require_catName')}},
		        },
		        valid: function(form){
		    	   var params = WST.getParams('.ipt');
		    	   params.catId = catId;
