...
$(function(){
    ...
    var cols = [
            {title:WST.lang('label_logshoplogins_user'), name:'loginName', width: 50},
            {title:WST.lang('suppliergoods_supplier'), name:'supplierName', width: 100},
    ...
    mmg = $('.mmg').mmGrid({height: h-162,indexCol: true,indexColWidth:50,cols: cols,method:'POST',
        url: WST.U('admin/supplierlogoperates/pageQuery'), fullWidthRows: true, autoLoad: true,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });   
...
function loadGrid(){
	mmg.load({page:1,startDate:$('#startDate').val(),endDate:$('#endDate').val(),supplierName:$('#supplierName').val(),loginName:$('#loginName').val(),operateUrl:$('#operateUrl').val()});
}
function toView(id){
	 var loading = WST.msg(WST.lang('loading'), {icon: 16,time:60000});
	 $.post(WST.U('admin/supplierlogoperates/get'),{id:id},function(data,textStatus){
	       layer.close(loading);
