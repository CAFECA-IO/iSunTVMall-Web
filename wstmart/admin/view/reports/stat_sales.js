	...
	$.post(WST.U('admin/reports/statSales'),WST.getParams('.ipt'),function(data,textStatus){
	    ...
	    if(json.status=='1' && json.data){
			var option = {
			    ...
			    calculable : true,
			    legend: {
			        data:[WST.lang('label_goods_cat_template_pc'),WST.lang('label_goods_cat_template_wechat'),WST.lang('touch_screen_terminal'),WST.lang('product_weapp'),WST.lang('product_app2'),WST.lang('product_app1'),WST.lang('total_sales')]
			    },
			    xAxis : [
			        {
			            type : 'category',
...
function toExport(){
    ...
    var box = WST.confirm({content:WST.lang('are_you_sure_you_want_to_export_this_data'),yes:function(){
        layer.close(box);
        location.href=WST.U('admin/reports/toExportStatSales',params);
    }});
}