...
function initGrid(p){
  ...
  var cols = [
            ...
            {title:WST.lang('op'), name:'' ,width:60, align:'center', renderer: function(val,item,rowIndex){
                var h="";
	            if(WST.GRANT.RZLC_02)h += "<a  class='btn btn-blue' href='javascript:toEditFlow(" + item['flowId'] + ")'><i class='fa fa-search'></i>"+WST.lang("view")+"</a> ";
	            if(WST.GRANT.RZLC_02)h += "<a  class='btn btn-blue' href='javascript:getForEdit(" + item['flowId'] + ")'><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";

	            if(WST.GRANT.RZLC_03 && item['isDelete'] == 1)h += "<a  class='btn btn-red' href='javascript:toDel(" + item['flowId'] + ")'><i class='fa fa-trash-o'></i>"+WST.lang('del')+"</a> ";
	            return h;
            }}
            ];
