...
function initGridMsg(p){
    var cols = [
            {title:WST.lang('label_template_msg_code'), name:'tplCode', width: 80},
            {title:WST.lang('label_template_msg_content'), name:'tplContent' ,width:600},
            {title:WST.lang('label_template_msg_is_show'), name:'status',align:'center',width:50,renderer: function(val,item,rowIndex){
            	return '<input type="checkbox" '+((item['status']==1)?"checked":"")+' name="isShow2" lay-skin="switch" lay-filter="isShow2" data="'+item['id']+'" lay-text="'+WST.lang('template_msg_is_show_type1')+'|'+WST.lang('template_msg_is_show_type2')+'">';

            }},
            {title:WST.lang('op'), name:'' ,width:20, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
                if(WST.GRANT.XXMB_02)h += "<a  class='btn btn-blue' onclick='javascript:toEditMsg("+item['id']+")'><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";
                return h;
            }}
            ];
...
function initGridEmail(p){
    var cols = [
            ...
            {title:WST.lang('label_template_msg_content'), name:'tplContent' ,width:600},
            {title:WST.lang('label_template_msg_is_show'), name:'status',align:'center',width:50,renderer: function(val,item,rowIndex){
            	return '<input type="checkbox" '+((item['status']==1)?"checked":"")+' name="isShow3" lay-skin="switch" lay-filter="isShow3" data="'+item['id']+'" lay-text="'+WST.lang('template_msg_is_show_type1')+'|'+WST.lang('template_msg_is_show_type2')+'">';

            }},
            {title:WST.lang('op'), name:'' ,width:20, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
                if(WST.GRANT.XXMB_02)h += "<a  class='btn btn-blue' onclick='javascript:toEditEmail("+item['id']+")'><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";
                return h;
            }}
            ];
...
function initGridSMS(p){
    var cols = [
            ...
            {title:WST.lang('label_template_msg_content'), name:'tplContent' ,width:600},
            {title:WST.lang('label_template_msg_is_show'), name:'status',align:'center',width:50,renderer: function(val,item,rowIndex){
            	return '<input type="checkbox" '+((item['status']==1)?"checked":"")+' name="isShow4" lay-skin="switch" lay-filter="isShow4" data="'+item['id']+'" lay-text="'+WST.lang('template_msg_is_show_type1')+'|'+WST.lang('template_msg_is_show_type2')+'">';

            }},
            {title:WST.lang('op'), name:'' ,width:20, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
                if(WST.GRANT.XXMB_02)h += "<a  class='btn btn-blue' onclick='javascript:toEditSMS("+item['id']+")'><i class='fa fa-pencil'></i>"+WST.lang('edit')+"</a> ";
                return h;
            }}
            ];
