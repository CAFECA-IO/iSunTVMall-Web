...
function initRefundGrid(p){
    ...
    var cols = [
            ...
            {title:WST.lang('label_finance_user_type3'), name:'supplierName',sortable: true},
...
function toRefund(id,sId){
	...
	$.post(WST.U('admin/supplierorderrefunds/toRefund',{id:id,serviceId:sId}),{},function(data){
		layer.close(ll);
		w =WST.open({type: 1,title:WST.lang('label_orderrefunds_refund_title1'),shade: [0.6, '#000'],offset:'50px',border: [0],content:data,area: ['650px', '550px']});
	});
}
...
function showDetail(id){
    parent.showBox({title:WST.lang('label_orderrefunds_order_view'),type:2,content:WST.U('admin/supplierorders/view',{id:id,from:1}),area: ['1020px', '500px'],btn:[WST.lang('close')]});
}
function toExport(){
	var params = {};
