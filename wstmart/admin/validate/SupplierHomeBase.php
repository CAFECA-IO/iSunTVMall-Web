 ...
 * 商家入驻字段验证器
 */
class SupplierHomeBase extends Validate{
    public function setRuleAndMessage($data){
        // 先过滤默认的验证规则
        unset($data['investmentStaff'],$data['invoiceRemarks']);
        $checkField = [];
        foreach($data as $k => $v){
            $field = Db::name('supplier_bases')->where(['fieldName'=>$k,'dataFlag'=>1])->find();
            if($field['isRequire']==1){
                ...
                switch($field['fieldType']){
                    ...
                    case 'radio':
                        ...
                        $this->message[$k.'.in'] = lang('supplierHomeBase_validate1', [$field['fieldTitle']]);
                        break;
                    case 'checkbox':
                        if($field['fieldAttr']!='custom'){
                            ...
                            $fieldAttrStr = implode(',',$fieldAttrArray);
                            $this->rule[$k] = 'in:'.$fieldAttrStr;
                            $this->message[$k.'.in'] = lang('supplierHomeBase_validate2', [$field['fieldTitle']]);
                        }else{
                            $this->rule[$k] = 'require';
                            $this->message[$k.'.require'] = lang('select').$field['fieldTitle'];
                        }
