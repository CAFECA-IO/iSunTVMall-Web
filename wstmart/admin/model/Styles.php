...
class Styles extends Base{
	...
	public function changeStyle(){
		 ...
		 $hasFile = file_exists($sqlPath);
		 if(!$hasFile)return WSTReturn(lang('styles_err1'));
		 $sql = file_get_contents($sqlPath);

		 $positionArr = ['home'=>1,'wechat'=>2,'mobile'=>3];
		 if(!isset($positionArr[$object['styleSys']]))return WSTReturn(lang('styles_err2'));
         ...
         try{
			 ...
			 Db::execute($delAdPosition);
         	 ...
         	 if($flag===false)throw new \Exception(lang('styles_err3'), 1);
