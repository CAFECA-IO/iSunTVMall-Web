...
class Menus extends Base{
	...
	public function listQuery($parentId = -1){
		...
		$rs = $this->alias('a')->join('__MENUS_LANGS__ ml','ml.menuId=a.menuId and langId='.wSTCurrLang())->where([['parentId','=',$parentId],['dataFlag','=',1]])->field('a.menuId id,menuName name')->order('menuSort', 'asc')->select();
		if(count($rs)>0){
			foreach ($rs as $key =>$v){
	...
	public function getById($id){
		$rs = $this->where([['dataFlag','=',1],['menuId','=',$id]])->find();
		$rs['langParams'] = Db::name('menus_langs')->where(['menuId'=>$id])->column('*','langId');
		return $rs;
	}
	...
	public function getMenus(){
		...
		$datas  = [];
		$dbo = $this->alias('m')
		            ->join('__MENUS_LANGS__ ml','ml.menuId=m.menuId and langId='.wSTCurrLang())
		...
		$menus = $dbo->field('m.menuId,menuName,privilegeUrl,m.parentId,m.menuIcon')
			->order('menuSort', 'asc')
			->select()->toArray();
		foreach ($menus as $key =>$v){
			$menus[$key]['menuName'] = $v['menuName'];
		}
		if(!empty($menus)){
