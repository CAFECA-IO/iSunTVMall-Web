...
class SupplierOrderRefunds extends Base{
	...
	public function complateOrderRefund($obj){
        ...
        try{
			...
			if(!$isOrderService && $supplierMoneys>0){
				...
				$lm['dataSrc'] = 1;
				$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'supplierOrderRefunds_db2', 'params'=>[$order->orderNo, $supplierMoneys]]);
				$lm['moneyType'] = 1;
	...
	public function saveOrderRefund($refund,$order){
        ...
        try{
			...
			if($refund->backMoney>0){
				...
				$lm['dataId'] = $order->orderId;
				$lm['dataSrc'] = 1;
				$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'orderRefunds_db5', [$order->orderNo,$refund->backMoney,(($content!='')?$content:'')]]);
				$lm['moneyType'] = 1;
	...
	public function toExport(){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', lang('label_order_order_no'))->setCellValue('B1', lang('label_orderrefunds_apply_user'))->setCellValue('C1', lang('label_finance_user_type3'))->setCellValue('D1', lang('label_ordercomplains_src'))->setCellValue('E1', lang('label_orderrefunds_delevery'))
		...
		->setCellValue('K1', lang('label_orderrefunds_refund_money1'))->setCellValue('L1', lang('brand_apply_shop_time'))->setCellValue('M1', lang('label_orderrefunds_refund_type'))->setCellValue('N1', lang('label_order_refund_remarks'));
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->setCellValue('L'.$i2, $page[$row]['createTime'])->setCellValue('M'.$i2, ($page[$row]['isRefund']==1)?lang('refunded'):lang('label_orderrefunds_refund_type0'))->setCellValue('N'.$i2, $page[$row]['refundRemark']);
