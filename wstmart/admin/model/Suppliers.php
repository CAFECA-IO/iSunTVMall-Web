...
class Suppliers extends Base{
	...
	public function pageQuery($supplierStatus=1){
        ...
        $page = Db::table('__SUPPLIERS__')->alias('s')
			       ...
			       ->where($where)
			       ->field('u.loginName,s.supplierId,supplierSn,supplierName,tl.tradeName,a2.areaName,supplierkeeper,telephone,supplierAddress,supplierCompany,supplierAtive,supplierStatus,expireDate')
			       ->order($order)
	...
	public function pageQueryByApply(){
		...
		return Db::table('__SUPPLIERS__')->alias('s')
		       ...
		       ->join('__USERS__ u','u.userId=s.userId','left')
		       ->where($where)
		       ->field('u.loginName,s.supplierId,applyLinkMan,applyLinkTel,investmentStaff,isInvestment,supplierName,tl.tradeName,a2.areaName,supplierAddress,supplierCompany,applyTime,applyStatus')
    ...
    public function del(){
        ...
        try{
            ...
            if(false !== $result){
            	...
            	//删除用户供货商身份
        	    Db::name('users')->where(['userId'=>$supplier->userId])->update(['userType'=>0]);
        	    //删除供货商角色
	...
	public function handleApply(){
		...
		if(empty($suppliers))return WSTReturn(lang('op_err'));
		if($suppliers->applyStatus==2)return WSTReturn(lang('shops_err2'),1);
        //新增入驻申请
        ...
        if($data['applyStatus']==2){
        	...
        	if($tuser['userType']>0){
        		return WSTReturn(lang('suppliers_err1'));
        	}
        }
        Db::startTrans();
        try{
            ...
            if($data['supplierSn']==''){
            	...
            	if(!$this->checkSupplierSn($data['supplierSn'],$supplierId)){
                    ...
                    return WSTReturn(lang('suppliers_err2'));
            	}
            }
            ...
            if($data['applyStatus']==2){
                ...
                $refundRes = '';
                // 用户还未支付
                if($suppliers->isPay==0)return WSTReturn(lang('op_ok'),1);
                // 审核不通过，将年费退款给用户
                if($suppliers->isRefund==1)return WSTReturn(lang('op_ok'));
	...
	public function delApply(){
	    $id = input('post.id/d');
	    $supplier = $this->get($id);
	    if($supplier->applyStatus==2)return WSTReturn(lang('suppliers_err3'));
		Db::startTrans();
        try{
            //删除供货商信息
            Db::name('cat_suppliers')->where(['supplierId'=>$id])->delete();
	...
	public function add(){
        ...
        $accredIds = input('post.accredIds');
        if($goodsCatIds=='')return WSTReturn(lang('shops_select1'));
        if(input('expireDate')=='')return WSTReturn(lang('suppliers_select1'));
        ...
        try{
        	...
        	if($isNewUser==1){
		    	...
		    	if(empty($supplierUser))return WSTReturn(lang('shops_err7'));
		    	if($supplierUser['userType']>0 && $supplierUser['userType']!=3)return WSTReturn(lang('suppliers_err5'));
		    	$tmpSupplier = $this->where(['dataFlag'=>1,'userId'=>$userId])->find();
		    	if(!empty($tmpSupplier))return WSTReturn(lang('suppliers_err4'));
	...
	public function edit(){
		...
		$suppliers = $this->get($supplierId);
		if(empty($suppliers) || $suppliers->dataFlag!=1)return WSTReturn(lang('suppliers_err6'));
		//先遍历前台传来的data,根据supplier_base表判断是属于suppliers表还是supplier_extras表，分别用两个数组保存
        $suppliersData = [];
        $supplierExtrasData = [];
        // 保存上传图片的路径，用来启用上传图片
        ...
        try{
            ...
            if($suppliersData['supplierStatus']==-1){
            	...
            	if($suppliersData['statusDesc']=='')return WSTReturn(lang('shops_err11'));
            }
            $supplierExtrasData['supplierId'] = $supplierId;
            if(!$validate->scene('add')->check($data))return WSTReturn($validate->getError());
	        WSTUnset($data,'id,supplierId,userId,dataFlag,createTime,goodsCatIds,accredIds');
	...
	*/
    ...
    public function toExportRenewMoney(){
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('A2', lang('supp_settlement_name'))
