...
class CronJobs extends Base{
	...
	public function autoByAdmin(){
        ...
        $this->autoHandleShopRefund();
        $this->autoHandleSupplierRefund();
	}
	/**
	 * 取消未支付订单
	 */
		    ...
		    try{
		    	...
		    	foreach ($rs as $okey => $order){;
		    	    ...
		    	    foreach($ordergoods as $goods){
						...
						$data['serviceScore'] = 5;
						$data['timeScore']= 5;
						$data['content'] = WSTLang('cron_jobs_tips2');
					...
					if($isFinish){
						if(WSTConf("CONF.isAppraisesScore")==1){
							...
							if($appraisesScore>0){
								...
								$score['dataRemarks'] = json_encode(['type'=>'lang_params','key'=>'cron_jobs_db1', 'params'=>[$order->orderNo,$appraisesScore]]);
								$score['scoreType'] = 1;
						...
						model('orders')->where('orderId',$order->orderId)->update(['isAppraise'=>1]);
					}
				}
		        Db::commit();
				return WSTReturn(lang('op_ok'),1);
	...
	public function autoReceive(){
	 	...
	 	$rs = model('orders')->where([['deliveryTime','<',$lastDay],['orderStatus','=',1],['dataFlag','=',1]])->field("orderId,orderNo,shopId,userId,shopId,orderScore,commissionFee,realTotalMoney,createTime")->select();
	 	if(!empty($rs)){
	 		$prefix = config('database.prefix');
	 		Db::startTrans();
		    try{
				...
				$afterSaleEndTime = date('Y-m-d H:i:s', strtotime("+{$limitDay} day"));
		 		foreach ($rs as $key => $order){
		 			...
		 			if(false !== $rsStatus){
						...
						$logOrder['logJson'] = json_encode(['type'=>'lang','key'=>'cron_jobs_db2']);
						$logOrder['logUserId'] = $order->userId;
						$logOrder['logType'] = 0;
						$logOrder['logTime'] = date('Y-m-d H:i:s');
						Db::name('log_orders')->insert($logOrder);
                        ...
                        $shopMember = model('common/ShopMembers')->checkIsShopMember($order->userId,$order["shopId"]);
                        if($shopMember){
                            // 计算用户在店铺有效的消费金额
                            WSTCalcUserValidConsumption($order->userId,$order["shopId"]);
                            // 更新用户的会员等级
                            $groupId = WSTGetMemberGroupId($order->userId,$order['shopId']);
                            if($groupId>0){
                                $member = Db::name('shop_members')->where([$order->userId,$order['shopId']])->find();
                                if($member['groupId'] != $groupId){
                                    Db::name('shop_members')->where([$order->userId,$order['shopId']])->update(['groupId'=>$groupId]);
                                }
                            }
                        }
						//发送一条商家信息
						...
						if(WSTConf("CONF.isOrderScore")==1){
							...
							$score['dataSrc'] = 1;
							$score['dataId'] = $order->orderId;
							$score['dataRemarks'] = json_encode(['type'=>'lang_params','key'=>'cron_jobs_db3', 'params'=>[$order->orderNo, $order->orderScore]]);
							$score['scoreType'] = 1;
							$score['createTime'] = date('Y-m-d H:i:s');
	...
	public function autoShopSettlement(){
		...
		return WSTReturn(lang('successful_settlement'),1);
	}
	/**
	 * 清除海报图片
	 * @return [type] [description]
				...
				while (($file=readdir($dh))!==false) {
					if($file!="." && $file!="..") {
						...
						if ($ctime - $vtime>86400){//清除1天前的文件
							if(is_file($vpath."/".$file)){
								unlink($vpath."/".$file);
							}else{
								WSTDelDir($vpath."/".$file);
							}
						}
				    }
				}
    ...
    public function autoHandleShopRefund(){
        $shopHandleRefundDays = (int)WSTConf('CONF.shopHandleRefundDays');
        $shopHandleRefundDays = ($shopHandleRefundDays>0)?$shopHandleRefundDays:3;//避免有些客户没有设置值
        $lastDay = date("Y-m-d 00:00:00",strtotime("-".$shopHandleRefundDays." days"));
        $rs = Db::name('orders')
            ->alias('o')
            ->join('__ORDER_REFUNDS__ orf','orf.orderId=o.orderId','left')
            ->where([['o.orderStatus','in',[-1,-3]],['o.isRefund','=',0],['orf.createTime','<',$lastDay],['orf.refundStatus','=',0]])->field("o.orderId,o.orderNo,o.useScore")->select();
        if(!empty($rs)){
            Db::startTrans();
            try{
                foreach ($rs as $key => $order){;
                    Db::name('order_refunds')->where(['orderId'=>$order['orderId']])->update(['refundStatus'=>1]);
                    $object = Db::name('order_refunds')->where(['orderId'=>$order['orderId']])->find();
                    //判断是否需要发送管理员短信
    ...
    public function autoHandleSupplierRefund(){
        $supplierHandleRefundDays = (int)WSTConf('CONF.supplierHandleRefundDays');
        $supplierHandleRefundDays = ($supplierHandleRefundDays>0)?$supplierHandleRefundDays:3;//避免有些客户没有设置值
        $lastDay = date("Y-m-d 00:00:00",strtotime("-".$supplierHandleRefundDays." days"));
        $rs = Db::name('supplier_orders')
            ->alias('o')
            ->join('__SUPPLIER_ORDER_REFUNDS__ sorf','sorf.orderId=o.orderId','left')
            ->where([['o.orderStatus','in',[-1,-3]],['o.isRefund','=',0],['sorf.createTime','<',$lastDay],['sorf.refundStatus','=',0]])->field("o.orderId,o.orderNo")->select();
        if(!empty($rs)){
            Db::startTrans();
            try{
                foreach ($rs as $key => $order){;
                    Db::name('supplier_order_refunds')->where(['orderId'=>$order['orderId']])->update(['refundStatus'=>1]);
                    $object = Db::name('supplier_order_refunds')->where(['orderId'=>$order['orderId']])->find();
                    //判断是否需要发送管理员短信
                    $tpl = WSTMsgTemplates('PHONE_ADMIN_REFUND_ORDER');
                    if((int)WSTConf('CONF.smsOpen')==1 && (int)WSTConf('CONF.smsRefundOrderTip')==1 &&  $tpl['tplContent']!='' && $tpl['status']=='1'){
                        $params = ['tpl'=>$tpl,'params'=>['ORDER_NO'=>$order['orderNo']]];
