...
class Informs extends Base{
	...
	public function pageQuery(){
		...
		$rs = Db::name('informs')->alias('o')
		                      ->field('o.*,s.shopId,s.shopName,u.userName,u.loginName,oc.goodsImg,oc.goodsId,gl.goodsName')
                              ...
                              ->join('__GOODS_LANGS__ gl','gl.goodsId=oc.goodsId and gl.langId='.WSTCurrLang())
						      ->where($where)
						      ->order('informId desc')
						      ->paginate()
						      ->toArray();
	 ...
	 */
	 ...
	 public function finalHandle(){
	 	$rd = array('status'=>-1,'msg'=>lang('informs_err1'));
	 	$informId = (int)Input('cid');
	 	$finalResult = Input('finalResult');
	 	$informStatus = Input('informStatus');
	 	if($informId==0){
	 		return WSTReturn(lang('informs_err1'),-1);
	 	}
	 	//判断是否已经处理过了
	    ...
	    if($informStatus == 3){
	    	 try{
	 	        ...
	 	        if($ers!==false){
					//发站内用户信息提醒
		 	    	WSTSendMsg($rs['informTargetId'],lang('informs_err2'),['from'=>3,'dataId'=>$informId]);
					Db::commit();
					return WSTReturn(lang('op_ok'),2);
	 	        }
	 	    }catch(\Exception $e){
	 	    	Db::rollback();
	 	...
	 	if($rs['informStatus']!=1 && $rs['informStatus']!=2){
		    ...
		    try{
	 	        ...
	 	        if($ers!==false){
					...
					//发站内用户信息提醒
		 	    	WSTSendMsg($rs['informTargetId'],lang('informs_err4'),['from'=>3,'dataId'=>$informId]);
					Db::commit();
	 	    ...
	 	    return WSTReturn(lang('informs_err3'),-1);
	 	}
