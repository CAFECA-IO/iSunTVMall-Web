...
class SpecCats extends Base{
	...
	public function add(){
		...
		if($data['isAllowImg']==1){
			if($this->checkExistAllowImg((int)$data['goodsCatId'],0)){
				return WSTReturn(lang('specCats_err1'));
			}
		}
		...
		$result = $this->allowField(['isShow','isAllowImg','goodsCatPath','goodsCatId','dataFlag','createTime'])->save($data);
        if(false !== $result){
            $catId = $this->catId;
            $datas = [];
	...
	public function edit(){
	    ...
	    $result = $this->allowField(['goodsCatPath','goodsCatId','isShow','isAllowImg'])->save($data,['catId'=>$catId,"dataFlag"=>1,"shopId"=>0]);
        if(false !== $result){
    ...
    public function del(){
	    $catId = input('post.catId/d');
	    $data["dataFlag"] = -1;
	  	$result = $this->save($data,['catId'=>$catId,"shopId"=>0]);
        if(false !== $result){
        	return WSTReturn(lang('op_ok'), 1);
        }else{
        	return WSTReturn($this->getError(),-1);
	...
	public function setToggle(){
		...
		$result = $this->where(['catId'=>$catId,"dataFlag"=>1,"shopId"=>0])->setField("isShow", $isShow);
		if(false !== $result){
			return WSTReturn(lang('op_ok'), 1);
		}else{
			return WSTReturn($this->getError(),-1);
	...
	public function pageQuery(){
		...
		$dbo = $this
                ...
                ->field("scl.catName, a.catId id, isShow ,isAllowImg,goodsCatPath")->where($map);
