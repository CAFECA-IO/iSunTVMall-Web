...
class Shops extends Base{
	...
	public function pageQuery($shopStatus=1){
		...
		$page =  Db::table('__SHOPS__')->alias('s')
			       ...
			       ->where($where)
			       ->field('u.loginName,s.shopId,shopSn,shopName,tl.tradeName,a2.areaName,shopkeeper,telephone,shopAddress,shopCompany,shopAtive,shopStatus,expireDate')
			       ->order($order)
	...
	public function pageQueryByApply(){
        ...
        return Db::table('__SHOPS__')->alias('s')
               ...
               ->join('__USERS__ u','u.userId=s.userId','left')
               ->where($where)
               ->field('u.loginName,s.shopId,applyLinkMan,applyLinkTel,applyLinkTelAreaCode,investmentStaff,isInvestment,shopName,tl.tradeName,a2.areaName,shopAddress,shopCompany,applyTime,applyStatus')
    ...
    public function del(){
	    $id = input('post.id/d');
	    if($id==1)return WSTReturn(lang('shops_err1'));
		Db::startTrans();
        try{
	        $shop = $this->get($id);
	        $shop->dataFlag = -1;
            ...
            if(false !== $result){
            	...
            	//删除用户店铺身份
        	    Db::name('users')->where(['userId'=>$shop->userId])->update(['userType'=>0]);
        	    //删除店铺角色
	...
	public function delByUserId($userId){
		if($userId==1)return WSTReturn(lang('shops_err3'));
	...
	public function handleApply(){
		...
		if(empty($shops))return WSTReturn(lang('op_err'));
		if($shops->applyStatus==2)return WSTReturn(lang('shops_err2'),1);
        //新增入驻申请
        ...
        try{
            ...
            if($data['shopSn']==''){
            	...
            	if(!$this->checkShopSn($data['shopSn'],$shopId)){
                    ...
                    return WSTReturn(lang('the_store_number_already_exists'));
            	}
            }
            ...
            if($data['applyStatus']==2){
                ...
                $refundRes = '';
                // 用户还未支付
                if($shops->isPay==0)return WSTReturn(lang('op_ok'),1);
                // 审核不通过，将年费退款给用户
                if($shops->isRefund==1)return WSTReturn(lang('op_ok'));
	...
	public function sendMessages($shopId,$userId,$data,$method){
        ...
        if((int)$data['applyStatus']==2){
            ...
            if( $tpl['tplContent']!='' && $tpl['status']=='1' && $data['applyLinkTel']!=''){
                ...
                $rv = model('admin/LogSms')->sendSMS(0,$userId,$data['applyLinkTelAreaCode'].$data['applyLinkTel'],$params,$method);
            }
            //发送邮件
            ...
            if( $tpl['tplContent']!='' && $tpl['status']=='1' && $data['applyLinkEmail']){
                ...
                $sendRs = WSTSendMail($data['applyLinkEmail'],lang('shopApplys_ok2'),str_replace($find,$replace,$tpl['content']));
            }
            // 会员发送一条商城消息
            ...
            if((int)WSTConf('CONF.wxenabled')==1){
                ...
                $params['REASON'] = lang('shopApplys_ok1');
            ...
            if( $tpl['tplContent']!='' && $tpl['status']=='1' && $data['applyLinkEmail']){
                ...
                $sendRs = WSTSendMail($data['applyLinkEmail'],lang('shopApplys_err1'),str_replace($find,$replace,$tpl['content']));
            }
            // 会员发送一条商城消息
	...
	public function delApply(){
	    $id = input('post.id/d');
	    $shop = $this->get($id);
	    if($shop->applyStatus==2)return WSTReturn(lang('shops_err4'));
		Db::startTrans();
        try{
            //删除店铺信息
            Db::name('cat_shops')->where(['shopId'=>$id])->delete();
	...
	public function add(){
        ...
        $goodsCats = [];
        foreach($data as $k => $v){
            $field = Db::name('shop_bases')->where(['fieldName'=>$k,'dataFlag'=>1])->field('fieldName,fieldType,fieldAttr,isShopsTable,dateRelevance,isShow,isRequire,isPhone')->find();
            if($field['isShopsTable']==1){
                // 属于shops表
                $shopsData[$k] = $v;
                //获取地区
                ...
                if($field['isPhone']==1){
                    $shopsData[$field['fieldName']."AreaCode"] = input($field['fieldName']."AreaCode");
                }
            }else{
                // 属于shop_extras表
                $shopExtrasData[$k] = $v;
        ...
        try{
            ...
            if($isNewUser==1){
                ...
                if(empty($shopUser))return WSTReturn(lang('shops_err7'));
                if($shopUser['userType']>0 && $shopUser['userType']!=1)return WSTReturn($shopUser['userType'].lang('shops_err8'));
                $tmpShop = $this->where(['dataFlag'=>1,'userId'=>$userId])->find();
                if(!empty($tmpShop))return WSTReturn(lang('shops_err9'));
                $shopUser->userType = 1;
                $shopUser->save();
            }
            if($userId>0){
	...
	public function edit(){
        ...
        $shops = $this->get($shopId);
        if(empty($shops) || $shops->dataFlag!=1)return WSTReturn(lang('shops_err10'));
        //先遍历前台传来的data,根据shop_base表判断是属于shops表还是shop_extras表，分别用两个数组保存
        $shopsData = [];
        $shopExtrasData = [];
        // 保存上传图片的路径，用来启用上传图片
        ...
        try{
            ...
            if($shopsData['shopStatus']==-1){
                ...
                if($shopsData['statusDesc']=='')return WSTReturn(lang('shops_err11'));
            }
	...
	*/
    ...
    public function renewMoneyByPage(){
        ...
        if(count($page['data'])>0){
            foreach ($page['data'] as $key => $v) {
                ...
                $page['data'][$key]['remark'] = WSTLogJson($v['remark']);
            }
        }
        return $page;
    }
    ...
    public function toExportRenewMoney(){
        ...
        ->setKeywords(lang('shops_excel1'));//种类
        // 开始操作excel表
        ...
        $objPHPExcel->getActiveSheet()
            ->setCellValue('G1', lang('total_payment_amount').':'.$totalRenewMoney.lang('unit'))
            ->setCellValue('A2', lang('brand_apply_shop_name'))
            ->setCellValue('B2', lang('label_finance_payment_desc'))
            ->setCellValue('C2', lang('label_finance_year_money'))
            ->setCellValue('D2', lang('label_order_outer_no'))
            ->setCellValue('E2', lang('label_finance_payment_time'))
            ->setCellValue('F2', lang('start_date'))
            ->setCellValue('G2', lang('end_date'));
        ...
        for ($row = 0; $row < count($rs); $row++){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('B'.$i, ($rs[$row]['isRefund']==1)?$rs[$row]['remark'].'('.lang('label_orderrefunds_refund_type1').')':WSTLogJson($rs[$row]['remark']))
                ->setCellValue('C'.$i, lang('currency_symbol').$rs[$row]['money'])
