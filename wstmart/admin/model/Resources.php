...
class Resources extends Base{
	...
	public function summary(){
		...
		//获取目录名称
		$rs2 = Db::name('datas d')->join('__DATAS_LANGS__ dl','dl.dataId=d.id and langId='.wSTCurrLang())->where(['catId'=>3])->field('dataName,dataVal')->select();
		$imagesMap = [];
    ...
    public function getPicInfo(){
        ...
        $rs = Db::name('resources')->where(['dataFlag'=>1,'isUse'=>1,'fromTable'=>$fromTable])->field('resId')
            ->select();
        return WSTReturn(lang('resouces_return1', [count($rs)]), 1,$rs);
    }
    /*
     * 图片处理
     */
    public function picHandle(){
        $type = (int)input('type');
        if(!in_array($type,[1,2]))return WSTReturn(lang('resouces_err1'), -1);
        $id = (int)input('id');
