...
class ShopFlows extends Base{
    ...
    public function saveField(){
        ...
        try{
            ...
            if($data['id']!=0){
                ...
                if($old['isShopsTable'] == 0 && $old['isDelete'] == 0){
                    ...
                    if($fieldName != $old['fieldName']){
                        $sql = "alter table `".$prefix."shop_extras` change {$old['fieldName']} {$fieldName} {$dataAttr} {$default}";
                        Db::query($sql);
                    }else{
                        $sql = "alter table `".$prefix."shop_extras` modify {$fieldName} {$dataType}{$dataAttr} {$default}";
                        Db::query($sql);
                    }
                }
                ...
                $fields = Db::query('show columns from '.$prefix.'shop_extras');
                $fieldArray = [];
                foreach($fields as $v){
                    $fieldArray[] = $v['Field'];
                }
                ...
                if(!in_array($fieldName,$fieldArray)){
                    $sql = "alter table `".$prefix."shop_extras` add column {$fieldName} {$dataType}{$dataAttr} {$default}";
                    Db::query($sql);
                }else{
                    return WSTReturn(lang('shopFlows_return1'),-1);
                }
                $array['flowId'] = $flowId;
            ...
            Db::rollback();
            return WSTReturn(lang('op_err').$e->getMessage().$e->getLine(),-1);
        }
    }
    ...
    public function delField(){
        ...
        try{
            ...
            if(in_array($fieldName,$fieldArray)){
                $sql = "alter table `".$prefix."shop_extras` drop column {$fieldName}";
                Db::query($sql);
            }
            if(false !== $result){
                Db::commit();
