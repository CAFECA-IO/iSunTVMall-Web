	...
	public function pageQuery(){
        ...
        if(count($rs['data'])>0) {
            foreach ($rs['data'] as $key => $val) {
                ...
                $rs['data'][$key]['feedbackStatusName'] = $val['feedbackStatus']==0?lang('label_feedback_status1'):lang('label_feedback_status2');
                if($rs['data'][$key]['userId'] != 0){
                    $rs['data'][$key]['userName'] = Db::name('users')->where('userId','=',$rs['data'][$key]['userId'])->value('userName');
                }else{
                    $rs['data'][$key]['userName'] = lang('label_feedback_user');
                }
            }
        }
		return $rs;
    ...
    public function getById($feedbackId){
        ...
        if($rs){
            if($rs['userId'] != 0){
                ...
                $rs['userName'] = lang('label_feedback_user');
            }
