...
class UserScores extends Base{
    ...
    public function addByAdmin(){
        ...
        if(empty($user))return WSTReturn(lang('logMoneys_err1'));
        if(!in_array($data['scoreType'],[0,1]))return WSTReturn(lang('userScores_err1'));
        if($data['score']<=0)return WSTReturn(lang('userScores_err2'));
        Db::startTrans();
		try{
