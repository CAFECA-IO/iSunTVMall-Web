	 	...
	 	if(!empty($orders)){
		    ...
		    try{
                ...
                foreach ($orders as $okey => $order){
					...
					$logOrder['logJson'] = json_encode(['type'=>'supplierCronJobs_db1']);
					...
					if($isFinish){
						if(WSTConf("CONF.isAppraisesScore")==1){
							...
							if($appraisesScore>0){
								...
								$score['dataRemarks'] = json_encode(['type'=>'lang_params','key'=>'supplierCronJobs_db2', 'params'=>[$order->orderNo, $appraisesScore]]);
								$score['scoreType'] = 1;
