...
class LogMoneys extends Base{
    ...
    public function addByAdmin(){
        ...
        if(empty($rs))return WSTReturn(lang('logMoneys_err1'));
        if(!in_array($data['moneyType'],[0,1]))return WSTReturn(lang('logMoneys_err2'));
        if($data['money']<0.01)return WSTReturn(lang('logMoneys_err3'));
        Db::startTrans();
		try{
