	...
	public function sendMessages($id){
	    ...
	    if((int)$data['applyStatus']==1){
		    ...
		    if( $tpl['tplContent']!='' && $tpl['status']=='1' && $data['userEmail']){
		        ...
		        $sendRs = WSTSendMail($data['userEmail'],lang('shopApplys_ok2'),str_replace($find,$replace,$tpl['content']));
		    }
		    // 会员发送一条商城消息
		    ...
		    if( $tpl['tplContent']!='' && $tpl['status']=='1' && $data['userEmail']){
		        ...
		        $sendRs = WSTSendMail($data['userEmail'],lang('shopApplys_err1'),str_replace($find,$replace,$tpl['content']));
		    }
	    	// 会员发送一条商城消息
