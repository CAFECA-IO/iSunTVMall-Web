...
class Orders extends Base{
	...
	*/
	...
	public function getVerificatOrder(){
        ...
        if(empty($rs)){
			return WSTReturn(lang('invalid_verificat_code'));
		}else{
			return WSTReturn("", 1,$rs);
		}
	}
     ...
     * 跳去修改订单收货信息页面
     */
	public function toEditOrderReceiveInfo(){
        $type = input('param.type','all');
        $this->assign('type',$type);
        //获取省级地区信息
        $area = model('areas')->listQuery(0);
        $this->assign('area',$area);
        $this->assign('orderId',(int)input('orderId'));
        return $this->fetch('users/sellerorders/orders_receive');
    }
     ...
     * 跳去修改订单物流信息页面
     */
    public function toEditOrderExpressInfo(){
        $type = input('param.type','all');
        $this->assign('type',$type);
        $this->assign('orderId',(int)input('orderId'));
        $express = model('Express')->listQuery();
        $this->assign('express',$express);
        return $this->fetch('users/sellerorders/orders_express');
    }
