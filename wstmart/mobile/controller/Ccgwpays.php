...
class Ccgwpays extends Base{
	...
	public function initialize() {
		...
		$this->payConfig['notifyurl'] = url("mobile/ccgwpays/ccgwNotify","",true,true);
	...
	public function toCcgwPay() {
		...
		return $this->fetch('users/orders/orders_ccgwpay');
	...
	public function ccgwNotify() {
		...
		$qrcodePay = new \CcgwQrcodePay ();
		$bkData = input();
		file_put_contents("runtime/ccgwpay_mobile_log.txt",date('Y-m-d H:i:s')."----ttbb----".json_encode($bkData)."----\n", FILE_APPEND); 
		$ccTxid = $bkData['txid'];
