...
class Base extends Controller {
	public function __construct(){
		...
		WSTSwitchs();
		$lang = WSTSwitchLang();
		// \think\facade\Lang::load(APP_PATH . 'mobile' . DS . 'lang' . DS . $lang . '.php');
    ...
    protected function checkAuth(){
        ...
        if(empty($USER)){
        	if(request()->isAjax()){
        		die('{"status":-999,"msg":"'.lang('not_login').'"}');
        	}else{
    ...
    protected function checkShopAuth($opt){
       	...
       	if($opt=="list"){
       		if(count($shopMenus)==0){
       			session('moshoporder',lang('no_privilege'));
       		...
       		if(!array_key_exists($opt,$shopMenus)){
	       		if(request()->isAjax()){
		    		die('{"status":-1,"msg":"'.lang('no_privilege').'"}');
		    	}else{
		    		session('moshoporder',lang('no_privilege'));
		    		$this->redirect('mobile/error/message',['code'=>'moshoporder']);
		    		exit;
