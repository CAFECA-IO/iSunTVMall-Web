...
class Users extends Base{
    ...
    protected $beforeActionList = [
          'checkAuth' =>  ['except'=>'index,checklogin,checkloginbyphone,login,register,registerbyaccount,getverify,toregister,forgetpass,forgetpasst,forgetpasss,findpass,getfindphone,resetpass,getphoneverifycode,getphoneverifycode2,checkuserphone,langset']// 访问这些except下的方法不需要执行前置操作
    ];
    /**
     * 会员登录页
     */
    ...
    public function checkLogin(){
        if(strpos(WSTConf('CONF.mobileLoginType'),'1')===false)return WSTReturn(lang('illegal_login'),-1);
    	$m = new M();
    	$rs =  $m->checkLogin(2);
    ...
    public function checkLoginByPhone(){
        if(strpos(WSTConf('CONF.mobileLoginType'),'2')===false)return WSTReturn(lang('illegal_login'),-1);
        $m = new M();
    ...
    public function register(){
        if(strpos(WSTConf('CONF.mobileRegisterType'),'2')===false)return WSTReturn(lang('illegal_registration'),-1);
    	$m = new M();
    	$rs =  $m->regist(2);
    	$rs['url'] = session('WST_MO_WlADDRESS');
    	return $rs;
    ...
    public function registerByAccount(){
        if(strpos(WSTConf('CONF.mobileRegisterType'),'1')===false)return WSTReturn(lang('illegal_registration'),-1);
        $m = new M();
    ...
    public function checkUserPhone(){
    	...
    	if($rs["status"]!=1){
    		return WSTReturn(lang('phone_number_has_regist'),-1);
    	}else{
    		return WSTReturn("",1);
    	}
    }
	...
	public function index(){
		...
		if($userId>0){
		    ...
		    $user['userName'] = lang('consult_tourist');
            $user['userPhone'] = '';
		    $user['userPhoto'] = '';
		    $user['userType'] = 0;
		    $user['userMoney'] = 0;
    ...
    protected function expire($msg=''){
        $this->assign('button',['text'=>lang('re_operation'),'url'=>url('mobile/users/forgetpass')]);
        $this->assign('message',lang('page_is_invalid'));
    ...
    */
    ...
    public function langSet(){
        return $this->fetch('users/userset/lang');
    }
}
