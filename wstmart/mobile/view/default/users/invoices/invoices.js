...
function switchPage(n) {
    ...
    if (n == 1) {
        ...
        $("#invoId").val(0);
        $('.interData').hide();
        $('#header-title').text(WST.lang('invoice_add'));
    } else if (n == 2) {
        $('.invo-js1').hide();
        $('.invo-js2').show();
        $("#invoId").val(0);
...
function getInvoice(id, invoiceType) {
    switchPage(1);
    $('#header-title').text(WST.lang('edit_invoice_information'));
    $("#invoId").val(id);
    ...
    $.post(WST.U('mobile/invoices/get'), {'id': id}, function (data, textStatus) {
        ...
        if (json.status == 1) {
            ...
            WST.msg(WST.lang('sys_err_page_title'), 'error');
        }
    });
}
...
function delInvoice(id) {
    WST.dialog(WST.lang('confirm_del_inquiry'),'del('+id+')');
}
