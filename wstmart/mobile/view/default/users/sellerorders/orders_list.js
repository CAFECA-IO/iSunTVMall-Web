...
function toEditOrderReceiveInfo(orderId){
    var type = $('#type').val();
    location.href=WST.U('mobile/orders/toEditOrderReceiveInfo',{'orderId':orderId,'type':type});
}

// 修改物流信息
function toEditOrderExpressInfo(orderId){
    var type = $('#type').val();
    location.href=WST.U('mobile/orders/toEditOrderExpressInfo',{'orderId':orderId,'type':type});
}
...
function showRefundBox(id){
    $.post(WST.U('mobile/orders/toShopRefund'),{id:id},function(data){
           ...
           $('#refundMoney').html(WST.lang('currency_symbol')+json.backMoney);
           $('#useScore').html(json.useScore);
           $('#scoreMoney').html(WST.lang('currency_symbol')+json.scoreMoney);
           // 弹出层滚动条
    ...
    if(type==0){
        if(delivery_type == 1){
            if(expressId == '' || expressNo == ''){
                WST.msg(WST.lang('please_fill_logistics_information'),{icon:2});
                return;
            }
        }
        var selectIds = getSelectIds('.active');
        if(selectIds.length==0){
            WST.msg(WST.lang('please_select_deliver_goods'),{icon:2});
            return;
        }
        params.selectOrderGoodsIds = selectIds.join(',');
        params.deliverType = delivery_type;
    }
    params.id = oid;
