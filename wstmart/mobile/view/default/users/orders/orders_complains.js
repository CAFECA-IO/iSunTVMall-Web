...
function dataHide(type){
	if(type==1){
	    ...
	    if(!flag){
	      WST.msg(WST.lang('require_complain_type'));
	      return;
	    }
...
function saveCom(oId){
  ...
  if(type==''){
    dataShow();
    WST.msg(WST.lang('require_complain_type'),'info');
    return;
  }
  var complainContent = $.trim($('#complain').val());
  if(complainContent==''){
    WST.msg(WST.lang('complain_please_input_content'),'info');
    return;
  }
  var param = {};
  param.orderId = oId;
  ...
  $.post(WST.U('mobile/ordercomplains/saveComplain'),param,function(data){
    var json = WST.toJson(data);
    if(data.status){
      WST.msg(WST.lang('complain_submit_success_tips'),'success');
