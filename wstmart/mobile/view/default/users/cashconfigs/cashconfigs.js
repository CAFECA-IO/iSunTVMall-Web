...
function editAddress(id,accBank){
    ...
    $('#accNo').val('');
    $('#accAreaId').val('');
    $('#addresst').html(WST.lang('require_address'));
    $('#addAddress').attr('onclick','javascript:saveConfig('+id+');');
    $('#addAddress').html(WST.lang('save'));
    if(id>0){
    	$('#header-title').html(WST.lang('update_drawal_account'));
    	...
    	$('#header-title').html(WST.lang('add_account'));
    }
    switchPage(1);
}
...
function switchPage(n){
	if(n==1){
        ...
        $('#addAddress').html(WST.lang('add_account'));
	}
}
...
function saveAccId() {
    ...
    if(!flag){
        WST.msg(WST.lang('require_account_type'));
        return;
    } else {
        dataHide(2)
    }
...
function saveConfig(cId){
    ...
    if(accTargetId==''){
      WST.msg(WST.lang('require_account_type'),'info');
      $('#accTargetId').focus();
        return false;
    }
    ...
    if(accUser==''){
      WST.msg(WST.lang('require_card_holder'),'info');
      $('#accUser').focus();
        return false;
    }
    if(accNo==''){
      WST.msg(WST.lang('require_card_number'),'info');
        return false;
    }
...
//删除提现账号
function del(id){
  WST.dialog(WST.lang('confirm_del_inquiry'),'toDel('+id+')');
}
//删除提现账号
function toDel(id){
