...
function editAddress(addressId){
	...
	$('#areaId').val('');
	$('#addresst').html(WST.lang('please_select_address'));
	$('.wst-ad-submit .button').attr('onclick','javascript:saveAddress('+addressId+');');
    if(addressId>0){
    	$('#header-title').html(WST.lang('update_receiving_address'));
    	...
    	$('#header-title').html(WST.lang('add_address'));
    }
    switchPage(1);
}
...
function delAddress(addressId){
	WST.dialog(WST.lang('confirm_del_inquiry'),'toDelAddress('+addressId+')');
}
//删除收货地址
