...
function chkClickListener(){
  var _obj = $('.ui-icon-chooseg');
  _obj.unbind("click");
  //选中商品
  _obj.click(function(){
...
$(function(){
  goodsServiceType = $("#goodsServiceType").val();
    chkClickListener();
    // 上传图片
...
function choseType(n){
  ...
  var isFixed = ($("#isFixedCombination").val()==1);
  ...
  var sameSt = goodsServiceType==_val;
  ...
  if(_val==2){
      // 换货
      if(isFixed){
        _chkObj.attr("class","ui-icon-chooseg ui-icon-unchecked-s clist os-chk");
        chkClickListener();
      }
      _rmObj.hide();
  }else{
    if (isFixed) {
      _chkObj.attr("class","ui-icon-chooseg ui-icon-success-block clist os-chk wst-active");
      $('.ui-icon-chooseg').unbind("click");
    }
    getCanRefundMoney();
    _rmObj.show();
  }
...
function getCanRefundMoney(){
    ...
    $.post(WST.U('mobile/orderservices/getRefundableMoney'),_glist,function(res){
      ...
      if(json.status==1){
        $('#maxRefundMoney').html(WST.lang('currency_symbol')+json.data.totalMoney);
      }
    });
}
...
function commitOrderService(){
    ...
    if(postData.serviceType==-1){
      return WST.msg(WST.lang('requrie_apply_reason'));
    }
    if(postData.serviceRemark.length==0){
      return WST.msg(WST.lang('requrie_problem_description'));
    }
    if(postData.goodsServiceType!=2 && postData.refundMoney<0){
      return WST.msg(WST.lang('require_order_refund_amount'));
    }
    ...
    if(_gids.length==0){
      return WST.msg(WST.lang('require_order_services_goods_limit'));
    }
