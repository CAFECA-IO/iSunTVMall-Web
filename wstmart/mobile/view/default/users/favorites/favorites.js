  ...
  $('#Load').show();
    ...
    $.post(WST.U('mobile/favorites/listGoodsQuery'), param, function(data){
        ...
        if(json && json.data && json.data.length>0){
          ...
          html += '<div style="text-align:center"><div class="wst-prompt-icon"><img style="width:1.62rem;height:1.27rem" src="'+ window.conf.MOBILE +'/img/nothing-follow.png"></div><div class="wst-prompt-info"><p>'+WST.lang('no_attention')+'</p></div></div>';
          $('#goods-list').html(html);
...
function toCancel(){
  ...
  gids = gids.join(',');
  if(gids==''){
    WST.msg(WST.lang('please_select_goods'),'info');
    return;
  }
  WST.dialog(WST.lang('cancel_attention_confirm_inquiry'),'cancelFavorite()');
}
// 取消关注
