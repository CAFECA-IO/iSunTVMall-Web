...
function checkGoodsBuyStatus(){
	...
	$('.wst-active').each(function(){
		...
		if(stockNum < 0 || stockNum < cartNum){
			if(stockNum < 0){
				msg = WST.lang('cart_under_stock');
			}else{
				msg = WST.lang('cart_buy_over_stock');
			}
...
function deletes(){
    ...
    if(goodsIds!=''){
    	WST.dialog(WST.lang('confirm_del_goods'),'del("'+goodsIds+'")');
    }else{
    	WST.msg(WST.lang('cart_choose_goods'),'info');
    }
}
...
function toFavorites(){
    ...
    if(goodsIds!=''){
        WST.dialog(WST.lang('confirm_move_into_attention',[selectGoods]),'moveToFavorites("'+goodsIds+'","'+cartIds+'")');
    }else{
        WST.msg(WST.lang('select_move_into_attention_goods'),'info');
    }
}
...
function toSettlement(){
    ...
    if(noGoodsSelected){
    	WST.msg(WST.lang('cart_select_settlement_goods'),'info');
        return false;
    }
