...
class Index extends Base{
	...
	public function pageQuery(){
		...
		if($limit>9)return;
		$cacheData = WSTCache('MO_CATS_ADS'.$limit);
		...
		WSTCache('MO_CATS_ADS'.$limit,$rs,86400);
		return $rs;
	}
	...
	*/
	function getSysMsg($msg='',$order='',$follow='',$history=''){
		...
		if($userId>0){
			...
			if($follow!=''){
				$data['follow']['goods'] = Db::name('favorites')->where(['userId'=>$userId])->count();
				$data['follow']['shops'] = Db::name('shop_members')->where(['userId'=>$userId])->count();
			}
		}else{
			$data['message']['num'] = 0;
