...
class Goods extends CGoods{
    ...
    public function pageQuery($goodsCatIds = []){
        ...
        $list = Db::name('goods')->alias('g')
                ...
                ->join("__SHOPS__ s","g.shopId = s.shopId")->join('__GOODS_SCORES__ gs','gs.goodsId=g.goodsId')
                ->where($where)
        ...
        $list = $list->field('g.goodsId,gl.goodsName,saleNum,shopPrice,marketPrice,isSpec,goodsImg,appraiseNum,visitNum,s.shopId,shopName,isSelf,isFreeShipping,gallery,gs.totalScore,gs.totalUsers');
