...
$(function(){
	...
	$('.item-more').click(function(){
		if($(this).attr('v')==1){
			$('.hideItem').show(300);
			$(this).find("span").html(WST.lang('pack_up'));
			$(this).find("i").attr({"class":"drop-up"});
			$(this).attr('v',0);
		}else{
			$('.hideItem').hide(300);
			$(this).find("span").html(WST.lang('more_item'));
...
function checkGoodsStock(){
	...
	$('#j-market-price').html(WST.lang('goods_price_unit')+marketPrice);
...
function addCart(type,iptId){
	...
	if(goodsInfo.isSpec==1){
		...
		if(specIds.length==0){
			WST.msg(WST.lang('choose_goods_spec'),{icon:2});
		}
		specIds.sort(function(a,b){return a-b});
		if(goodsInfo.sku[specIds.join(':')]){
...
function consultCommit(){
	...
	if(!params.consultType){
		WST.msg(WST.lang('select_consult_type'),{icon:2});
		return;
	}
	params.consultContent = $('#consultContent').val();
	if(params.consultContent == ''){
		WST.msg(WST.lang('input_consult_content'),{icon:2});
		return;
	}
	if(params.consultContent.length<3 || params.consultContent.length>200){
		WST.msg(WST.lang('consult_content_msg'),{icon:2});
		return;
	}
      	  ...
      	  if(json.data.data.length>0){
	          ...
	          laytpl(gettpl).render(json.data.data, function(html){
	           ...
	           laypage({
        	    ...
        	    $('#consultBox').html('<div style="margin:20px 45px">'+WST.lang('consult_no_result')+'</div>');
	       }
        }
  });
}
