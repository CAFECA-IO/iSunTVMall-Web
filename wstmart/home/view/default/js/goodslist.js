...
function choiceArea(t,pid){
	...
	if(ids[2]==3){
		...
		// 替换下级地图标题
		$(nextid).html('<a href="javascript:void(0)">'+WST.lang('please_select')+'</a>');
...
function multi_done(obj,type){
	...
	if(ids.length==0){
		WST.msg(WST.lang('select_screen_item'));
		return false;
	}
	if(type=='brand'){
		$('#brand').val(ids.join(','));
...
function choiceContrast(obj,itemId,catId,goodsId){
	...
	$('#goods-price-'+goodsId).html(WST.lang('currency_symbol')+' <span>'+goodsPrice+'</span>');
}