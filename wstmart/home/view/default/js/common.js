...
WST.confirm = function(options){
	var opts = {};
	opts = $.extend(opts, {title:WST.lang('system_prompt'),offset:'200px'}, options);
	...
	try{
		...
		if(typeof(noAlert)=='undefined'){
			if(json.status && json.status=='-999'){
				WST.msg(WST.lang('logout_tips'),{icon:5},function(){
					if(window.parent){
				...
				WST.msg(WST.lang('no_operation_permission_tips'));
				return;
			}
		}
	}catch(e){
...
WST.checkCart = function(){
	$('#list-carts2').html('');
	$('#list-carts3').html('');
	$('#list-carts').html('<div style="padding:32px 0px 77px 112px;"><img src="'+WST.conf.ROOT+'/wstmart/home/view/default/img/loading.gif">'+WST.lang('loading_tips')+'</div>');
	$.post(WST.U('home/carts/getCartInfo'),'',function(data) {
		...
		if(json.status==1){
			...
			if(json.list.length>0){
				...
				$('#list-carts2').html('<div class="comm" id="list-comm">&nbsp;&nbsp;'+WST.lang('items_goods_total_tips',[json.goodsTotalNum])+'<span class="span2">'+WST.lang('goods_price_unit')+json.goodsTotalMoney+'</span></div>');
				$('#list-carts3').html('<a href="'+window.conf.ROOT+'/home/carts/index" class="btn btn-3">'+WST.lang('go_cart_settlement')+'</a>');
				...
				$('#list-carts').html('<p class="carts">'+WST.lang('cart_empty_tips')+'</p>');
			}
...
WST.shopQQ = function(val){
	if(WST.blank(val) !=''){
      return [
              '<a href="tencent://message/?uin='+val+'&Site='+WST.lang('qq_chat')+'&Menu=yes">',
		      '<img border="0" src='+window.conf.HTTP+'wpa.qq.com/pa?p=1:'+val+':7" alt="'+WST.lang('qq_chat')+'" width="71" height="24" />',
		      '</a>'
		      ].join('');
	}else{
		return '';
...
WST.shopWangWang = function(val){
	if(WST.blank(val) !=''){
		return [
		       ...
		       '<img border="0" src='+window.conf.HTTP+'amos.alicdn.com/realonline.aw?v=2&uid='+val+'&site=cntaobao&s=1&charset=utf-8" alt="'+WST.lang('contact_me')+'" />',
	           '</a>'
		       ].join('');
	}else{
		return '';
...
WST.cancelFavorite = function(obj,type,id,fId){
	...
	var param = {},str = WST.lang('goods');
	param.id = fId;
	param.type = type;
	str = (type==1)?WST.lang('shop'):WST.lang('goods');
	$.post(WST.U('home/favorites/cancel'),param,function(data,textStatus){
	    var json = WST.toJson(data);
	    if(json.status=='1'){
	       WST.msg(json.msg,{icon:1});
	       $(obj).removeClass('j-fav').addClass('j-fav2');
	       $(obj).html(WST.lang('follow')+str)[0].onclick = function(){
...
WST.addFavorite = function(obj,type,id,fId){
	...
	$.post(WST.U('home/favorites/add'),{type:type,id:id},function(data,textStatus){
	     ...
	     if(json.status==1){
	    	 ...
	    	 $(obj).html(WST.lang('has_favorited'))[0].onclick = function(){
	...
	if(opts.childIds.length>0){
		...
		$.post(WST.U('home/goodscats/listQuery'),{parentId:opts.val},function(data,textStatus){
		     ...
		     if(json.data && json.data.length>0){
			     ...
			     html.push("<option value=''>"+WST.lang('please_select')+"</option>");
...
WST.inSign = function(){
    ...
    $.post(WST.U('home/userscores/signScore'),{},function(data){
        ...
        if(json.status==1){
            ...
            setInterval(function(){
                $("#j-sign").addClass('actives').html(WST.lang('signed_in'));
            },600);
            WST.msg(json.msg, {icon: 1});
        }else{
            WST.msg(json.msg, {icon: 5});
