		...
		if($('.toolbar-wrap').hasClass('toolbar-open')){
			if($(this).find('.tab-text').length > 0){
				if(! $('.tbar-tab-follow').find('.tab-text').length > 0){
					var info = "<em class='tab-text '>"+WST.lang('my_attention')+"</em>";
					$('.tbar-tab-follow').append(info);
					$('.tbar-tab-follow').removeClass('tbar-tab-click-selected'); 
					$('.tbar-panel-follow').css({'visibility':"hidden","z-index":"-1"});
				}
				if(! $('.tbar-tab-history').find('.tab-text').length > 0){
					var info = "<em class='tab-text '>"+WST.lang('my_tracks')+"</em>";
		...
		if($('.toolbar-wrap').hasClass('toolbar-open')){
			if($(this).find('.tab-text').length > 0){
				if(! $('.tbar-tab-cart').find('.tab-text').length > 0){
					var info = "<em class='tab-text '>"+WST.lang('shopping_cart')+"</em>";
	...
	$.post(WST.U('home/carts/getCart'),'',function(data) {
		...
		if(json.status==1){
			...
			if(json.carts && !json.carts.length){
				...
				$('#cart-panel').html('<p class="right-carts-empty">'+WST.lang('cart_empty_tips')+'</p>');
			}
		}
	});
}
...
function jumpSettlement(){
	if($('#cart-panel').children().size()==0){
		WST.msg(WST.lang('cart_empty_add_goods_tips'),{icon:2});
		return;
	}
