  ...
  function changeType(target,val){
    ...
    var isFixed = ($("#isFixedCombination").val()==1);
     ...
     if(val!=2){
       $('#refundMoneyBox').show();
       if(isFixed){
        $('.os-chk').prop("disabled", true).prop("checked", true);
       }
       getCanRefundMoney();
      }else{
        // 换货
        if(isFixed){
          $('.os-chk').prop("disabled", false);
         }
       $('#refundMoneyBox').hide();
     }
  }
  ...
  function commitOrderService(){
    ...
    if(postData.serviceType==-1){
      return WST.msg(WST.lang('please_input_reasons'));
    }
    if(postData.serviceRemark.length==0){
      return WST.msg(WST.lang('please_input_problem_description'));
    }
