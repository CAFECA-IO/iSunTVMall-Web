...
class Suppliers extends CSuppliers{
    ...
    public function saveStep($data = []){
        ...
        //判断是否存在入驻申请
        $suppliers = $this->alias('s')->join('__SUPPLIER_USERS__ sur','s.supplierId=sur.supplierId','left')->field('s.*')->where(['sur.userId'=>$userId,'s.dataFlag'=>1])->find();
        if(!empty($suppliers))return WSTReturn(lang('enter_apply_repeat_tips'));
        $suppliers = $this->where(['userId'=>$userId,'dataFlag'=>1])->find();
        $supplierId = 0;
        if(empty($suppliers)){
        ...
        if($suppliers['applyStatus']==1)return WSTReturn(lang('enter_apply_auditing_repeat_tips'));
        if($suppliers['applyStatus']==2)return WSTReturn(lang('enter_apply_repeat_tips'));
        ...
        try{
            ...
            return WSTReturn(lang('operation_success'), 1, ['nextflowId'=>$supplierFlows['nextStep']['flowId']]);
        }catch (\Exception $e) {
            Db::rollback();
    ...
    public function checkApply(){
        $userId = (int)session('WST_USER.userId');
        $rs = $this->alias('s')->join('__SUPPLIER_USERS__ sur','s.supplierId=sur.supplierId','left')->field('s.*')->where(['sur.userId'=>$userId,'s.dataFlag'=>1])->find();
        if(empty($rs)){
    ...
    */
    public function indexSupplierQuery($num=4){
        $cacheData = WSTCache('PC_SUPPLIER_STREET');
        if(!$cacheData){
            ...
            WSTCache('PC_SUPPLIER_STREET',$cacheData,86400);
        }
        return $cacheData;
    }
