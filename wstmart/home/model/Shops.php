...
class Shops extends CShops{
    ...
    public function pageQuery($pagesize){
    	...
    	->join('__SHOP_MEMBERS__ f','f.userId = '.$userId.' and f.shopId=s.shopId','left')
    	->where($where);
        if($recommendWhere!='')$rs->whereOrRaw($recommendWhere)->orderRaw($recommendOrder);
    	$page = $rs->orderRaw('isnull(r.dataSort),r.dataSort asc')->order('distince asc,s.shopId desc')->distinct(true)
    	->field('s.shopId,s.shopImg,s.shopName,s.longitude,s.latitude,s.shopTel,s.shopQQ,s.shopWangWang,s.shopCompany,ss.totalScore,ss.totalUsers,ss.goodsScore,ss.goodsUsers,ss.serviceScore,ss.serviceUsers,ss.timeScore,ss.timeUsers,u.loginName,u.userName,f.id favoriteId,s.areaIdPath')
    	...
    	foreach ($page['data'] as $key =>$v){
    		...
    		$goods = Db::name('goods')
                ...
                ->where(['dataFlag'=> 1,'goodsStatus'=>1,'isSale'=>1,'shopId'=> $v["shopId"]])->field('g.goodsId,gl.goodsName,shopPrice,goodsImg')->limit(10)->order('saleTime desc')->select();
		...
		if(!empty($shopIds)){
			...
			$goodsCats = Db::name('cat_shops')->alias('cs')->join('__GOODS_CATS__ gc','cs.catId=gc.catId and gc.dataFlag=1','left')
                           ->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=cs.catId and langId='.WSTCurrLang())
			               ->where([['shopId','in',$shopIds]])->field('cs.shopId,catName')->select();
    ...
    public function saveStep($data = []){
        ...
        //判断是否存在入驻申请
        $shops = $this->alias('s')->join('__SHOP_USERS__ sur','s.shopId=sur.shopId','left')->field('s.*')->where(['sur.userId'=>$userId,'s.dataFlag'=>1])->find();
        if(!empty($shops))return WSTReturn(lang('enter_apply_repeat_tips'));
        $shops = $this->where(['userId'=>$userId,'dataFlag'=>1])->find();
        $shopId = 0;
        if(empty($shops)){
        ...
        if($shops['applyStatus']==1)return WSTReturn(lang('enter_apply_auditing_repeat_tips'));
        if($shops['applyStatus']==2)return WSTReturn(lang('enter_apply_repeat_tips'));
        ...
        foreach($data as $k => $v){
            ...
            if($field['isShopsTable']==1){
                ...
                if($field['isPhone']==1){
                    $shopExtrasData[$field['fieldName']."AreaCode"] = input($field['fieldName']."AreaCode");
                }
                //经营范围
        ...
        try{
            ...
            return WSTReturn(lang('operation_success'), 1, ['nextflowId'=>$shopFlows['nextStep']['flowId']]);
        }catch (\Exception $e) {
            Db::rollback();
    ...
    public function getShopApply(){
        ...
        $rs = $this->alias('s')->join('__SHOP_EXTRAS__ ss','s.shopId=ss.shopId','inner')
                   ->where(['s.userId'=>$userId,'s.dataFlag'=>1])
                   ->find();
        if(!empty($rs)){
            $rs = $rs->toArray();
    ...
    public function checkApply(){
        $userId = (int)session('WST_USER.userId');
        $rs = $this->alias('s')->join('__SHOP_USERS__ sur','s.shopId=sur.shopId','left')->field('s.*')
        ->where(['sur.userId'=>$userId,'s.dataFlag'=>1])
        ->find();
        if(empty($rs)){
            $rs = $this->where(['userId'=>$userId,'dataFlag'=>1])->find();
        }
        if(!empty($rs)){
    ...
    */
    public function indexShopQuery($num=4){
        $cacheData = WSTCache('PC_SHOP_STREET');
        if(!$cacheData){
            ...
            WSTCache('PC_SHOP_STREET',$cacheData,86400);
        }
        return $cacheData;
    }
