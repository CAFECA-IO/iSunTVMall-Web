...
class Goods extends CGoods{
	...
	public function pageQuery($goodsCatIds = []){
        ...
        $list = $list->field('g.goodsId,gl.goodsName,goodsSn,goodsStock,isNew,saleNum,shopPrice,marketPrice,isSpec,goodsImg,appraiseNum,visitNum,s.shopId,shopName,isSelf,isFreeShipping,gallery');
		...
		if(!empty($list['data'])){
			foreach ($list['data'] as $key => $v) {
				$list['data'][$key]['tags'] = [];
				if($v['isSelf']==1)$list['data'][$key]['tags'][] = "<span class='tag'>".lang('self_support')."</span>";
		...
		if(!empty($list['data'])){
			foreach ($list['data'] as $key =>$v){
				$list['data'][$key]['favGood'] = model('Favorites')->checkFavorite($v['goodsId']);
			}
		}
	...
	public function getPriceGrade($goodsCatIds = []){
		...
		$where[] = ['g.dataFlag','=',1];
		$where[] = ['isSale','=',1];
		if($keyword!='')$where[] = ['gl.goodsName','like','%'.$keyword.'%'];
		$areaId = (int)input('areaId');
		if($areaId>0)$where[] = ['areaId','=',$areaId];
        //属性筛选
		$goodsIds = $this->filterByAttributes();
