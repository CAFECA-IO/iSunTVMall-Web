...
class ListenProtectedUrl 
{
    public function run($params){
        ...
        if(isset($urls[$visit])){
        	...
        	if($userType==-1){
		        if($request->isAjax()){
		        	echo json_encode(['status'=>-999,'msg'=>lang('not_login')]);
		        }else{
                ...
                if(isset($puser['shopId'])){
                    ...
                    if(empty($shop) || $shop['shopStatus']==-1){
                        model('Users')->logout();
                        if($request->isAjax()){
                            echo json_encode(['status'=>-999,'msg'=>lang('shop_out_service')]);
                        }else{
				...
				if($menuType==1){
					if($request->isAjax()){
						echo json_encode(['status'=>-999,'msg'=>lang('apply_shop_to_visit')]);
					}else{
						header("Location:".url('shop/index/login'));
					}
					exit();
