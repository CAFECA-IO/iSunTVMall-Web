...
class Users extends Base{
	...
	public function toRegist(){
        if(strpos(WSTConf('CONF.registerType'),'2')===false)return WSTReturn(lang('illegal_registration'),-1);
		$m = new MUsers();
		$rs = $m->regist();
    ...
    public function toRegistByAccount(){
        if(strpos(WSTConf('CONF.registerType'),'1')===false)return WSTReturn(lang('illegal_registration'),-1);
        $m = new MUsers();
	...
	public function checkLogin(){
        if(strpos(WSTConf('CONF.loginType'),'1')===false)return WSTReturn(lang('illegal_login'),-1);
		$m = new MUsers();
    	$rs = $m->checkLogin();
    	$rs['url'] = session('WST_HO_CURRENTURL');
    	return $rs;
    ...
    public function checkLoginByPhone(){
        if(strpos(WSTConf('CONF.loginType'),'2')===false)return WSTReturn(lang('illegal_login'),-1);
        $m = new MUsers();
	...
	public function getPhoneVerifyCode(){
        ...
        $areaCode = (int)input("areaCode");
		$rs = array();
		if(!WSTIsPhone($userPhone)){
			return WSTReturn(lang('phone_not_correct'));
		}
		$m = new MUsers();
		$rs = $m->checkUserPhone($userPhone,(int)session('WST_USER.userId'));
		if($rs["status"]!=1){
			return WSTReturn(lang('phone_exist'));
		}
		$phoneVerify = rand(100000,999999);
        $rv = ['status'=>-1,'msg'=>lang('send_msg_fail')];
    ...
    public function getPhoneVerifyCode2(){
        ...
        if($rs["status"]==1){
            return WSTReturn(lang('phone_number_not_regist'));
        }
        $phoneVerify = rand(100000,999999);
        $rv = ['status'=>-1,'msg'=>lang('send_msg_fail')];
	...
	public function checkEmail(){
		...
		if(isset($data['error']))$data['error'] = lang('email_exist');
		return $data;
	}
	...
	public function checkFindKey(){
		...
		if($rs["status"]==1){
			return array("error"=>lang('user_not_exist'));
		}else{
			return array("ok"=>"");
		}
    ...
    */
    ...
    public function getEmailVerify(){
        ...
        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
            ...
            $sendRs = WSTSendMail($userEmail,lang('bind_email'),str_replace($find,$replace,$tpl['content']));
        }
    	if($sendRs['status']==1){
            // 绑定的邮箱
    ...
    public function emailEdit(){
    	...
    	if(time()>floatval($bindTime)+30*60)$this->error(lang('checkcode_invalid'));
    	$rs = WSTCheckLoginKey($bindEmail,(int)session('WST_USER.userId'));

    	if($rs["status"]!=1){
    		$this->error(lang('email_exist'));
    		exit();
    	}
        $secretCode = input('secretCode');

        if($code!=$secretCode)return WSTReturn(lang('checkcode_error'),-1);
    ...
    */
    ...
    public function getPhoneVerifyo(){
    	...
    	if($rv['status']==1){
    		...
    		return WSTReturn(lang('send_msg_success'),1);
    	}
    	return $rv;
    }
    /**
    ...
    public function phoneEdito(){
    	...
    	if(!session('Verify_info.phoneVerify') || time()>floatval($timeVerify)+10*60){
    		return WSTReturn(lang('checkcode_invalid'));
    		exit();
    	}
   		if($phoneVerify==session('Verify_info.phoneVerify')){
   			$m = new MUsers();
   			$rs = $m->editPhone((int)session('WST_USER.userId'),session('Verify_info.userPhone'),session('Verify_info.areaCode'));
   			if($process=='Two'){
   				$rs['process'] = $process;
   			}else{
   				$rs['process'] = '0';
   		...
   		return WSTReturn(lang('checkcode_disaccord'));
    }
    ...
    public function phoneEditt(){
    	...
    	return WSTReturn(lang('checkcode_disaccord'),-1);
    }
    ...
    */
    ...
    public function forgetPasst(){
    	if(time()<floatval(session('findPass.findTime'))+30*60){
    		...
    		$this->error(lang('verify_page_expired'));
    	}
    }
    // 重置密码
    public function resetPass(){
         if(!session('findPass')){
            $this->error(lang('link_invalid'),url('home/users/index'));
         }
        if(time()>floatval(session('REST_Time'))+30*60)$this->error(lang('link_invalid'));
        return $this->fetch('forget_pass3');
    }
    // 验证验证码
    public function forgetPasss(){
        if(!session('findPass')){
            $this->error(lang('link_invalid'));
         }
        if(time()>floatval(session('REST_Time'))+30*60)$this->error(lang('link_invalid'));
        $USER = session('findPass');
        if(empty($USER) && $USER['userId']!=''){
            $this->error(lang('please_in_same_browser_operate'));
        }
        $USER = session('findPass');
        if(empty($USER) && $USER['userId']!=''){
            $this->expire(lang('please_in_same_browser_operate'));
        }
    ...
    public function findPass(){
    	...
    	switch ($step) {
    		case 1:#第一步，验证身份
    			...
    			if($rs["status"]==1){
    				return WSTReturn(lang('username_not_exist'));
    				exit();
    			}
    			$m = new MUsers();
    			$info = $m->checkAndGetLoginInfo($loginName);
    			if ($info != false) {
    				session('findPass',array('userId'=>$info['userId'],'loginName'=>$loginName,'userPhone'=>$info['userPhone'],'areaCode'=>$info['areaCode'],'userEmail'=>$info['userEmail'],'loginSecret'=>$info['loginSecret'],'findTime'=>time()));
    				return WSTReturn(lang('operation_success'),1);
    			}else return WSTReturn(lang('username_not_exist'));
    			break;
    		case 2:#第二步,验证方式
    			if (session('findPass.loginName') != null ){
    				if(input("post.modes")==1){
    					if ( session('findPass.userPhone') == null) {
    						return WSTReturn(lang('verify_no_phone'),-1);
    					}
    					$phoneVerify = input("post.Checkcode");
    					if(!$phoneVerify){
    						return WSTReturn(lang('verify_empty'),-1);
    					}
    					return $this->checkfindPhone($phoneVerify);
    				}else{
    					if (session('findPass.userEmail')==null) {
    						return WSTReturn(lang('verify_no_email'),-1);
    					}
    					if(!WSTVerifyCheck($code)){
    						return WSTReturn(lang('checkcode_error'),-1);
    					}
    			...
    			}else $this->error(lang('verify_page_expired'));
    			break;
    		case 3:#第三步,设置新密码
    			$resetPass = session('REST_success');
    			if($resetPass != 1)$this->error(lang('verify_page_invalid'));
    			...
    			if($decrypt_data['status']==1 && $decrypt_data2['status']==1){
    				...
    				return WSTReturn(lang('setting_fail'));
    			}
    			...
    			}else return WSTReturn(lang('verify_two_different_pwd'),-1);
    			break;
    		default:
    			$this->error(lang('verify_page_expired'));
    			break;
    ...
    public function getfindEmail(){
        ...
        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
            ...
            $sendRs = WSTSendMail(session('findPass.userEmail'),lang('password_reset'),str_replace($find,$replace,$tpl['content']));
        }
        if($sendRs['status']==1){
            $uId = session('findPass.userId');
