...
class Goods extends Base{
    ...
    public function lists(){
        ...
        // 分类信息
        $catInfo = Db::name("goods_cats")->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->field("seoTitle,seoKeywords,seoDes,catListTheme")->where(['c.catId'=>$catId,'dataFlag'=>1])->find();
        ...
        foreach($brandIds as $bId){
        	if($bId>0){
                ...
                $selector[] = ['id'=>join(',',$bIds),'type'=>'brand','label'=>lang('brand'),"val"=>join('、',$brandNames)];
            }
        }
        // 当前是否有品牌筛选
        if(!empty($selector)){
    	...
    	if($data['minPrice']!='' && $data['maxPrice']!=''){
    		$selector[] = ['id'=>0,'type'=>'price','label'=>lang('price'),"val"=>$data['minPrice']."-".$data['maxPrice']];
    	}
        if($data['minPrice']!='' && $data['maxPrice']==''){
        	$selector[] = ['id'=>0,'type'=>'price','label'=>lang('price'),"val"=>$data['maxPrice'].lang('above')];
    	}
        if($data['minPrice']=='' && $data['maxPrice']!=''){
        	$selector[] = ['id'=>0,'type'=>'price','label'=>lang('price'),"val"=>"0-".$data['maxPrice']];
    	}
    	//处理已选属性
        $at = new AT();
        ...
        $data['catNamePath'] = lang('all_goods_cat');
    ...
    public function detail(){
    	...
    	if(!empty($goods)){
            ...
            // 分类信息
            $catInfo = Db::name("goods_cats")->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->field("detailTheme")->where(['c.catId'=>$goods['goodsCatId']])->find();
	...
	public function contrastGoods(){
		...
		if($id>0){
			...
			$catId = $catId[0];
			if(isset($contras['catId']) && $catId!=$contras['catId'])return WSTReturn(lang('contrast_goods_limit_cat_desc'),-1);
			if(isset($contras['list']) && count($contras['list'])>3)return WSTReturn(lang('contrast_goods_column_full'),-1);
	...
	public function contrastDel(){
		...
		return WSTReturn(lang('del_cart_success'),1);
	}
	/**
	 *  商品对比
	 */
	public function contrast(){
		...
		if(isset($contras['list'])){
			...
			$shops['name'] = lang('shop');
			$shops['type'] = 'shop';
			$shops['info'] =  $shop;
			$lists[] = $shops;
			$scores['name'] = lang('goods_score');
			$scores['type'] = 'score';
			$scores['info'] =  $score;
			$lists[] = $scores;
			$brands['name'] = lang('brand');
			$brands['type'] = 'brand';
			$brands['info'] =  $brand;
			$lists[] = $brands;
