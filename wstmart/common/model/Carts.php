...
class Carts extends Base{
	...
	public function addCart($uId = 0){
		...
		if($userId==0)return WSTReturn(lang('add_cart_fail_no_login'),-2);
		//验证传过来的商品是否合法
		$chk = $this->checkGoodsSaleSpec($goodsId,$goodsSpecId);
		if($chk['status']==-1)return $chk;
		//检测库存是否足够
		if($chk['data']['stock']<$cartNum)return WSTReturn(lang('add_cart_fail_no_stock'), -1);
		//添加实物商品
		if($chk['data']['goodsType']==0){
			...
			if(false !==$rs){
				...
				return WSTReturn(lang('add_cart_success'), 1);
			}
		}else{
			//非实物商品
            $carts = [];
            ...
            return WSTReturn(lang('add_cart_success'), 1,['forward'=>'quickSettlement']);
		}
		return WSTReturn(lang('add_cart_fail'), -1);
	}
	/**
	 * 验证商品是否合法
	 */
	...
	public function getQuickCarts($uId=0){
		...
		$rs = Db::name('goods')->alias('g')
		           ...
		           ->where($where)
		           ->field('s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,gl.goodsName,g.shopPrice,g.shopPrice defaultShopPrice,g.goodsStock,g.goodsImg,g.goodsCatId,g.isFreeShipping,s.isInvoice,g.goodsType')
		           ->find();
	...
	public function getCarts($isSettlement = false, $uId=0){
		...
		$rs = Db::table($prefix.'carts')
		           ->alias([$prefix.'carts'=>'c',$prefix.'goods' => 'g',$prefix.'shops' => 's',$prefix.'goods_specs' => 'gs',$prefix.'goods_langs' => 'gl'])
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,s.userId,s.shopId,s.shopName,g.goodsId,g.shippingFeeType,g.shopExpressId,s.shopQQ,shopWangWang,gl.goodsName,g.shopPrice,g.shopPrice defaultShopPrice,g.goodsStock,g.goodsWeight,g.goodsVolume,g.isSpec,gs.specPrice,gs.specStock,gs.specWeight,gs.specVolume,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.goodsCatId,g.isFreeShipping,s.isInvoice,g.goodsType')
		           ->select();
		$carts = [];
		$goodsIds = [];
		$goodsTotalNum = 0;
		...
		if(count($goodsIds)>0){
		    $specs = DB::name('spec_items')->alias('s')->join('__SPEC_CATS__ sc','s.catId=sc.catId','left')
		        ...
		        ->where([['s.goodsId','in',$goodsIds],['s.dataFlag','=',1]])->field('scl.catName,s.itemId,sil.itemName')->select();
		    if(count($specs)>0){
		    	$specMap = [];
		    	foreach ($specs as $key =>$v){
		    		$specMap[$v['itemId']] = $v;
	...
	public function getCartInfo($isSettlement = false,$uId = 0){
		...
		$rs = $this->alias('c')->join('__GOODS__ g','c.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,g.goodsId,gl.goodsName,g.shopPrice,g.goodsStock,g.isSpec,gs.specPrice,gs.specStock,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.shopId')
		           ->select();
		$goodsIds = [];
		$goodsTotalMoney = 0;
		$goodsTotalNum = 0;
		...
		if(count($goodsIds)>0){
		    $specs = DB::name('spec_items')->alias('s')->join('__SPEC_CATS__ sc','s.catId=sc.catId','left')
		        ->join('__SPEC_ITEMS_LANGS__ sil','sil.itemId=s.itemId and sil.langId='.WSTCurrLang())
		        ->where([['s.goodsId','in',$goodsIds],['s.dataFlag','=',1]])->field('s.itemId,sil.itemName')->select();
		    if(count($specs)>0){
		    	$specMap = [];
		    	foreach ($specs as $key =>$v){
		    		$specMap[$v['itemId']] = $v;
    ...
    public function moveToFavorites($uId = 0){
        ...
        try{
            ...
            Db::commit();
            return WSTReturn(lang('follow_success'), 1);
        }catch (\Exception $e) {
            Db::rollback();
            return WSTReturn(lang('follow_fail'),-1);
        }
    }
}
