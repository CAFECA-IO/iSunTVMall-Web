...
class OrderComplains extends Base{
	...
	public function queryUserComplainByPage($uId=0){
		...
		foreach($rs['data'] as $k=>$v){
			if($v['complainStatus']==0){
				$rs['data'][$k]['complainStatus'] = lang('complain_status_0');
			}elseif($v['complainStatus']==1){
				$rs['data'][$k]['complainStatus'] = lang('complain_status_1');
			}elseif($v['complainStatus']==2 || $v['complainStatus']==3 ){
				$rs['data'][$k]['complainStatus'] = lang('complain_status_2');
			}elseif($v['complainStatus']==4){
				$rs['data'][$k]['complainStatus'] = lang('complain_status_4');
			}
	...
	public function saveComplain($uId=0){
		...
		$userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
		$orderId = (int)input('orderId');
		hook("beforeSaveComplain",["orderId"=>$orderId,"userId"=>$userId]);
		$data['orderId'] = $orderId;
        //判断订单是否该用户的
		$order = db('orders')->field('orderId,shopId,orderNo')->where("userId=$userId")->find($data['orderId']);
		if(!$order){
			return WSTReturn(lang('invalid_order_information'),-1);
		}
		...
		if((int)$rs['complainId']>0){
			return WSTReturn(lang('order_already_complain'),-1);
		}
		Db::startTrans();
		try{
			...
			if($rs !==false){
				...
				Db::commit();
				return WSTReturn(lang('complain_submit_success_tips'),1);
			}else{
				return WSTReturn($this->getError(),-1);
			}
		}catch (\Exception $e) {
	...
	public function saveRespond($sId=0){
        ...
        if((int)$complainRs['needRespond']!=1){
			return WSTReturn(lang('invalid_complain_information'),-1);
		}
		if((int)$complainRs['complainStatus']!=1){
			return WSTReturn(lang('complain_lawsuit_tips'),-1);
		}
		Db::startTrans();
		try{
