      ...
      */
      ...
      public function complateRecharge($obj){
			$trade_no = $obj["trade_no"];
	      	...
	      	if(!empty($log)){
	      		return WSTReturn(lang('has_recharge'),-1);
	      	}
	      	Db::startTrans();
	      	try {
	      		$giveMoney = 0;
	      		...
	      		$lm['dataSrc'] = 4;
	      		if($giveMoney>0){
	      			$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'recharge_money_tips','params'=>[$payMoney]]);
	      		}else{
	      			$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'recharge_give_money_tips','params'=>[$payMoney,$giveMoney]]);
	      		}
	      		...
	      		Db::commit();
	      		return WSTReturn(lang('recharge_success'),1);
	      	} catch (Exception $e) {
	      		Db::rollback();
	      		return WSTReturn(lang('recharge_fail'),-1);
	      	}
      }
