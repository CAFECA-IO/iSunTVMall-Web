...
class CashConfigs extends Base{
      ...
      */
      public function pageQuery($targetType,$targetId){
          ...
          $page = $this->alias('c')->join('__BANKS__ b','c.accTargetId=b.bankId')->join('__BANKS_LANGS__ bl','bl.bankId=b.bankId and langId='.WSTCurrLang())->where($where)->field('bl.bankName,b.bankImg,c.*')->order('c.id desc')->paginate()->toArray();
      ...
      public function listQuery($targetType,$targetId){
          ...
          $where['a.dataFlag'] = 1;
          $list = $this->alias('a')->join('__BANKS__ b','a.accTargetId=b.bankId')->join('__BANKS_LANGS__ bl','bl.bankId=b.bankId and langId='.WSTCurrLang())->where($where)->field('a.id,a.accNo,a.accUser,bl.bankName')->select();
          if(count($list)>0){
      ...
      public function add($uId=0){
          ...
          if ($accNo>0) return WSTReturn(lang('drawal_account_already_exist'), -1);
          unset($data['id']);
          $data['targetType'] = 0;
          $data['accType'] = 3; 
          ...
          if(false !== $result){
              return WSTReturn(lang('operation_success'), 1,['id'=>$this->id]);
          }else{
      ...
      public function del($uId=0){
        $userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
         ...
         if($object==null)return WSTReturn(lang('operation_fail'),-1);
         $object->dataFlag = -1;
         $result = $object->save();
         if(false !== $result){
            return WSTReturn(lang('operation_success'),1);
