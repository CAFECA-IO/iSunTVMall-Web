...
class Invoices extends Base{
	...
	*/
	public function add($uId=0){
		...
		if($count>20)return WSTReturn(lang('require_invoice_add_limit'));
		$data = input('param.');
		$data['userId'] = $userId;
		...
		if (!$validate->scene('add')->check($data)) {
            ...
            if($rs!==false) return WSTReturn(lang('add_success'),1,['id'=>$this->id]);
		}
	...
	*/
	public function del($uId=0){
		...
		if($rs!==false)return WSTReturn(lang('del_success'),1);
