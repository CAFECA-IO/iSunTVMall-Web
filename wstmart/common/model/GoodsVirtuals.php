...
class GoodsVirtuals extends Base{
    ...
    public function del($sId=0){
        ...
        $ids = input('ids');
        $id = input('id');
        if($ids=='')return WSTReturn(lang('please_select_del_card_number'));
        try{
    ...
    public function edit($sId=0){
        ...
        if(empty($rs))return WSTReturn(lang('illegal_card_number'));
        $cardNo = input('cardNo');
        $cardPwd = input('cardPwd');
        if($cardNo=='' || $cardPwd=='')return WSTReturn(lang('please_input_complete_card_number'));
        $conts = $this->where([['id','<>',$id],['shopId','=',$shopId],['cardNo','=',$cardNo],['dataFlag','=',1]])->Count();
        if($conts>0)return WSTReturn(lang('card_number_has_exist'));
    ...
    public function add($sId=0){
        ...
        if(empty($goods))return WSTReturn(lang('illegal_card_number_goods'));
        $cardNo = input('cardNo');
        $cardPwd = input('cardPwd');
        if($cardNo=='' || $cardPwd=='')return WSTReturn(lang('please_input_complete_card_number'));
        $conts = $this->where(['shopId'=>$shopId,'dataFlag'=>1,'cardNo'=>$cardNo])->Count();
