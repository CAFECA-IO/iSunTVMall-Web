...
class LogSms extends Base{
	...
	public function sendSMS($smsSrc,$phoneNumber,$params,$smsFunc,$verfyCode,$userId=0,$isLimit=1){
		//判断有没有开启短信功能
		if((int)WSTConf('CONF.smsOpen')==0)return WSTReturn(lang('no_enabled_msg_interface'));
		...
		if($isLimit==1){
			...
			if(WSTConf("CONF.smsVerfy")==1){
				...
				if((int)WSTConf('CONF.isAddonCaptcha')!=1){
					...
					if(!$rs)return WSTReturn(lang('verifycode_incorrect'),-2);
				}else{
					$rs =  WSTReturn(lang('sms_send_fail'),-2);
			...
			if($smsRs['counts']>(int)WSTConf("CONF.smsLimit")){
				return WSTReturn(lang('donnt_frequently_send_sms'));
			}
		...
		$rdata = ['msg'=>lang('sms_send_fail'),'status'=>-1];
		try{
