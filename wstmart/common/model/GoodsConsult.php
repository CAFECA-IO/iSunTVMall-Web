    ...
    */
    public function add($uId=0){
    	...
    	if(!WSTCheckFilterWords($data['consultContent'],WSTConf("CONF.limitWords"))){
			return WSTReturn(lang('consult_content_illegal_characters'));
		}
    ...
    */
    public function pageQuery($sId=0){
    	...
    	$rs = $this->alias('gc')
        		   ...
        		   ->field('gc.*,u.loginName,u.userPhoto,gl.goodsName,g.goodsImg')
        		   ->where($where)
    ...
    */
    public function reply($sId=0){
    	...
    	if(!WSTCheckFilterWords($data['reply'],WSTConf("CONF.limitWords"))){
				return WSTReturn(lang('reply_content_illegal_characters'));
		}
    	// 转义,防止xss攻击
    	...
    	if($hasReply!='')return WSTReturn(lang('consult_already_reply'));
        // 检测是否属于该商家
        $owner = $this->alias('gc')
                      ->join('__GOODS__ g','g.goodsId=gc.goodsId','inner')
                      ->where([['gc.id','=',$data['id']],['g.shopId','=',$shopId]])
                      ->find();
        if(empty($owner))return WSTReturn(lang('no_privilege_reply'));
    	$validate = new Validate;
