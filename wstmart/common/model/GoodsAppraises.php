...
class GoodsAppraises extends Base{
	public function queryByPage($sId=0){
		...
		if($goodsName != ''){
			$where[] = ['gl.goodsName','like',"%$goodsName%"];
		}
		if($c2Id!=0 && $c1Id!=0){
		...
		$data = $model->alias('g')
					  ...
					  ->field('u.userPhoto, u.loginName, g.goodsId,g.goodsImg,gl.goodsName,ga.shopReply,ga.id gaId,ga.replyTime,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,u.loginName')
	...
	*/
	public function userAppraise(){
		...
		$data = $model->alias('g')
                      ->join('__GOODS_LANGS__ gl','gl.goodsId=g.goodsId and gl.langId='.WSTCurrLang())
					  ->field('g.goodsId,g.goodsImg,gl.goodsName,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,ga.shopReply,ga.replyTime,s.shopName,u.userName,o.orderNo')
					  ->join('__GOODS_APPRAISES__ ga','g.goodsId=ga.goodsId','inner')
	...
	*/
	public function add($uId=0){
		...
		hook("beforeSaveGoodsAppraises",["orderId"=>$orderId,"uId"=>$uId,"goodsId"=>$goodsId,"goodsSpecId"=>$goodsSpecId,"orderGoodsId"=>$orderGoodsId]);

		// 没有传order_goods表的id
		if($orderGoodsId==0)return WSTReturn(lang('data_error'));
		...
		if(isset($content)){
			if(!WSTCheckFilterWords($content,WSTConf("CONF.limitWords"))){
				return WSTReturn(lang('appraise_content_illegal_characters'));
			}
		}
		...
		if(empty($orders))return WSTReturn(lang('invalid_order'));
		if($orders['orderStatus']!=2)return WSTReturn(lang('order_status_changed'));
		//检测商品是否已评价
		$apCount = $this->where(['orderGoodsId'=>$orderGoodsId,'dataFlag'=>1])->count();
		if($apCount>0)return WSTReturn(lang('goods_has_appraised'));
		Db::startTrans();
		try{
			//增加订单评价
			$data = [];
				...
				if($isFinish){
					if(WSTConf("CONF.isAppraisesScore")==1){
						...
						if($appraisesScore>0){
							...
							$score['dataRemarks'] = json_encode(['type'=>'lang_params','key'=>'goods_appraise_get_score_tips','params'=>[$orders['orderNo'],$appraisesScore]]);
							$score['scoreType'] = 1;
