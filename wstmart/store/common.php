...
function WSTStoreCats($parentId){
	...
	$dbo = Db::table('__SHOP_CATS__')->alias('a')->join('__SHOP_CATS_LANGS__ scl','scl.catId=a.catId and langId='.WSTCurrLang())->where(['dataFlag'=>1, 'isShow' => 1,'parentId'=>$parentId,'shopId'=>$shopId]);
	return $dbo->field("catName,a.catId")->order('catSort asc')->select();
}
