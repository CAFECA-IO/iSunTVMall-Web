...
class Users extends CUsers{
	...
	public function checkStoreLogin(){
    	...
    	if(!empty($rs)){
            if($rs['loginPwd']!=md5($loginPwd.$rs['loginSecret']))return WSTReturn(lang("pwd_err"));
			...
			if(empty($shoprs)){
				return WSTReturn(lang("is_not_store"));
			}
    		...
    		if($rs['userType']==2){
                ...
                if(empty($store)){
                    return WSTReturn(lang("disable_store"),-1);
                }else{
                	$shopId = $store['shopId'];
                	$shop = Db::name("shops")->where(['shopId'=>$shopId,'shopStatus'=>1,'dataFlag'=>1])->find();
                	if(empty($shop)){
                		return WSTReturn(lang("expired_store"),-1);
                	}
                    //处理处店铺权限
    	...
    	return WSTReturn(lang("no_find_user"));
    }
	/**
