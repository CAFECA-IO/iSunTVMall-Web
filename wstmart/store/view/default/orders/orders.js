...
function deliver(id,deliverType){
	if(deliverType==1){
        WST.confirm({content:WST.lang("delivery_confirm"), yes:function(tips){
            var ll = WST.load(WST.lang("processing"));
...
function toExport(typeId,status,type){
	...
	params.type = type;
	var box = WST.confirm({content:WST.lang("export_orders_confirm"),yes:function(){
		layer.close(box);
...
function toDeliverTemplate(){
	var params = {};
	params = WST.getParams('.s-ipt');
	var box = WST.confirm({content:WST.lang("delivery_tips"),yes:function(){
		layer.close(box);
		location.href=WST.U('store/orders/toDeliverTemplate',params);
         }});
}
//导入订单
function toExportDeliver(){
	location.href=WST.U('store/imports/toDeliverExport');
}
