...
WST.upload = function(opts){
	var _opts = {};
	_opts = $.extend(_opts,{duplicate:true,auto: true,swf: WST.conf.STATIC +'/plugins/webuploader/Uploader.swf',server:WST.U('store/index/uploadPic'),duplicate:true},opts);
...
WST.getVerify = function(id){
    $(id).attr('src',WST.U('store/index/getVerify','rnd='+Math.random()));
}
WST.getAreas = function(obj,id,val,fval,callback){
	...
	$.post(WST.U('store/areas/listQuery'),params,function(data,textStatus){
...
WST.ITSetGoodsCats = function(opts){
	...
	if(opts.childIds.length>0){
		...
		$.post(WST.U('store/goodscats/listQuery'),{parentId:opts.val},function(data,textStatus){
		     var json = WST.toJson(data);
...
WST.ITAreas = function(opts){
	...
	$.post(WST.U('store/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
	     var json = WST.toJson(data);
	     if(json.data && json.data.length>0){
	    	 json = json.data;
	         var html = [],tmp;
...
WST.shopsCats = function(objId,pVal,objVal){
	$('#'+objId).empty();
	$.post(WST.U('store/shopcats/listQuery'),{parentId:pVal},function(data,textStatus){
	     var json = WST.toJson(data);
	     var html = [],cat;
	     html.push("<option value='' >-"+WST.lang('select')+"-</option>");
...
WST.getSysMessages = function(val){
	...
	$.post(WST.U('store/index/getSysMessages'),{tasks:val},function(data){
		var json = WST.toJson(data);
		var totalNum = 0
		if(json.message){
			$('#m-msg').hide();
		...
		if(json.storeorder){
			for(var key in json.storeorder){
				if($('#m-'+key)[0]){
					$('#m-'+key).hide();
					if(json.storeorder[key]!='0'){
						$('#m-'+key).show();
					    $('#m-'+key+" span").html(json.storeorder[key]);
					}
				}
				totalNum += parseInt(json.storeorder[key],10);
			}
		}
		if(totalNum>0){
			$('.msg-num').show();
