...
function initGrid(p){
    ...
    var cols = [
        ...
        {title:WST.lang("creat_time"), name:'createTime' ,width:300,sortable:true},
    ...
    mmg = $('.mmg').mmGrid({height: h-100,indexCol: true, cols: cols,method:'POST',
        url: WST.U('store/storeroles/pageQuery'), fullWidthRows: true, autoLoad: false,remoteSort: true,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function toEdit(id){
	location.href = WST.U('store/storeroles/edit','id='+id+'&p='+WST_CURR_PAGE);
}
function toAdd(){
    location.href = WST.U('store/storeroles/add','p='+WST_CURR_PAGE);
}
...
function save(p){
	$('#shoprole').isValid(function(v){
		if(v){
		    ...
		    $.post(WST.U('store/storeroles/'+((params.id==0)?"toAdd":"toEdit")),params,function(data,textStatus){
		    	layer.close(loading);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
		    		WST.msg(json.msg,{icon:1},function(){
						location.href=WST.U('store/storeroles/index',"p="+p);
					});
		    	}else{
...
//删除角色
function del(id){
	var c = WST.confirm({content:WST.lang("role_del_confirm"),yes:function(){
		layer.close(c);
		var load = WST.load({msg:WST.lang("waiting")});
		$.post(WST.U('store/storeroles/del'),{id:id},function(data,textStatus){
			layer.close(load);
		    var json = WST.toJson(data);
		    if(json.status==1){
