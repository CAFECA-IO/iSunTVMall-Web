...
class Paypals extends Base{
	...
	public function toPaypal(){
        ...
        if($payObj=="recharge"){//充值
        	...
        	$returnUrl = url("app/paypals/payRes","",true,true);
	    	$cm = new CM();
	    	...
	    	if($needPay==0){
				header("Location:".$returnUrl."?status=0");
				exit();
			}
		...
		$appPath = $this->request->root().'/wstmart/app/view/paypal';
		return $this->fetch('paypal/pay', [
			"v"=>WSTConf("wstVersion"), 
			"APP_PATH"=>$appPath,
			"resourceDomain"=>$this->domain(),
			"payurl"=>url('app/paypals/getpaypalsurl','','',true),
	...
	public function payRes(){
		// http://localhost/dz_isuntvmall2/app/paypals/toPaypal?pkey=gKZ2rIN4paiAeHTcr6qnp4Gppq-vtYGxhWKirA&tokenId=b25f943d4783185d3378b137029b0b16
