...
class Shops extends CShops{
    ...
    public function pageQuery(){
    	...
    	$page = $rs->orderRaw('isnull(r.dataSort),r.dataSort asc')->orderRaw($order)->field('s.shopId,s.shopImg,s.isSelf,s.shopName,s.shopCompany,ss.totalScore,ss.totalUsers,ss.goodsScore,ss.goodsUsers,ss.serviceScore,ss.serviceUsers,ss.timeScore,ss.timeUsers,s.areaIdPath')
       ...
       $span = ($maxScore - $minScore) / 3;
		...
		if(!empty($shopIds)){
			$rccreds = Db::name('shop_accreds')->alias('sac')->join('__ACCREDS__ a','a.accredId=sac.accredId and a.dataFlag=1','left')
                         ->join('__ACCREDS_LANGS__ al','al.accredId=a.accredId and langId='.WSTCurrLang())
			...
			$goodsCats = Db::name('cat_shops')->alias('cs')->join('__GOODS_CATS__ gc','cs.catId=gc.catId and gc.dataFlag=1','left')
                           ->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=cs.catId and gcl.langId='.WSTCurrLang())
