...
function WSTSendMail($to, $subject, $content)
{
	...
	return ['status' => 0, 'msg' => lang('mail_sending_disabled')];
}
...
function WSTConfig()
{
	...
	if (!$rs) {
		...
		//设置语言
		$rs['sysLangs'] = Db::name('languages')->where(['status'=>1])->column('*','code');
		//判断资源路径
...
{
	if ($val == '') return WSTReturn(lang('user_add_login_name_empty'));
	if (!WSTCheckFilterWords($val, WSTConf("CONF.registerLimitWords"))) {
		return WSTReturn(lang('user_add_login_name_fail_code'));
	}
	...
	$rs = $dbo->count();
	if ($rs == 0) {
		return WSTReturn(lang('user_add_login_name_use_ok'), 1);
	}
	return WSTReturn(lang('user_add_login_name_exist'));
}
...
{   $rs = [];
	if ($isSelf == 1) {
		$rs = Db::name('goods_cats')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where(['dataFlag' => 1, 'isShow' => 1, 'catId' => $parentId])
			->field("catName,simpleName,c.catId,parentId")->order('catSort asc')->find();
	} else {
		$dbo = Db::name('goods_cats')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where(['dataFlag' => 1, 'isShow' => 1, 'parentId' => $parentId]);
		if ($isFloor != -1) $dbo->where('isFloor', $isFloor);
		$rs = $dbo->field("catName,simpleName,c.catId")->order('catSort asc')->select();
	}
	return $rs;
}
...
{
	if ($isSelf == 1) {
		return Db::name('trades t')->join('__TRADES_LANGS__ tl','tl.tradeId=t.tradeId and tl.langId='.WSTCurrLang())->where(['t.dataFlag' => 1, 't.isShow' => 1, 't.tradeId' => $parentId])
			->field("tl.tradeName,tl.simpleName,t.tradeId,t.parentId")->order('t.tradeSort asc')->find();
	} else {
		$dbo = Db::name('trades t')->join('__TRADES_LANGS__ tl','tl.tradeId=t.tradeId and tl.langId='.WSTCurrLang())->where(['t.dataFlag' => 1, 't.isShow' => 1, 't.parentId' => $parentId]);
		return $dbo->field("tl.tradeName,tl.simpleName,t.tradeId,t.tradeFee")->order('t.tradeSort asc')->select();
	}
}
...
function WSTShopApplyGoodsCats($parentId = 0)
{
	...
	$rs = Db::name('goods_cats')->alias('gc')
	    ->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=gc.catId and gcl.langId='.WSTCurrLang(),'inner')
...
function WSTUploadPic($fromType = 0)
{
	$fileKey = key($_FILES);
	$dir = Input('param.dir');
	if ($dir == '') return json_encode(['msg' => lang('no_specified_directory'), 'status' => -1]);
	$dirs = WSTConf("CONF.wstUploads");
	if (!in_array($dir, $dirs)) {
		return json_encode(['msg' => lang('illegal_file_directory'), 'status' => -1]);
	}
	// 上传文件
	$file = request()->file($fileKey);
	if ($file === null) {
		return json_encode(['msg' => lang('upload_file_not_exist_limit'), 'status' => -1]);
	}
	$rule = [
...
function WSTImg($imgurl, $imgType = 1, $default = '')
{
	...
	switch ($imgType) {
		case 0:
			...
			break;
		case 1:
			$img =  str_replace('.', '_thumb.', $imgurl);
			break;
		case 2:
			$img =  str_replace('.', $m . '.', $imgurl);
			break;
		case 3:
			$img =  str_replace('.', $m . '_thumb.', $imgurl);
			break;
	}
...
function WSTLangPayType($v)
{
	switch ($v) {
		case 0:
			return lang('pay_type_cash_on_delivery');
		case 1:
			return lang('pay_type_online');
		case 2:
			return lang('pay_type_offline');
	}
}
/**
 * 收货方式
...
function WSTLangDeliverType($v)
{
	switch ($v) {
		case 1:
			return lang('deliver_type_self');
		case 0:
			return lang('deliver_type_provide_home');
		default:
			return '';
	}
}
...
function WSTLangOrderStatus($v)
{
	switch ($v) {
		case -3:
			return lang('order_status_reject');
		case -2:
			return lang('order_status_wait_pay');
		case -1:
			return lang('order_status_cancel');
		case 0:
			return lang('order_status_wait_deliver');
		case 1:
			return lang('order_status_wait_receive');
		case 2:
			return lang('order_status_received');
	}
}
/**
 * 积分来源
...
function WSTLangScore($v)
{
	switch ($v) {
		case 1:
			return lang('score_src_type_1');
		case 2:
			return lang('score_src_type_2');
		case 4:
			return lang('score_src_type_3');
		case 5:
			return lang('score_src_type_4');
		case 6:
			return lang('score_src_type_5');
		case 7:
			return lang('score_src_type_6');
		case 10001:
			return lang('score_src_type_7');
		default:
			return lang('score_src_type_8');
	}
}
/**
 * 资金来源
...
function WSTLangMoneySrc($v)
{
	switch ($v) {
		case 1:
			return lang('money_src_type_1');
		case 2:
			return lang('money_src_type_2');
		case 3:
			return lang('money_src_type_3');
		case 4:
			return lang('money_src_type_4');
		case 5:
			return lang('money_src_type_5');
		case 6:
			return lang('money_src_type_6');
		case 10001:
			return lang('money_src_type_7');
		default:
			return lang('money_src_type_8');
	}
}
/**
 * 投诉状态
...
function WSTLangComplainStatus($v)
{
	switch ($v) {
		case 0:
			return lang('complain_status_1');
		case 1:
			return lang('complain_status_2');
		case 2:
			return lang('complain_status_3');
		case 3:
			return lang('complain_status_4');
		case 4:
			return lang('complain_status_5');
	}
}
...
function WSTLangSex($v)
{
	switch ($v) {
		case 0:
			return lang('sex_type_1');
		case 1:
			return lang('sex_type_2');
		case 2:
			return lang('sex_type_3');
	}
}
...
function WSTBrandApplyStatus($type)
{
	switch ($type) {
		case -1:
			return lang('apply_status_1');
		case 0:
			return lang('apply_status_2');
		case 1:
			return lang('apply_status_3');
	}
}
...
function WSTLangLoginSrc($v)
{
	switch ($v) {
		case 0:
			return lang('login_src_type_1');
		case 1:
			return lang('login_src_type_2');
		case 2:
			return lang('login_src_type_3');
		case 3:
			return lang('login_src_type_4');
		case 4:
			return lang('login_src_type_5');
		case 5:
			return lang('login_src_type_6');
	}
}
 ...
 */
function WSTDatas($catCode, $id = 0 , $lang = '')
{
	...
	if (!$data) {
		...
		//加载多语言
		$drs = Db::name('datas_langs')->where([['dataId','in',$ids]])->select();
        $drsmp = [];
        foreach ($drs as $key => $v) {
        	$drsmp[$v['dataId']][$v['langId']] = $v['dataName'];
        }
        foreach ($data as $ckey => $cv) {
        	foreach ($cv as $key => $v) {
	        	$data[$ckey][$v['dataVal']]['langParams'] = $drsmp[$v['id']];
	        }
        }
		WSTCache('WST_DATAS', $data, 378432000,'CACHETAG_DATA');
	}
	//加载数据
	$lang = ($lang!='')?WSTSysLangs($lang):WSTCurrLang();

	foreach ($data as $ckey => $cv) {
		foreach ($cv as $key => $v) {
			$data[$ckey][$key]['dataName'] = $v['langParams'][$lang];
		}
	}
	if (isset($data[$catId])) {
 ...
 * 订单流程
 */
function WSTOrderLog($orderStatus){
   switch($orderStatus){
   	  case -3:return WSTLang('order_reject');
   	  case -2:return WSTLang('order_success_wait_pay');
   	  case -1:return WSTLang('user_cancel_reason');
   	  case 0:return WSTLang('order_success');
   	  case 1:return WSTLang('shop_has_deliverd');
   	  case 2:return WSTLang('user_has_received');
   }
}
 ...
 * 日志解析
 */
function WSTLogJson($str){
   if($str=='')return '';
   $json = json_decode($str,true);
   if($json && isset($json['type'])){
      if($json['type']=='data'){
         $data = WSTDatas($json['key'],$json['val']);
         if($data)return $data['dataName'].(isset($json['remark'])?':'.$json['remark']:'');
      }else if($json['type']=='lang_data'){
      	 $data = WSTDatas($json['datakey'],$json['dataval']);
      	 if(!$data)return '';
         return WSTLang($json['key'],[$data['dataName'],(isset($json['remark'])?':'.$json['remark']:'')]);
      }else if($json['type']=='lang_params'){
         return WSTLang($json['key'],$json['params']);
      }else{
         return WSTLang($json['key']).(isset($json['remark'])?':'.$json['remark']:'');
      }
   }
   return $str;
}
...
function WSTSubDatas($catCode, $id = 0)
{
	...
	if (!$data) {
        ...
        foreach ($data as $key => $v) {
        	$data[$key]['langParams'] = $drsmp[$v['id']];
        }
		WSTCache('WST_SUBDATAS_' . $catId, $data, 378432000);
	}
	//加载数据
	foreach ($data as $key => $v) {
		$data[$key]['dataVal'] = $v['langParams'][WSTCurrLang()];
	}
	return $data;
}
...
function WSTScore($score, $users, $type = 5, $len = 0, $total = 1)
{
	...
	switch ($type) {
		...
		case 10:
			return round($score / $total * 2 / $users, $len);
		case 100:
			return round($score / $total * 2 / $users, $len);
	}
}
...
function WSTEditUpload($fromType)
{
	...
	if (!empty($_FILES['imgFile']['error'])) {
		switch ($_FILES['imgFile']['error']) {
			case '1':
				$error = lang('upload_error_type_1');
				break;
			case '2':
				$error = lang('upload_error_type_2');
				break;
			case '3':
				$error = lang('upload_error_type_3');
				break;
			case '4':
				$error = lang('upload_error_type_4');
				break;
			case '6':
				$error = lang('upload_error_type_5');
				break;
			case '7':
				$error = lang('upload_error_type_6');
				break;
			case '8':
				$error = lang('upload_error_type_7');
				break;
			case '999':
			default:
				$error = lang('upload_error_type_8');
		}
	...
	$dirs = WSTConf("CONF.wstUploads");
	if (!in_array($dir, $dirs)) {
		return json_encode(['code' => "001", 'message' => lang('illegal_file_directory')]);
	}
	// 上传文件
	$file = request()->file($fileKey);
	if ($file === null) {
		return json_encode(["code" => "001", "message" => lang('upload_file_not_exist_limit')]);
	}
	$rule = [
		'type' => 'image/png,image/gif,image/jpeg,image/x-ms-bmp',
		'ext' => 'jpg,jpeg,gif,png,bmp',
...
function WSTSendMsg($to, $content, $msgJson = [], $msgType = 1)
{
	...
	// 推送消息
	hook('pushNoticeToApp', ["type" => "pushToSingle", "userId" => $to, "title" => lang('new_message'), "content" => $content]);
...
function WSTUserRank($userScore)
{
	...
	if (!$data) {
		...
		$urs = Db::name('user_ranks_langs')->select();
		$ump = [];
		foreach ($urs as $key => $v) {
			$ump[$v['rankId']][$v['rankId']] = $v['rankName'];
		}
        foreach ($data as $key => $v) {
        	$data[$key]['langParams'] = $ump[$v['rankId']];
        }
		WSTCache('WST_USER_RANK', $data, 2592000);
	}
	if (!$data) $data = [];
	foreach ($data as $key => $v) {
		$v['rankName'] = isset($v['langParams'][WSTCurrLang()])?$v['langParams'][WSTCurrLang()]:'';
...
function WSTTimeToStr($second)
{
	...
	//返回字符串
	return (($day > 0) ? ($day . lang('day')) : "") . ($hour < 10 ? "0" . $hour : $hour) . ':' . ($minute < 10 ? "0" . $minute : $minute) . ':' . ($second < 10 ? "0" . $second : $second);
}
...
function WSTOrderModule($orderCode = '')
{
	...
	if ($orderCode != '') {
		return array_key_exists($orderCode, $addonMaps) ? $addonLangParams[WSTCurrLang()][$orderCode] : lang('normal_order');
	} else {
		$data = [];
		$data[] = ['name' => 'order', 'title' => lang('normal_order')];
		foreach ($addonMaps as $key => $v) {
            if(in_array(strtolower($key),['auction','combination','presale','groupon','integral','seckill'])){
                $data[] = ['name' => strtolower($key), 'title' => $addonLangParams[WSTCurrLang()][strtolower($key)]];
            }
		}
		return $data;
	}
...
function WSTShopOrderMenus()
{
	...
	$orderMenus = array(
		"waitPay" => "shop/orders/waituserPay",
...
{
	$searchKeys = [];
	$goods = Db::name('goods_langs')->where(['goodsId' => $goodsId])->select();
	...
	$specs = Db::name('spec_cats')->alias('gc')
	        ...
	        ->join('__SPEC_ITEMS_LANGS__ scl','scl.itemId=sit.itemId','inner')
	...
	$attrs = Db::name('attributes')->alias('a')->join('goods_attributes ga', 'a.attrId=ga.attrId', 'inner')
		...
		->where(['a.isShow' => 1, 'dataFlag' => 1, 'ga.goodsId' => $goodsId])->field('gal.attrVal')
		->select();
	if (count($attrs) > 0) {
...
function WSTShopCats($parentId, $shopId = 0)
{
	...
	$dbo = Db::table('__SHOP_CATS__')->alias('a')
	         ->join('__SHOP_CATS_LANGS__ scl','a.catId=scl.catId and langId='.WSTCurrLang())
	         ->where(['dataFlag' => 1, 'isShow' => 1, 'parentId' => $parentId, 'shopId' => $shopId]);
	return $dbo->field("scl.catName,scl.catId")->order('catSort asc')->select();
}
/**
 * 获取商城搜索关键字
 */
...
{
	$rs = Db::table('__GOODS_CATS__')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where(['c.catId' => $catId])->field("parentId,catName,c.catId")->find();
	$rs['catName'] = $rs['catName'];
	$data[] = $rs;
	if ($rs['parentId'] == 0) {
		krsort($data);
		return $data;
...
{
	$cats = Db::table('__ARTICLE_CATS__')->alias('ac')->join('__ARTICLE_CATS_LANGS__ al','al.catId=ac.catId and al.langId='.WSTCurrLang())
	          ->where(['ac.catType' => 1, 'ac.dataFlag' => 1, 'ac.isShow' => 1, 'ac.parentId' => 7])
		      ->field("al.catName,ac.catId")->order('ac.catSort asc')->limit($pnum)->select();
	if (!empty($cats)) {
		foreach ($cats as $key => $v) {
			$cats[$key]['articlecats'] = Db::table('__ARTICLES__')->alias('a')->join('__ARTICLES_LANGS__ al','a.articleId=al.articleId and langId='.WSTCurrLang())->where(['dataFlag' => 1, 'isShow' => 1, 'catId' => $v['catId']])
				->field("a.articleId, a.catId, al.articleTitle")->order('catSort asc,createTime asc')->limit($cnum)->select();
		}
	}
	return $cats;
}
...
function WSTSideCategorys()
{
	...
	// if (!$data) {
		$cats1 = Db::table('__GOODS_CATS__')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where([['dataFlag', '=', 1], ['isShow', '=', 1], ['parentId', '=', 0]])->field("catName,c.catId,catImg")->order('catSort asc')->select();
		if (count($cats1) > 0) {
			$ids1 = [];
			$ids2 = [];
			$cats2 = [];
			...
			$tmp2 = Db::table('__GOODS_CATS__')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where([['dataFlag', '=', 1], ['isShow', '=', 1], ['parentId', 'in', $ids1]])->field("catName,c.catId,parentId,catImg")->order('catSort asc')->select();
			if (count($tmp2) > 0) {
				foreach ($tmp2 as $key => $v) {
					$ids2[] = $v['catId'];
				}
				$tmp3 = Db::table('__GOODS_CATS__')->alias('c')->join('__GOODS_CATS_LANGS__ gcl','gcl.catId=c.catId and gcl.langId='.WSTCurrLang())->where([['dataFlag', '=', 1], ['isShow', '=', 1], ['parentId', 'in', $ids2]])->field("catName,c.catId,parentId,catImg")->order('catSort asc')->select();
				if (count($tmp3) > 0) {
					//组装第三级
...
function WSTShopMessageBox()
{
	...
	$msg = [];
	if ($USER['shopMoney'] < 0) {
		$msg[] = lang('account_arrears_reminder_1',[$USER['shopMoney']]);
	}
	if (($USER['noSettledOrderFee'] + $USER['paymentMoney']) < 0 && (($USER['shopMoney'] + $USER['noSettledOrderFee'] + $USER['paymentMoney']) < 0)) {
		$shopMoney = $USER['shopMoney'];
		$paymentMoney = (-1 * ($USER['noSettledOrderFee'] + $USER['paymentMoney']));
		$msg[] = lang('account_arrears_reminder_2',[$shopMoney,$paymentMoney]) ;
	}
	if ((strtotime($USER['expireDate']) - strtotime(date('Y-m-d'))) < 2592000) {
		$msg[] =  lang('account_arrears_reminder_2');
	}
	return implode('||', $msg);
}
...
function WSTTableJoin($table, $join = [],$where = [], $field = '*', $num = 1, $order = '')
{
	$db = Db::name($table)->alias('a');
	foreach ($join as $key => $v) {
		$db->join($v['table'],$v['relate'],isset($v['joinRelate'])?$v['joinRelate']:'inner');
	}
	if (!empty($where)) $db->where($where);
	if ($field != '*') $db->field($field);
	if ($num > 1) {
...
function WSTTargetTypes()
{
	...
	$targertTypes = [0 => lang('target_type_1'), 1 => lang('target_type_2'), 2 => lang('target_type_3')];
	if ($isOpenSupplier == 1) $targertTypes[3] = lang('target_type_4');
	return $targertTypes;
}
 ...
 * 创建js语言文件
 */
function WSTLangJs($langSet = 'zh-tw')
{
	$l = \think\facade\Lang::get(null);
	$js = '';
	if (empty($l)) {
		$js = "var WSTLang = {};";
	} else {
		$js = "var WSTLang = " . json_encode($l) . ';';
	}

	$module = request()->module();
	$path = '';
	$name = 'lang-' . $langSet . '.js';
	if ($module == 'admin') {
		$path = Env::get('root_path') . 'runtime' . DIRECTORY_SEPARATOR .'js'. DIRECTORY_SEPARATOR.$module . DIRECTORY_SEPARATOR ;
	} else {
		$style = (WSTConf('CONF.wst' . $module . 'Style') != '') ? WSTConf('CONF.wst' . $module . 'Style') : 'default';
		$path = Env::get('root_path') . 'runtime' . DIRECTORY_SEPARATOR . 'js'.DIRECTORY_SEPARATOR.$module . DIRECTORY_SEPARATOR  . $style . DIRECTORY_SEPARATOR;
	}
...
function WSTLangAddonJs()
{
	$dir =  scandir(Env::get('root_path') . 'addons');
	$l = [];
	foreach ($dir as $dirName) {
		if($dirName=='.' || $dirName=='..'|| $dirName=='.htaccess')continue;
		$dirPath = Env::get('root_path') . 'addons' . DS . $dirName . DS . 'lang';
		// 忽略不是文件夹，跳过上一级,lang,等
		if(!is_dir($dirPath))continue;
		$dirTemp = scandir($dirPath);

		// 遍历获取语言,根据语言标识分类
		foreach ($dirTemp as $langPath) {
			$sub_path = $dirPath . '/' . $langPath;
			if ($langPath == '.' || $langPath == '..') {
				continue;
			} else if (is_file($sub_path)) {
				$arr = explode('.', $langPath);
				$lang = $arr[0];
				if ($arr[1] == 'php') {
					$list = include($dirPath . DS . $lang . '.php');

					$l[$lang] = isset($l[$lang]) ?  array_merge($list, $l[$lang]) : $list;
				}
			}
		}
	...
	// 批量生产js
	foreach ($l as $key => $jsonLnag) {
		$js = '';
		if (empty($jsonLnag)) {
			$js = "var WSTAddonsLang = {};";
		} else {
			$js = "var WSTAddonsLang = " . json_encode($jsonLnag) . ';if(typeof WSTLang !== "undefined"){ WSTLang = Object.assign(WSTLang, WSTAddonsLang); }else{ WSTLang = WSTAddonsLang }';
		}
        $name = 'lang-' . $key . '.js';
		$path = Env::get('root_path') . 'runtime'. DS. 'js'.DS.'addons' . DS  ;
        if (!file_exists($path.$name)) {
	        if(!is_dir($path))mkdir($path,'0755',true);
			if ($fp = fopen($path.$name, 'w')) {
				fwrite($fp, $js);
...
function WSTLang($str, $array = [], $lang = '')
{
	// $sysLang = config('app.default_lang');
	// $langFile = Env::get('root_path').'wstmart'.DS.'lang'.DS.$sysLang.'.php';
 //    if(is_file($langFile)){
 //        Lang::load($langFile);
 //    }
	return lang($str, $array, $lang);
}

function WSTSwitchLang()
{
	...
	// 	setcookie('think_var', $lang,2592000,'/');
	// 	session('think_var',$lang);
	// }
	// if(session('think_var')!=''){
	// 	//echo "2:";
	// 	$lang = session('think_var');
	// 	setcookie('think_var', $lang,2592000,'/');
	// 	\think\facade\Lang::range($lang);
	// }else if(isset($_COOKIE['think_var'])){
	// 	//echo "3:";
	// 	$lang = $_COOKIE['think_var'];
	// }
	// //echo '-->'.$lang."==>".\think\facade\Lang::range();
	$lang =  \think\facade\Lang::range();
	// echo "4:".$lang;
	return $lang;
}

function WSTAareCodes(){
	$areaCodes = ['852','886','81','1'];
	return $areaCodes;
}

function WSTSysLangs($lang = ''){
   $l = Db::name('languages')->where(['status'=>1])->select();
   if($lang==''){
   	   return $l;
   }else{
       foreach ($l as $key => $v) {
       	   if($v['code']==strtolower($lang))return $v['id'];
       }
       return '';
   }
}

function WSTCurrLang(){
   $code = WSTSwitchLang();
   $langId = WSTSysLangs($code);
   return ($langId>0)?$langId:WSTConf('CONF.defaultSysLang');
}

function WSTMergeLang($rs,$isPage = false){
	if($isPage){
        if(count($rs['data'])>0){
	        $langId = WSTCurrLang();
	        foreach($rs['data'] as $key =>$v){
	        	$lg = $rs['data'][$key]['langParams'][$langId];
	        	$rs['data'][$key] = array_merge($rs['data'][$key],$lg);
	        }
	    }
	}else{
        if(count($rs)>0){
	        $langId = WSTCurrLang();
	        foreach($rs as $key =>$v){
	        	$lg = $rs[$key]['langParams'][$langId];
	        	$rs[$key] = array_merge($rs[$key],$lg);
	        }
	    }
	}
    return $rs;
...
function WSTGetPaypalClientId(){
	$paypal = model('common/Payments')->getPayment("paypals");
	if(!empty($paypal)){
