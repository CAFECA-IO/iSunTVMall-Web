...
function queryByPage(){
    ...
    var cols = [
        ...
        {title:WST.lang('label_goodsappraise_score_info'), name:'', width: 100,renderer:function(val,item,rowIndex){
        ...
        {title:WST.lang('label_goodsappraise_service_score'), name:'', width: 100,renderer:function(val,item,rowIndex){
        ...
        {title:WST.lang('label_goodsappraise_time_score'), name:'', width: 30,renderer:function(val,item,rowIndex){
        ...
        {title:WST.lang('reply'), name:'', width: 300,renderer:function(val,item,rowIndex){
                var html="";
                html+=WST.lang("goodsappraise_appraise")+[item['loginName']]+"："+item['content'];
                ...
                if(item['supplierReply']==null || item['supplierReply']==''){
                    html+="<textarea style=\"width:98%;height:80px;margin-bottom:2px;\" id=\"reply-"+item['gaId']+"\" ></textarea>\n" +
                        "              <a class=\"btn btn-primary\" onclick=\"reply(this,"+item['gaId']+")\"><i class='fa fa-mail-reply'></i>"+WST.lang('reply')+"</a>";
                }else{
                    html+="<p class=\"reply-content\">"+item['supplierReply']+"【"+item['replyTime']+"】</p>";
                }
                html+="</div>";
                return html;
    ...
    mmg = $('.mmg').mmGrid({height: h-100,indexCol: true, cols: cols,method:'POST',
        url: WST.U('supplier/goodsappraises/queryByPage'), fullWidthRows: true, autoLoad: true,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function reply(t,id){
 ...
 params.id=id;
 $.post(WST.U('supplier/goodsappraises/supplierReply'),params,function(data){
    var json = WST.toJson(data);
    if(json.status==1){
      var today = new Date();
