...
function(a) {
    a.GZSTLayout = {
        ...
        positionWay: function(h) {
            var f = null,
            g = [];
            var langId = WST.conf.sysLangs[WST.conf.sysLang]['id'];
            for (var e in menus) {
                if (menus[e]["list"] && menus[e]["list"].length) {
                        ...
                        if (menus[e]["list"][d]["list"] && menus[e]["list"][d]["list"].length) {
                            for (var c = 0; c < menus[e]["list"][d]["list"].length; c++) {
                                if (menus[e]["list"][d]["list"][c]["list"] && menus[e]["list"][d]["list"][c]["list"].length) {
                                    for (var b = 0; b < menus[e]["list"][d]["list"][c]["list"].length; b++) {
                                        if (menus[e]["list"][d]["list"][c]["list"][b]["menuId"] == h) {
                                            g.push(WST.TransLang(menus[e]["list"][d]["list"][c]["list"][b]['langParams'][langId]["menuName"]));
                                ...
                                if (menus[e]["list"][d]["list"][c]["menuId"] == h) {
                                    g.push(WST.TransLang(menus[e]["list"][d]["list"][c]['langParams'][langId]["menuName"]));
                                    f = menus[e]["list"][d];
                                    break
                                }
                            }
                            if (g.length > 0 && f["menuId"] == menus[e]["list"][d]["menuId"]) {
                                g.push(WST.TransLang(menus[e]["list"][d]['langParams'][langId]["menuName"]));
                                f = menus[e];
                                break
                            }
                        }
                    ...
                    if (g.length > 0 && f["menuId"] == menus[e]["menuId"]) {
                        g.push(WST.TransLang(menus[e]['langParams'][langId]["menuName"]));
                        f = menus[e];
                        break
                    }
                }
        ...
        initEvent: function() {
            ...
            WST.getSysMessages(WST.conf.MSG_SUPPLIER_GRANT);
            if(WST.conf.TIME_TASK=='1'){
                    setInterval(function(){
                        WST.getSysMessages(WST.conf.MSG_SUPPLIER_GRANT);
                },10000);
            }
            WST.dropDownLayer("#lang",".j-dorpdown-layer");
            WST.dropDownLayer("#toMsg",".j-dorpdown-layer");
            WST.dropDownLayer("#toUser",".j-dorpdown-layer");
            $('#toClearCache').click(function(){
                clearCache();
            });
        }
    };
...
function logout() {
    var a = WST.msg(WST.lang('logout_loading'), {
        icon: 16,
        time: 60000,
        offset: "200px"
    });
    $.post(WST.U("supplier/index/logout"), {},
    function(c, d) {
        layer.close(a);
        var b = WST.toJson(c);
        if (b.status == "1") {
            ...
            function() {
                location.href = WST.U("supplier/index/login")
            })
        } else {
            WST.msg(b.msg, {
                icon: 2,
...
function clearCache() {
    var a = WST.msg(WST.lang('clear_supplier_caceh_loading'), {
        icon: 16,
        time: 60000,
        offset: "200px"
    });
    $.post(WST.U("supplier/index/clearCache"), {},
    function(c, d) {
        layer.close(a);
        var b = WST.toJson(c);
        if (b.status == "1") {
        ...
        yes: function(c, b) {
            $("#editPassFrom").isValid(function(d) {
                if (d) {
                    ...
                    var e = WST.msg(WST.lang('loading'));
                    $.post(WST.U("supplier/users/passedit"), f,
                    function(h) {
                        layer.close(e);
                        var g = WST.toJson(h);
                        if (g.status == 1) {
