...
$(function() {
    ...
    $('.suppliersImg').lazyload({ effect: "fadeIn",failurelimit : 10,skip_invisible : false,threshold: 100,placeholder:window.conf.RESOURCE_PATH+'/'+window.conf.SUPPLIER_LOGO});//供货商默认头像
...
WST.upload = function(opts){
	var _opts = {};
	_opts = $.extend(_opts,{duplicate:true,auto: true,swf: WST.conf.STATIC +'/plugins/webuploader/Uploader.swf',server:WST.U('supplier/index/uploadPic'),duplicate:true},opts);
...
WST.getVerify = function(id){
    $(id).attr('src',WST.U('supplier/index/getVerify','rnd='+Math.random()));
}
WST.getAreas = function(obj,id,val,fval,callback){
	...
	$.post(WST.U('supplier/areas/listQuery'),params,function(data,textStatus){
...
WST.ITSetGoodsCats = function(opts){
	...
	if(opts.childIds.length>0){
		...
		$.post(WST.U('supplier/goodscats/listQuery'),{parentId:opts.val},function(data,textStatus){
		     var json = WST.toJson(data);
...
WST.ITAreas = function(opts){
	...
	$.post(WST.U('supplier/areas/listQuery'),{parentId:opts.val},function(data,textStatus){
	     var json = WST.toJson(data);
	     if(json.data && json.data.length>0){
	    	 json = json.data;
	         var html = [],tmp;
...
WST.suppliersCats = function(objId,pVal,objVal){
	$('#'+objId).empty();
...
WST.getSysMessages = function(val){
	...
	$.post(WST.U('supplier/index/getSysMessages'),{tasks:val},function(data){
		var json = WST.toJson(data);
		var totalNum = 0
		if(json.message){
			$('#m-msg').hide();
		...
		if(json.supplierorder){
			for(var key in json.supplierorder){
				if($('#m-'+key)[0]){
					$('#m-'+key).hide();
					if(json.supplierorder[key]!='0'){
						$('#m-'+key).show();
					    $('#m-'+key+" span").html(json.supplierorder[key]);
					}
				}
				totalNum += parseInt(json.supplierorder[key],10);
			}
			if(newOrderNum<parseInt(json.supplierorder['515'],10)){
				...
				newOrderNum = parseInt(json.supplierorder['515'],10);
			}
		}
		if(totalNum>0){
			$('.msg-num').show();
