...
class SupplierLogOperates extends Base{
	...
	public function pageQuery(){
        ...
        $supplierId = session('WST_SUPPLIER.supplierId');
		$startDate = input('startDate');
		$endDate = input('endDate');
        $loginName = input('loginName');
        $operateUrl = input('operateUrl');
		$where = [];
		$where[] = [' l.supplierId','like',$supplierId];
	...
	public function add($param){
		$data = [];
		$data['userId'] = (int)session('WST_SUPPLIER.userId');
		...
		$data['supplierId'] = (int)session('WST_SUPPLIER.supplierId');
		$this->create($data);
	}
	...
	public function getById($id){
		...
		return WSTReturn(lang("no_find_log_operate"), -1);
	}
}
