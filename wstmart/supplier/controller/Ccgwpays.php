...
class Ccgwpays extends Base{
	...
	public function initialize() {
		...
		$this->payConfig['notifyurl'] = url("supplier/ccgwpays/ccgwNotify","",true,true);
	...
	public function getBtc(){
		...
		if($pkeys[0]=="recharge"){
            ...
            $catSupplierInfo = model('supplier/suppliers')->getCatShopInfo();
            $needPay = (int)$catSupplierInfo['needPay'];
        }
	...
	public function getCcgwPaysURL(){
		...
		$data["url"] = url('supplier/ccgwpays/createQrcode',array("pkey"=>base64_encode($pkey)));
		return $data;
	}
	...
	public function getPayStatus() {
		...
		$obj = array();
		$obj["userId"] = (int)session('WST_SUPPLIER.supplierId');
	...
	public function ccgwNotify() {
		...
		$qrcodePay = new \CcgwQrcodePay ();
		$bkData = input();
		file_put_contents("runtime/ccgwpay_supplier_log.txt",date('Y-m-d H:i:s')."----ttbb----".json_encode($bkData)."----\n", FILE_APPEND);
		$ccTxid = $bkData['txid'];
