...
class Base extends Controller {
	public function __construct(){
		parent::__construct();
        $lang = WSTSwitchLang();
        // \think\facade\Lang::load(APP_PATH . 'supplier' . DS . 'lang' . DS . $lang . '.php');
		...
		$this->view->filter(function($content){
			$content = str_replace("__SUPPLIER__",str_replace('/index.php','',$this->request->root()).'/wstmart/supplier/view/default',$content);
    ...
    */
    ...
    protected function checkAuth(){
        ...
        if(!empty($USER) && $USER['userType']==3){
            ...
            if(!$USER['SUPPLIER_MASTER']){
                $request = request();
                $visit = strtolower($request->module()."/".$request->controller()."/".$request->action());
                if(!in_array($visit,$USER['visitPrivilegeUrls'])){
                    $msg = lang("no_privelege");
                    if(request()->isAjax()){
                        die('{"status":-998,"msg":"'.$msg.'"}');
                    }else{
                        die($msg);
                    }
                }
