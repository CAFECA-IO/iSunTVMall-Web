<?php
namespace wstmart\supplier\behavior;
/**
 * ============================================================================
 * WSTMart多用户商城
...
class ListenOperate 
{
    public function run($params){
        if(session("WST_SUPPLIER.supplierId")>0){
            $urls = session("WST_SUPPLIER.supplierMenuMaps");
            $request = request();
            $visit = strtolower($request->module()."/".$request->controller()."/".$request->action());
            $expurls = ['supplier/supplierlogoperates/index','supplier/supplierlogoperates/pagequery'];
            if(array_key_exists($visit,$urls) && !in_array($visit,$expurls)){
                ...
                $data['operateSrc'] = 1;
                model('supplier/SupplierLogOperates')->add($data);
            }
        }
    }
}