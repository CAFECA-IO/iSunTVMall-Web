...
class Users extends Base
{
    ...
    public function checkEmail()
    {
        ...
        if (isset($data['error'])) $data['error'] = lang('e_mail_exists');
        return $data;
    }
    ...
    public function getPhoneVerifyo()
    {
        ...
        if (!WSTIsPhone($userPhone)) {
            return WSTReturn(lang('incorrect_format'));
            exit();
        }
        $rs = array();
        $m = new MUsers();
    ...
    public function getEmailVerify()
    {
        ...
        if (!$userEmail) {
            return WSTReturn(lang('please_input_email'), -1);
        }
        $code = input("post.verifyCode");
        $process = input("post.process");
        if (!WSTVerifyCheck($code)) {
            return WSTReturn(lang('verification_code_error'), -1);
        }
        ...
        $code = rand(0, 999999);
        $sendRs = ['status' => -1, 'msg' => lang('op_err')];
    ...
    public function emailEdit()
    {
        ...
        if ($code != $secretCode) return WSTReturn(lang('verification_code_error'), -1);
    ...
    public function phoneEdito()
    {
        ...
        if (!session('Verify_info.phoneVerify') || time() > floatval($timeVerify) + 10 * 60) {
            return WSTReturn(lang('address_invalid'));
            exit();
        }
        ...
        return WSTReturn(lang('verification_code_is_invalid'));
    }
    ...
    public function editPhoneSut()
    {
        ...
        $this->error(lang('address_invalid'));
    }
    /**
     * 处理图像裁剪
     */
