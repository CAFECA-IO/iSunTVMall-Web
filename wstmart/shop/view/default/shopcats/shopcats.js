...
function initGrid(){
	grid = $('#maingrid').WSTGridTree({
		url:WST.U('shop/shopcats/pageQuery'),
		rownumbers:true,
		columns: [
			{ display: WST.lang('label_cat_name'), name: 'catName', id:'catId', align: 'left',isSort: false},
			...
			{ display: WST.lang('sort')+WST.lang('double_click_to_modify'), name: 'catSort',width: 160,isSort: false,render:function(rowdata){
...
function toggleIsShow(id,isShow){
	...
	$.post(WST.U('shop/shopcats/changeCatStatus'),{id:id,isShow:isShow},function(data,textStatus){
		layer.close(loading);
...
function delCat(id){
	var box = WST.confirm({content:WST.lang('delete_goods_cat'),yes:function(){
			...
			$.post(WST.U('shop/shopcats/del'),{id:id},function(data,textStatus){
				layer.close(loading);
				var json = WST.toJson(data);
				if(json.status=='1'){
...
function changeCatStatus(isShow,id,pid){
	...
	params.pid = pid;
	$.post(WST.U('shop/shopcats/changeCatStatus'),params,function(data,textStatus){
		location.reload();
	});
...
function toEdit(pid,id){
	$('#catForm')[0].reset();
	if(id>0){
		$.post(WST.U('shop/shopcats/get'),{id:id},function(data,textStatus){
			var json = WST.toJson(data);
			if(json){
				WST.setValues(json);
				if(json.langs){
...
function editsBox(id){
	...
	for(var i in WST.conf.sysLangs){
		...
		fields['langParams'+n+'catName'] = {
			...
			rule: WST.lang('label_cat_name')+':required;length[~50];'
		}
	}
	fields['catSort'] = {
	...
	$('#catForm').validator({
		...
		valid: function(form){
			...
			params.id = id;
			var loading = WST.msg(WST.lang('loading'), {icon: 16,time:60000});
			$.post(WST.U('shop/shopcats/'+((id>0)?"edit":"add")),params,function(data,textStatus){
				layer.close(loading);
				var json = WST.toJson(data);
				if(json.status=='1'){
					WST.msg(json.msg,{icon:1});
...
function doneChange(t,id){
	...
	$.post(WST.U('shop/shopcats/editSort'),{id:id,catSort:sort},function(data){
		var json = WST.toJson(data);
		if(json.status==1){
			WST.msg(json.msg,{icon:1});
			$(t).parent().attr('ondblclick','changeSort(this,'+id+')');
