...
function stockByPage(p){
    ...
    var cols = [
        {title:WST.lang('product_picture'), name:'goodsName', width: 40, renderer: function(val,item,rowIndex){
        ...
        {title:WST.lang('trade_name'), name:'goodsName', width: 200},

        {title:WST.lang('product_code'), name:'productNo', width: 100},
        {title:WST.lang('specifications'), name:'', width: 150,renderer:function(val,item,rowIndex){
                if(item['isSpec']==1){
                	var spec="";
                    ...
                    return "<span class='statu-wait'>"+WST.lang('nothing')+"</span>";
                }
            }},
        {title:WST.lang('stock'), name:'goodsStock', width: 30,renderer:function(val,item,rowIndex){
                var goodsStock="";
                if(item['isSpec']==1){
        ...
        {title:WST.lang('early_warning'), name:'warnStock', width: 30,renderer:function(val,item,rowIndex){
                var goodsStock="";
                if(item['isSpec']==1){
        ...
        {title:WST.lang('op'), name:'' ,width:200, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
                if(item['goodsType']==1)h += "<a class='btn btn-blue' href='javascript:toStock(" + item['goodsId'] + ",\"stockWarnByPage\")'><i class='fa fa-pencil'></i>"+WST.lang('stockwarn')+"</a>";
                h += " <a class='btn btn-blue' href='javascript:toEdit(" + item['goodsId'] + ",\"stockwarnbypage\")'><i class='fa fa-pencil'></i>"+WST.lang('commodity_editor')+"</a> ";
                return h;
            }}
    ];
...
function editGoodsStock(id,type,goodsId){
	...
	if($.trim(number)==''){
		WST.msg(WST.lang('stoclwarn'), {icon: 5});
        return;
	}
	var params = {};
	params.id = id;
