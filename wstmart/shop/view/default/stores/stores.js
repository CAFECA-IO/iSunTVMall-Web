...
function add(p){
	$('#editForm').isValid(function(v){
		if(v){
		    ...
		    $.post(WST.U('shop/stores/toAdd'),params,function(data,textStatus){
		    	layer.close(loading);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
		    		WST.msg(json.msg,{icon:1},function(){
						location.href=WST.U('shop/stores/index',"p="+p);
					});
		    	}else{
...
function edit(p){
	$('#editForm').isValid(function(v){
		if(v){
	        ...
	        params.areaId = WST.ITGetAreaVal('j-areaIdPath');
			var loading = WST.msg(WST.lang('loading'), {icon: 16,time:60000});
		    $.post(WST.U('shop/stores/toEdit'),params,function(data,textStatus){
		    	layer.close(loading);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
		    		WST.msg(json.msg,{icon:1},function(){
