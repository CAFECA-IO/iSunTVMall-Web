...
WST.checkCart = function(){
	$('#list-carts2').html('');
	$('#list-carts3').html('');
	$('#list-carts').html('<div style="padding:32px 0px 77px 112px;"><img src="'+WST.conf.ROOT+'/wstmart/shop/view/default/supplier/img/loading.gif">{:lang("loading")}</div>');
	$.post(WST.U('shop/suppliercarts/getCartInfo'),'',function(data) {
		...
		if(json.status==1){
			...
			if(json.list.length>0){
				...
				$('#list-carts2').html('<div class="comm" id="list-comm">&nbsp;&nbsp;'+WST.lang('supplier_tips12', [json.goodsTotalNum])+'<span class="span2">'+WST.lang('currency_symbol')+json.goodsTotalMoney+'</span></div>');
				$('#list-carts3').html('<a href="'+window.conf.ROOT+'/shop/suppliercarts/index" class="btn btn-3">'+WST.lang('supplier_tips11')+'</a>');
				...
				$('#list-carts').html('<p class="carts">'+WST.lang('supplier_tips10')+'</p>');
			}
...
WST.supplierQQ = function(val){
	if(WST.blank(val) !=''){
      return [
              '<a href="tencent://message/?uin='+val+'&Site=QQ&Menu=yes">',
		      '<img border="0" src='+window.conf.__HTTP__+'wpa.qq.com/pa?p=1:'+val+':7" alt="QQ" width="71" height="24" />',
		      '</a>'
		      ].join('');
	}else{
		return '';
...
WST.supplierWangWang = function(val){
	if(WST.blank(val) !=''){
		return [
		       ...
		       '<img border="0" src='+window.conf.__HTTP__+'amos.alicdn.com/realonline.aw?v=2&uid='+val+'&site=cntaobao&s=1&charset=utf-8" alt="'+WST.lang('contact_me')+'" />',
	           '</a>'
		       ].join('');
	}else{
		return '';
