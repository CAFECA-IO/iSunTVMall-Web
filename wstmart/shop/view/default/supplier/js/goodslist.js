$(function(){
	...
	$('.item-more').click(function(){
		if($(this).attr('v')==1){
			...
			$(this).find("span").html(WST.lang('put_it_away'));
			$(this).find("i").attr({"class":"drop-up"});
			$(this).attr('v',0);
		}else{
			$('.hideItem').hide(300);
			$(this).find("span").html(WST.lang('supplier_tips21'));
...
function goodsFilter(obj,vtype){
	...
	location.href=WST.U('shop/supplierindex/lists',params.join('&'),true);
}
...
function searchFilter(obj,vtype){
	...
	location.href=WST.U('shop/supplierindex/index',params.join('&'),true);
}
...
function choiceArea(t,pid){
	...
	if(ids[2]==3){
		...
		var url = ($(t).attr('search')==1)?'shop/supplierindex/index':'shop/supplierindex/lists';
		...
		// 替换下级地图标题
		$(nextid).html('<a href="javascript:void(0)">'+WST.lang('select')+'</a>');

		// 获取下级地区信息
		$.post(WST.U('shop/areas/listQuery'),{parentId:pid},function(data){
			// 判断搜索页面
...
function extra_brand_hide(obj){
	...
	$(obj).html(WST.lang('more')+'<i></i>');
}
function extra_show(obj){
	$(obj).addClass('extra_more_on');
	$(obj).parent().parent().find('ul').css({height:'auto'});
	$(obj).html(WST.lang('put_it_away')+'<i></i>');
...
function multibox_show(obj,type){
	...
	_topParent.append('<div class="multi_btns"><a class="confirm_btn" href="javascript:void(0)" onClick="multi_done(this,\''+type+'\')">'+WST.lang('ok')+'</a><a onClick="multibox_hide(this)" href="javascript:void(0)">'+WST.lang('cancel')+'</a></div>')
	// 隐藏上一个多选盒子
...
function multi_done(obj,type){
	...
	if(ids.length==0){
		WST.msg(WST.lang('supplier_tips22'));
		return false;
	}
	if(type=='brand'){
		$('#brand').val(ids.join(','));
...
function multibox_hide(obj){
	...
	_moreBtn.html(WST.lang('more')+'<i></i>');
}
