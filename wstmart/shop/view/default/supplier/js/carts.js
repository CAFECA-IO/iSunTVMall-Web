	...
	$('.j-gchk').each(function(){
		...
		if(stockNum < 0 || stockNum < cartNum){
			...
			if(stockNum < 0){
				...
				$('#err_'+cartId).css('color','red').html(WST.lang('insufficient_inventory_of_goods'));
			}else{
				$('#gchk_'+cartId).attr('allowbuy',1);
				$('#err_'+cartId).css('color','red').html(WST.lang('supplier_tips1'));
			}
		}else{
...
function toSettlement(){
	...
	if(!isChk){
		WST.msg(WST.lang('please_select_the_goods_to_be_settled'),{icon:1});
		return;
	}
	var msg = '';
	$('.j-gchk').each(function(){
		if($(this).prop('checked')){
			if($(this).attr('allowbuy')==0){
				msg = WST.lang('supplier_tips2');
				return;
			}else if($(this).attr('allowbuy')==1){
				msg = WST.lang(supplier_tips3);
				return;
			}
		}
	})
...
function delAddr(id){
	WST.confirm({content:WST.lang('are_you_sure_to_delete'),yes:function(index){
	     ...
	     if(json.status==1){
	    	 if(json.data && json.data && json.data.length){
	    		 ...
	    		 for(var i=0;i<json.data.length;i++){
	    			...
	    			if(tmp.isDefault!=1){
	    				html.push('<a href="javascript:;" onclick="setDeaultAddr('+tmp.addressId+')">'+WST.lang('set_as_default_address')+'</a>&nbsp;&nbsp;');
	    			}
		...
		var json = WST.toJson(data);
	     if(json.status==1){
	    	 ...
	    	 if(params.isDefault==1){
	    		 $('#isdefault').html(WST.lang('default_address')).addClass('j-default');
	    	 }else{
...
function invAdd(){
	...
	$('.inv_ul').append('<li class="inv_li inv_li_curr inv_editing"><input type="text" id="invoiceHead" placeholder="'+WST.lang('supplier_tips4')+'" value="" style="width:65%;height:21px;padding:1px;"><i></i><div style="top:8px;" class="inv_opabox"><a href="javascript:void(0)" onCLick="addInvoice()">'+WST.lang('save')+'</a></div></li>');
...
function addInvoice(){
	...
	if(head.length==0){
		WST.msg(WST.lang('supplier_tips5'));
		return;
	}
	var loading = WST.load({msg:WST.lang('loading')});
...
function setInvoiceText(invoiceHead){
	...
	var text = WST.lang('supplier_tips6');
	if(isInvoice==1){
		text = (invoiceObj==0)?WST.lang('supplier_tips7'):WST.lang('supplier_tips8', [invoiceHead]);
	}
...
function getPayUrl(){
	...
	jQuery.post(WST.U('shop/supplier'+params.payCode+'/get'+params.payCode+"Url"),params,function(data) {
		...
		if(json.status==1){
			...
			WST.msg(WST.lang('Your_order_has_been_paid'), {icon: 5,time:1500},function(){
...
function setPaypwd(){
	layerbox =	layer.open({
		...
		btn: [WST.lang('supplier_tips9'), WST.lang('close')],
