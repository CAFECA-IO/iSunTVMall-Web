...
$(function(){
	...
	$('.item-more').click(function(){
		if($(this).attr('v')==1){
			...
			$('.hideItem').hide(300);
			$(this).find("span").html(WST,lang('more'));
...
function addCart(type,iptId){
	...
	if(goodsInfo.isSpec==1){
		...
		if(specIds.length==0){
			WST.msg(WST.lang('supplier_tips13'),{icon:2});
		}
		specIds.sort(function(a,b){return a-b});
		if(goodsInfo.sku[specIds.join(':')]){
		...
		if(json.status==1){
			if(json.data.hasCopy){
				layer.open({
				    ...
				    btn: [WST.lang('supplier_tips17'),WST.lang('supplier_tips18')],
				...
				WST.confirm({content:WST.lang('supplier_tips19'),yes:function(){
			       	loading = WST.msg(WST.lang('loading'), {icon: 16,time:60000});
			       	toCopyGoods();
			    }});
			}
	...
	$.post(WST.U('shop/Suppliergoods/toAdd'),{supplierGoodsId:goodsId},function(data,textStatus){
      	...
      	if(json.status==1){
          	layer.open({
			    ...
			    btn: [WST.lang('supplier_tips20'),WST.lang('supplier_tips18')],
			    yes: function(index, layero){
			    	layer.close(index);
			    	location.href = WST.U('shop/goods/store');
			    },
