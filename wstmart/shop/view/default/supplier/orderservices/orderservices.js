function queryPage(p){
	...
	params.page = p;
	$.post(WST.U('shop/supplierorderservices/pagequery'),params,function(data,textStatus){
		$('#loading').hide();
...
function userExpress(){
    ...
    if(params.expressType==1){
        if(!params.expressId || params.expressId==0){
            return WST.msg(WST.lang('please_select_logistics_company'));
        }
        // 需要快递
        if(!params.expressNo || params.expressNo.length==0){
            return WST.msg(WST.lang('please_fill_in_the_logistics_number'));
        }
    }
    params.id = $('#id').val();
    $.post(WST.U('shop/supplierorderservices/userExpress'),params,function(res){
...
function userConfirm(){
    ...
    if(postData.isUserAccept==-1 && postData.userRejectType==0){
        return WST.msg(WST.lang('please_select_rejection_type'));
    }
    if(postData.userRejectType=='10000' && postData.userRejectOther.length==0){
        return WST.msg(WST.lang('please_input_rejection_reason'));
    }
    $.post(WST.U('shop/supplierorderservices/userReceive'),postData,function(res){
        var json = WST.toJson(res);
        WST.msg(json.msg);
        if(json.status==1){
            // 刷新当前页
