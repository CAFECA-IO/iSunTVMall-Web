  ...
  function commitOrderService(){
    ...
    if(postData.serviceType==-1){
      return WST.msg(WST.lang('please_select_the_reason_for_submission'));
    }
    if(postData.serviceRemark.length==0){
      return WST.msg(WST.lang('please_enter_the_problem_description'));
    }
    if(postData.goodsServiceType!=2 && postData.refundMoney<0){
      return WST.msg(WST.lang('please_enter_the_refund_amount'));
    }
    ...
    if(_gids.length==0){
      return WST.msg(WST.lang('please_check_at_least_one_item'));
    }
    postData.ids = _gids.join(',');
    $.extend(postData,_glist);
    $.post(WST.U('shop/supplierorderservices/commit'), postData, function(res){
  ...
  function getCanRefundMoney(){
    ...
    $.post(WST.U('shop/supplierorderservices/getRefundableMoney'),_glist,function(res){
      var json = WST.toJson(res);
      if(json.status==1){
