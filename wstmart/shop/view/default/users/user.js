...
function checkCoords()
{
  //判断是否有裁剪
  if (parseInt($('#w').val())){
    var loading = WST.msg(WST.lang('handle_image'), {icon: 16,shade: [0.3, '#999999']});
    /*获取裁剪数据*/
    var photoData = {};
    photoData.x = $('#x').val();
    photoData.y = $('#y').val();
    ...
    $.post(WST.U('supplier/users/editUserPhoto'),photoData,function(data, textStatus){
      if(data.status==1)
      {
        layer.close(loading);
        //将上传的图片路径赋给全局变量
  ...
  WST.msg(WST.lang('cut_image_error'),{icon:2});
  return false;
}
  ...
  WST.upload({
    ...
    callback:function(f){
      ...
      if(json.status==1){
        ...
        cutCode += '<div><p align="center">'+WST.lang('cut_preview')+'</p>';
        cutCode += '<div class="cut-help"><h4>'+WST.lang('operation_help')+'</h4>';
        cutCode += '<p>'+WST.lang('cut_tips')+'</p></div></div>';
...
$(function(){
  ...
  $('#userEditForm').validator({
          fields: {
              userName: {rule:"required",msg:{required:WST.lang('require_user_name')},tip:WST.lang('require_user_name_tip')},
              userSex:  {rule:"checked;",msg:{checked:WST.lang('require_user_sex')},tip:WST.lang('require_user_sex_tip')}
          },
        valid: function(form){
          ...
          $.post(WST.U('supplier/Users/toEdit'),params,function(data,textStatus){
            layer.close(loading);
