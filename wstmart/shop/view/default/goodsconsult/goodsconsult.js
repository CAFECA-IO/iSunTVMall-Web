function queryByPage(p){
    ...
    var cols = [
        ...
        {title:WST.lang('goods'), name:'goodsName', width: 200},

        {title:WST.lang('show'), name:'', width: 10, align:'center',renderer:function(val,item,rowIndex){
            if(item['isShow']==1){
                return "<span class='statu-yes'><i class='fa fa-check-circle'></i>"+WST.lang('show')+"</span>";
            }else{
                return "<span class='statu-no'><i class='fa fa-ban'></i>"+WST.lang('hide')+"</span>";
            }
        }},
        {title:WST.lang('consultation_content'), name:'goodsName', width: 300,renderer:function(val,item,rowIndex){
            var html="";
            if(WST.blank(item['loginName'])==''){
                html+=WST.lang('tourist')+"："+item['consultContent'];
            }else{
                ...
                if(item['reply']==null || item['reply']==''){
                   html+="<div><textarea style=\"width:98%;height:80px;margin-bottom:2px;\" id=\"reply-"+item['id']+"\" placeholder='"+WST.lang('goods_sconsult_tips1')+"' maxlength='200'></textarea>\n" +
...
function reply(t,id){
 var params = {};
 if($('#reply-'+id).val()==''){
    WST.msg(WST.lang('reply_content_cannot_be_empty'),{icon:2});
    return false;
 }
