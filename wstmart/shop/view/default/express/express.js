    ...
    var cols = [
        ...
        {title:WST.lang('op'), name:'' ,width:300, align:'center', renderer: function(val,item,rowIndex){
            ...
            if(item['isEnable'] == 1){
            	...
            	h += "<a class='btn btn-red' href='javascript:disableExpress(" + item['id'] + ")'><i class='fa fa-circle-o-notch'></i>"+WST.lang('not_enabled')+"</a> ";
            }else{
                h += "<a class='btn btn-blue' href='javascript:enableExpress(" + item['expressId'] + ")'><i class='fa fa-check-circle'></i>"+WST.lang('enabled')+"</a>";
            }
            return h;
        }}
    ];
...
function disableExpress(id){
	var c = WST.confirm({content:WST.lang('are_you_sure_you_want_to_stop'),yes:function(){
		layer.close(c);
		var load = WST.load({msg:WST.lang('loading')});
...
function freightAreas(){
	var box = WST.open({
		...
		area: ['700px', '500px'],
		btn:[WST.lang('ok'),WST.lang('cancel')],
		yes:function(){
			var provinceNames = []; 
			var provinceIds = []; 
			var cityIds = [];
		...
		cancel:function(){
			...
			$("#freightAreas").append("<div id='chkAllBox' style='display: none;'><div class='tpox'><input type='checkbox' name='chkAll' lay-skin='primary' title='"+WST.lang('Select_all/none')+"' "+(isChkAll?"checked":"")+" lay-filter='chooseAll' style='display: none;' /></div></div>")
			layui.form.render();
		},
...
function del(id){
	WST.confirm({content:WST.lang('are_you_sure_to_delete'), yes:function(tips){
