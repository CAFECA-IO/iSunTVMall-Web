...
function beforeCommit(){
    ...
    if(isShopAgree!==0 && isShopAgree!==1){
        return WST.msg(WST.lang('please_choose_whether_to_accept_or_not'));
    }
...
function refund(){
    ...
    if(postData.isShopAgree==0){
        ...
        if(disagreeRemark.length==0){
            return WST.msg(WST.lang('please_input_the_reason_for_not_accepting'));
        }
...
function commit(){
    ...
    if(postData.isShopAgree==1){
        ...
        if(shopAddress.length==0){
            return WST.msg(WST.lang('merchant_receiving_address_cannot_be_empty'));
        }
        if(shopName.length==0){
            return WST.msg(WST.lang('merchant_consignee_cannot_be_empty'));
        }
        if(shopPhone.length==0){
            return WST.msg(WST.lang(WST.lang('merchant_contact_cannot_be_empty')));
        }
...
function receive(p){
    ...
    if(postData.shopRejectType=='10000' && postData.shopRejectOther.length==0){
        return WST.msg(WST.lang('please_input_rejection_reason'));
    }
...
function send(p){
    ...
    if(postData.shopExpressType==1){
        if(postData.shopExpressId==0)return WST.msg(WST.lang('please_select_logistics_company'));
        if(postData.shopExpressNo.length==0)return WST.msg(WST.lang('please_input_logistics_number'));
    }
