...
function initGrid(p){
    ...
    var cols = [
        {title:WST.lang('news'), name:'msgContent' ,width:700,sortable:true,renderer: function(val,item,rowIndex){
            if(item['msgStatus'] == 0){
        ...
        {title:WST.lang('date'), name:'createTime' ,width:90,sortable:true},
        {title:WST.lang('op'), name:'' ,width:100, align:'center', renderer: function(val,item,rowIndex){
            var h = "";
            h += "<a  class='btn btn-blue' onclick='javascript:showMsg("+item['id']+")'><i class='fa fa-search'></i>"+WST.lang('see')+"</a> ";
...
function batchDel(){
    ...
    if(rows.length==0){
        WST.msg(WST.lang('please_select_the_message_to_delete'),{icon:2});
        return;
    }
    var ids = [];
...
function batchRead(){
    var rows = mmg.selectedRows();
    if(rows.length==0){
        WST.msg(WST.lang('please_select_the_message_to_process'),{icon:2});
        return;
    }
    var ids = [];
    for(var i=0;i<rows.length;i++){
