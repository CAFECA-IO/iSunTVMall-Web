...
class SupplierOrders extends Base
{
	...
	public function submit($orderSrc = 0)
	{
		...
		if ($userId == 0) return WSTReturn(lang('not_logged'));
		$shopId = (int)session('WST_USER.shopId');
		//检测购物车
		$carts = model('supplierCarts')->getCarts(true, $userId);
		if (empty($carts['carts'])) return WSTReturn(lang('please_select_a_valid_stock_item'));
		...
		try {
			...
			Db::commit();
			return WSTReturn(lang('op_ok'), 1, $orderunique);
		} catch (\Exception $e) {
			//print_r($e);
			Db::rollback();
	...
	public function userOrdersByPage($orderStatus, $isAppraise = -1)
	{
		...
		if (count($page['data']) > 0) {
			...
			foreach ($page['data'] as $key => $v) {
				...
				$page['data'][$key]['deliverTypeName'] = WSTLangDeliverType($v['deliverType']);
		...
		if (!empty($order)) {
			...
			try {
				...
				if (false != $result) {
					...
					$logOrder['logJson'] = json_encode(['type'=>'lang','key'=>'customer_received']);
					$logOrder['logUserId'] = $userId;
					$logOrder['logType'] = 0;
					$logOrder['logTime'] = date('Y-m-d H:i:s');
					Db::name('supplier_log_orders')->insert($logOrder);
				...
				Db::rollback();
				return WSTReturn(lang('op_err') . $e->getMessage(), -1);
			}
		}
	...
	public function cancel()
	{
		...
		if (empty($reasonData)) return WSTReturn(lang('invalid_cancellation_reason'));
	...
	public function reject()
	{
		...
		if (empty($reasonData)) return WSTReturn(lang('invalid_rejection_reason'));
		if ($reason == 10000 && $content == '') return WSTReturn(lang('please_input_rejection_reason'));
		if (!empty($order)) {
			Db::startTrans();
			try {
	...
	public function checkOrderPay()
	{
		...
		if (count($rs) > 0) {
			...
			return WSTReturn(lang('order_paid'), -1);
		}
	}
		...
		try {
			...
			if ($needPay > 0 && false != $rs) {
				foreach ($orders as $key => $v) {
					...
					$lm['dataId'] = $orderId;
					$lm['dataSrc'] = 1;
					$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'transaction_order_recharge', 'params'=>[$v['orderNo'], $needPay]]);
					$lm['moneyType'] = 1;
					$lm['money'] = $needPay;
					$lm['payType'] = $payFrom;
					$lm['tradeNo'] = $trade_no;
					...
					$lm['dataId'] = $orderId;
					$lm['dataSrc'] = 1;
					$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'transaction_order_expense', 'params'=>[$v['orderNo'], $needPay]]);
					$lm['moneyType'] = 0;
					$lm['money'] = $needPay;
					$lm['payType'] = 0;
					$lm['createTime'] = date('Y-m-d H:i:s');
				...
				$lm['dataId'] = $orderNo;
				$lm['dataSrc'] = 1;
				$lm['remark'] =  json_encode(['type'=>'lang_params','key'=>'$payMoney', 'params'=>[$payMoney]]);
				$lm['moneyType'] = 1;
				$lm['money'] = $payMoney;
				$lm['payType'] = $payFrom;
				$lm['tradeNo'] = $trade_no;
	...
	public function toExport()
	{
		...
		$objPHPExcel->getProperties()->setCreator("WSTMart") //创建人
			...
			->setKeywords(WSTLang('orders')) //关键字
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', WSTLang('order_number'))->setCellValue('B1', WSTLang('order_status'))->setCellValue('C1', WSTLang('consignee'))->setCellValue('D1', WSTLang('receiving_address'))->setCellValue('E1', WSTLang('contact_information'))
			->setCellValue('F1', WSTLang('payment_method'))->setCellValue('G1',  WSTLang('source_payment'))->setCellValue('H1', WSTLang('delivery_mode'))->setCellValue('I1', lang('invoice_status'))->setCellValue('W1', lang('buyer_message'))->setCellValue('J1', lang('invoice_information'))
			->setCellValue('K1', lang('order_goods'))->setCellValue('L1', lang('commodity_price'))->setCellValue('M1', lang('number'))->setCellValue('N1', lang('total_order_amount'))->setCellValue('O1', lang('freight'))->setCellValue('P1', lang('amount_actually_paid'))
			->setCellValue('Q1', lang('order_time'))->setCellValue('R1', lang('payment_time'))->setCellValue('S1', lang('delivery_time'))->setCellValue('T1', lang('receiving_time'))->setCellValue('U1', lang('reasons_for_cancellation/rejection'))->setCellValue('V1', lang('refund_or_not'));
	...
	{
		$payPwd = input('payPwd');
		if (!$payPwd) return WSTReturn(lang('please_input_password'), -1);
		...
		if ($isEnbalePay == 0) return WSTReturn(lang('op_err'), -1);
		//判断订单状态
		$where = ["shopId" => $shopId, "dataFlag" => 1, "orderStatus" => -2, "isPay" => 0, "payType" => 1];
		if ($isBatch == 1) {
			$where['orderunique'] = $orderNo;
		...
		if (count($orders) == 0) return WSTReturn(lang('Your_order_has_been_paid'), -1);
		//判断订单金额是否正确
		$needPay = 0;
		foreach ($orders as $v) {
			$needPay += $v->needPay;
		...
		if ($user->payPwd == '') return WSTReturn(lang("Please_set_the_payment_password_first"), -1);
		if ($user->payPwd != md5($payPwd . $user->loginSecret)) return WSTReturn(lang('Incorrect_payment_password'), -1);
		if ($needPay > $shop->shopMoney) return WSTReturn(lang('Sorry_your_credit_is_running_low'), -1);
		...
		try {
			...
			foreach ($orders as $order) {
				...
				if (false != $result) {
					...
					$lm['dataSrc'] = 1;
					$lm['remark'] = json_encode(['type'=>'lang_params','key'=>'transaction_order_expense', 'params'=>[$order->orderNo, $tmpNeedPay]]);
					$lm['moneyType'] = 0;
					$lm['money'] = $tmpNeedPay;
					$lm['payType'] = 'wallets';
			...
			Db::commit();
			return WSTReturn(lang('order_paid_successfully'), 1);
		} catch (\Exception $e) {
			//print_r($e);
			Db::rollback();
			return WSTReturn(lang('order_paid_failure'));
		}
	}
