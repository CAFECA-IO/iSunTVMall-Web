...
class Shops extends CShops{
    ...
    */
    ...
    public function getShopSummary($shopId){
        ...
        $stat['goodsTop'] = $rs = Db::table($prefix.'order_goods')->alias([$prefix.'order_goods'=>'og',$prefix.'orders'=>'o',$prefix.'goods'=>'g',$prefix.'goods_langs'=>'gl'])
                                          ->join($prefix.'orders','og.orderId=o.orderId')
                                          ->join($prefix.'goods','og.goodsId=g.goodsId')
                                          ->join($prefix.'goods_langs','gl.goodsId=g.goodsId')
                                          ->order('goodsNum desc')
                                          ...
                                          ->field('og.goodsId,gl.goodsName,goodsSn,sum(og.goodsNum) goodsNum,g.goodsImg')
                                          ->limit(10)->select();
    ...
    public function getShopAccount(){
        $shopId = (int)session('WST_USER.shopId');
        $shops = Db::name('shops')->alias('s')->join('banks b','b.bankId=s.bankId','inner')->join('__BANKS_LANGS__ bl','bl.bankId=b.bankId and langId='.WSTCurrLang(),'inner')->where('s.shopId',$shopId)->field('bl.bankName,s.bankAreaId,bankNo,bankUserName,userId')->find();
        //获取店铺主账号微信openId
        $shops['wxOpenId'] = Db::name('users')->where('userId',$shops['userId'])->value('wxOpenId');
        return $shops;
    }
    /**
     * 保存入驻资料
    ...
    public function saveStep2($data = []){
        ...
        try{
            if(empty($shops)){
                ...
                if($shops['applyStatus']>=1)return WSTReturn(lang('please_do_not_repeat_the_application'));
    ...
    public function saveStep3($data = []){
        ...
        */
        ...
        if($shopId==0)return WSTReturn(lang('op_illegal'));
    ...
    */
    ...
    public function createPoster($userId,$qr_code,$outImg){
        ...
        $shopId = (int)session('WST_USER.shopId');
        $shop = Db::name("shops")->where(['shopId'=>$shopId])->field("shopId,shopImg")->find();
        ...
        $new_qrcode = WSTConf('CONF.resourceDomain').'/'.$shop['shopImg'];

        if( !(@fopen( $new_qrcode, 'r' )) ){
            return WSTReturn(lang('Failed_to_generate_The_main_graph_does_not_exist'));
        }
        $new_qr_width = 80;
        $tmpImg = "";
        if($vtype==2){
            $tmpImg = WSTRootPath().'/upload/shares/shop/'.date("Y-m").'/'.$shopId.'.jpg';
        ...
        //重新组合图片并调整大小
        imagecopyresampled($share_bg, $new_qrcode, $from_width, $from_width, 0, 0, $new_qr_width, $new_qr_height, $new_width, $new_height);
