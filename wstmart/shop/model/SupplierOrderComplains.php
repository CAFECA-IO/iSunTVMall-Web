	...
	public function queryUserComplainByPage()
	{
		...
		foreach ($rs['data'] as $k => $v) {
			if ($v['complainStatus'] == 0) {
				$rs['data'][$k]['complainStatus'] = lang('waiting_for_processing');
			} elseif ($v['complainStatus'] == 1) {
				$rs['data'][$k]['complainStatus'] = lang('waiting_for_the_respondent_response');
			} elseif ($v['complainStatus'] == 2 || $v['complainStatus'] == 3) {
				$rs['data'][$k]['complainStatus'] = lang('pending_arbitration');
			} elseif ($v['complainStatus'] == 4) {
				$rs['data'][$k]['complainStatus'] = lang('arbitrated');
			}
	...
	public function saveComplain()
	{
		...
		try {
			...
			if ($rs !== false) {
				...
				Db::commit();
				return WSTReturn(lang('the_complaint_has_been_submitted_please_pay_attention_to_the_mall_notice_information'), 1);
			} else {
				return WSTReturn($this->getError(), -1);
			}
		} catch (\Exception $e) {
