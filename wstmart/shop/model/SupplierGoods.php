...
class SupplierGoods extends Base
{
	...
	public function getBySale($goodsId)
	{
		...
		$rs = Db::name('supplier_goods')
            ->alias('g')
            ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
            ->field('g.*,sgl.goodsName,sgl.goodsTips,sgl.goodsDesc,sgl.goodsSeoKeywords,sgl.goodsSeoDesc')
		...
		if (!empty($rs)) {
			...
			if ($rs['isSpec'] == 1) {
				...
				$specs = Db::name('spec_cats')->alias('gc')->join('supplier_spec_items sit', 'gc.catId=sit.catId', 'inner')
                    ->join('__SPEC_CATS_LANGS__ scl','scl.catId=gc.catId and scl.langId='.WSTCurrLang())
                    ->join('__SUPPLIER_SPEC_ITEMS_LANGS__ sil','sil.itemId=sit.itemId and sil.langId='.WSTCurrLang())
			...
			$rs['attrs'] = Db::name('attributes')->alias('a')->join('supplier_goods_attributes ga', 'a.attrId=ga.attrId', 'inner')
                ...
                ->join('__SUPPLIER_GOODS_ATTRIBUTES_LANGS__ gal','gal.goodsAttrId=ga.id and gal.langId='.WSTCurrLang())
	...
	public function pageQuery($goodsCatIds = [])
	{
		...
		$list = Db::name("supplier_goods")->alias('g')
            ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
            ->join("suppliers s", "g.supplierId = s.supplierId")
			->where($where)
			->where(function ($query) use ($where2) {
				$query->whereOr($where2);
			})
			->where($condition)
			->field('g.goodsId,sgl.goodsName,goodsSn,goodsStock,isNew,saleNum,supplierPrice,marketPrice,isSpec,goodsImg,appraiseNum,visitNum,s.supplierId,supplierName,isFreeShipping,gallery')
		...
		if (!empty($list['data'])) {
			foreach ($list['data'] as $key => $v) {
				$list['data'][$key]['tags'] = [];
				if ($v['isFreeShipping'] == 1) $list['data'][$key]['tags'][] = "<span class='tag'>" . lang('free_shipping') . "</span>";
			}
		}
	...
	public function getById($goodsId)
	{
		$rs = $this
            ...
            ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
            ->field('g.*,sgl.goodsName,sgl.goodsTips,sgl.goodsDesc,sgl.goodsSeoKeywords,sgl.goodsSeoDesc')
            ->where(['g.goodsId' => $goodsId, 'dataFlag' => 1, 'isSale' => 1])->find();
		if (!empty($rs)) {
			...
			$specs = Db::name('spec_cats')->alias('gc')->join('supplier_spec_items sit', 'gc.catId=sit.catId', 'inner')
				...
				->where(['sit.goodsId' => $goodsId, 'gc.isShow' => 1, 'sit.dataFlag' => 1])
				->field('gc.isAllowImg,sit.catId,sit.itemId,sil.itemName,sit.itemImg')
				->order('gc.isAllowImg desc,gc.catSort asc,gc.catId asc')->select();
			$spec0 = [];
			$spec1 = [];
			foreach ($specs as $key => $v) {
			...
			$rs['attrs'] = Db::name('supplier_goods_attributes')->alias('ga')->join('attributes a', 'ga.attrId=a.attrId', 'inner')
				...
				->where('ga.goodsId', $goodsId)->field('ga.attrId,a.attrType,gal.attrVal')->select();
			if ($rs['isWholesale'] == 1) {
				$rs['wholesale']  = Db::name('supplier_wholesale_goods')->where('goodsId', $goodsId)->order('buyNum asc')->select();
				$wholesale = [];
				foreach ($rs['wholesale'] as $key => $v) {
	...
	public function add()
	{
		...
		if ($hasCopy) return WSTReturn(lang('you_have_distributed_the_goods'));
		$supplierGoods = Db::name("supplier_goods")
                        ->alias('g')
                        ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
                        ->field('g.*,sgl.goodsName,sgl.goodsTips,sgl.goodsDesc,sgl.goodsSeoKeywords,sgl.goodsSeoDesc')
                        ->where(['dataFlag' => 1, 'g.goodsId' => $supplierGoodsId])->find();
		if (empty($supplierGoods) || ($supplierGoods['isSale'] == 0 || $supplierGoods['goodsStatus'] == 0)) return WSTReturn(lang('invalid_product_information'));
		...
		if (empty($isApplyCatIds)) return WSTReturn(lang('your_store_has_not_opened_the_mall_classification', [$gcat['catName']]));
		...
		try {
			...
			$supplierGoodsLangs = Db::name('supplier_goods_langs')->where(['goodsId'=>$supplierGoodsId])->column('*','langId');
			...
			if (false !== $result) {
                ...
                foreach (WSTSysLangs() as $key => $v) {
                    ...
                    $dataLang['goodsName'] = $supplierGoodsLangs[$v['id']]['goodsName'];
                    $dataLang['goodsSeoKeywords'] = $supplierGoodsLangs[$v['id']]['goodsSeoKeywords'];
                    $dataLang['goodsSeoDesc'] = $supplierGoodsLangs[$v['id']]['goodsSeoDesc'];
                    $dataLang['goodsTips'] = $supplierGoodsLangs[$v['id']]['goodsTips'];
                    $dataLang['goodsDesc'] = $supplierGoodsLangs[$v['id']]['goodsDesc'];
                    //对图片域名进行处理
				...
				if ($data['goodsType'] == 0 && $data['isSpec'] == 1) {
                    ...
                    $specItemsLangs = Db::name('supplier_spec_items')
                        ->alias('sit')
                        ->join('__SUPPLIER_SPEC_ITEMS_LANGS__ sil','sil.itemId=sit.itemId','inner')
                        ->field('sit.itemId,sil.langId,sil.itemName')
                        ->where(['sit.goodsId' => $supplierGoodsId])->select();
					foreach($specItemsLangs as $k => $v){
                        $sitemLang = [];
                        $sitemLang['itemId'] = $itemIdMaps[$v['itemId']];
                        $sitemLang['langId'] = $v['langId'];
                        $sitemLang['goodsId'] = $goodsId;
                        $sitemLang['itemName'] = $v['itemName'];
				...
				foreach ($goodsAttributes as $key => $v) {
					...
					$attrIdMaps[$v['attrId']] = $goodsAttrId;
				}
				$goodsAttributesLangs = Db::name('supplier_goods_attributes')
                                        ->alias('ga')
                                        ->join('__SUPPLIER_GOODS_ATTRIBUTES_LANGS__ gal','gal.goodsAttrId=ga.id','inner')
                                        ->field('ga.attrId,gal.langId,gal.attrVal')
                                        ->where(['ga.goodsId' => $supplierGoodsId])->select();
				foreach($goodsAttributesLangs as $k => $v){
                    $attrLang = [];
                    $attrLang['goodsAttrId'] = (int)$attrIdMaps[$v['attrId']];
                    $attrLang['langId'] = $v['langId'];
                    $attrLang['goodsId'] = $goodsId;
                    $attrLang['attrVal'] = $v['attrVal'];
                    $attrLangs[] = $attrLang;
                }
                //保存商品属性多语言
	...
	public function supplierGoods($supplierId)
	{
		...
		$goods = Db::name('supplier_goods')->alias('g')
			...
			->field('g.goodsId,sgl.goodsName,g.goodsImg,g.supplierPrice,g.marketPrice,g.saleNum,g.appraiseNum,g.goodsStock,g.isFreeShipping,gallery')
			->orderRaw($order)
	...
	public function listSupplierCats($parentId = 0, $num, $supplierId = 0, $cache = 0)
	{
		...
		$cacheData = WSTCache('TAG_SUPPLIER_CATS_' . $supplierId . "_" . $parentId . "_" . $num . "_" . $cache);
		...
		$data = Db::name('supplier_cats')
			...
			->field('a.catId,supplierId,scl.catName')
			->limit($num)->where($where)
			->order('catSort asc')->select();
		WSTCache('TAG_SUPPLIER_CATS_' . $supplierId . "_" . $parentId . "_" . $num . "_" . $cache, $data, $cache, 'CACHETAG_SUPPLIER_' . $supplierId);
		return $data;
	}
	...
	public function listSupplierGoods($type, $supplierId, $cat = 0, $num = 0, $cache = 0)
	{
		...
		$goods = Db::name('supplier_goods')
            ->alias('g')
            ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
			->where($where)->field('g.goodsId,sgl.goodsName,goodsImg,goodsSn,goodsStock,saleNum,supplierPrice,marketPrice,isSpec,appraiseNum,visitNum')
