...
class SupplierGoodsAppraises extends Base
{
	public function queryByPage($sId = 0)
	{
		...
		$data = Db::name("supplier_goods g")
			...
			->field('u.userPhoto, u.loginName, g.goodsId,g.goodsImg,sgl.goodsName,ga.supplierReply,ga.id gaId,ga.replyTime,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,u.loginName')
	...
	public function userAppraise()
	{
		...
		$data = Db::name("supplier_goods g")
            ->join('__SUPPLIER_GOODS_LANGS__ sgl','sgl.goodsId=g.goodsId and sgl.langId='.WSTCurrLang())
			->field('g.goodsId,g.goodsImg,sgl.goodsName,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,ga.supplierReply,ga.replyTime,s.supplierName,u.userName,o.orderNo')
			->join('supplier_goods_appraises ga', 'g.goodsId=ga.goodsId', 'inner')
	...
	public function add($uId = 0)
	{
		...
		// 没有传order_goods表的id
		if ($orderGoodsId == 0) return WSTReturn(lang('op_err'));
		...
		if (isset($content)) {
			if (!WSTCheckFilterWords($content, WSTConf("CONF.limitWords"))) {
				return WSTReturn(lang('the_content_contains_illegal_characters'));
			}
		}
		...
		if (empty($orders)) return WSTReturn(lang('invalid_orders'));
		...
		if ($apCount > 0) return WSTReturn(lang('the_product_has_been_appraised'));
		Db::startTrans();
		try {
			//增加订单评价
			$data = [];
