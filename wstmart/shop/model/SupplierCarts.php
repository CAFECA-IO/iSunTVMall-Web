...
class SupplierCarts extends Base{
	...
	public function addCart($uId = 0){
		...
		if($userId==0)return WSTReturn(lang('not_logged'),-2);
		//验证传过来的商品是否合法
		$chk = $this->checkGoodsSaleSpec($goodsId,$goodsSpecId);
		if($chk['status']==-1)return $chk;
		//检测库存是否足够
		if($chk['data']['stock']<$cartNum)return WSTReturn(lang('insufficient_inventory_of_goods'), -1);
		//添加实物商品
		if($chk['data']['goodsType']==0){
            ...
            return WSTReturn(lang('op_ok'), 1,['forward'=>'quickSettlement']);
		}
	...
	public function checkGoodsSaleSpec($goodsId,$goodsSpecId){
		...
		if(empty($goods))return WSTReturn(lang('invalid_product_information'), -1);
		...
		if($goods['isSpec']==1){
			...
			if(count($specs)==0){
				return WSTReturn(lang('invalid_product_information'), -1);
			}
			...
			if($defaultGoodsSpecId==0)return WSTReturn(lang('invalid_product_information'), -1);//有规格却找不到规格的话就报错
	...
	public function getCarts($isSettlement = false, $uId=0){
		...
		$rs = Db::table($prefix.'supplier_carts')
		           ->alias([$prefix.'supplier_carts'=>'c',$prefix.'supplier_goods' => 'g',$prefix.'suppliers' => 's',$prefix.'supplier_goods_specs' => 'gs',$prefix.'supplier_goods_langs'=>'sgl'])
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,s.userId,s.supplierId,s.supplierName,g.goodsId,g.shippingFeeType,g.supplierExpressId,s.supplierQQ,supplierWangWang,sgl.goodsName,g.supplierPrice,g.supplierPrice defaultSupplierPrice,g.goodsStock,g.goodsWeight,g.goodsVolume,g.isSpec,gs.specPrice,gs.specStock,gs.specWeight,gs.specVolume,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.goodsCatId,g.isFreeShipping,s.isInvoice')
		           ->select();
		$carts = [];
		$goodsIds = [];
		$goodsTotalNum = 0;
		...
		if(count($goodsIds)>0){
		    $specs = DB::name('supplier_spec_items')->alias('s')->join('__SPEC_CATS__ sc','s.catId=sc.catId','left')
                ...
                ->join('__SUPPLIER_SPEC_ITEMS_LANGS__ sil','sil.itemId=s.itemId','inner')
	...
	public function getCartInfo($isSettlement = false,$uId = 0){
		...
		$rs = $this->alias('c')->join('supplier_goods g','c.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,g.goodsId,sgl.goodsName,g.supplierPrice,g.goodsStock,g.isSpec,gs.specPrice,gs.specStock,g.goodsImg,c.isCheck,gs.specIds,c.cartNum')
		           ->select();
		$goodsIds = [];
		$goodsTotalMoney = 0;
		$goodsTotalNum = 0;
	...
	public function batchChangeCartGoods($uId = 0){
		$ids = input('ids');
		if($ids=='')return WSTReturn(lang('op_err'));
