...
class Lang
{
    ...
    {
        $this->setAllowLangList(config('app.allow_lang_list'));
        // 自动侦测设置获取语言选择
        $langSet = config('app.default_lang');
        if (isset($_GET[$this->langDetectVar])) {
            // url中设置了语言变量
            $langSet = strtolower($_GET[$this->langDetectVar]);
            setcookie($this->langCookieVar, $langSet, time()+2678400);
            session($this->langDetectVar,$langSet);
        }elseif(session($this->langDetectVar)!=''){
            $langSet = session($this->langDetectVar);
            setcookie($this->langCookieVar, $langSet, time()+2678400);
